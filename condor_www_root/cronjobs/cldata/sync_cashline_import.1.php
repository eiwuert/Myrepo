...
$prefix          = "";
//$prefix          = "rc_";
$email_list      = "jason.gabriele@sellingsource.com,andy.roberts@sellingsource.com,mike.genatempo@sellingsource.com,brian.feaver@sellingsource.com";
...
foreach($data as $property => $values)
{
    ...
    {
		system("/usr/bin/scp cashline@drop1.clkonline.com:cldata/$property/$file $local_directory$property/$file");
        if(!file_exists($data[$property]["file"]))
        {
        	//Send Email
        	mail(
        		$email_list,
        		'Missing Cashline File',
        		"Cashline file: {$data[$property]['file']} does not exist.");
        }
    }
    else
    {
        ...
        if(!(file_exists($local_directory . $property . "/" . $file)))
        {
            ...
            if(!file_exists($local_directory . $property . "/" . $file) && date("N", strtotime("-$day_count day"))!=1)
            {
            	...
            	mail(
            		$email_list,
            		'Missing Cashline File',
            		"Cashline file: $local_directory$property/$file does not exist."
            	);
            }
        }
        else
...
foreach($data as $property => $values)
{
    ...
    if((strlen($values["file"]) > 0) && $values["continue"] != FALSE)
    {
        // Open the file for reading.
        if(($ofile = fopen($values["file"], "r")))
        {
        	// Don't delete the previous data unless we have to. [BrianF]
