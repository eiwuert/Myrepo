...
function Get_Pending_Complete_Apps($mode)
{
	...
	$query = "
			...
			FROM application a
				INNER JOIN personal p USING (application_id)
				...
				INNER JOIN campaign_info c USING(application_id)
			WHERE
				a.modified_date BETWEEN '{$thirty_ago}' AND '{$fifteen_ago}'
				AND a.application_type IN ('PENDING', 'CONFIRMED')
				AND a.target_id IN ('74','227')";
