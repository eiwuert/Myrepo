...
my $sqlx = "TRUNCATE TABLE ucl";
my $sthx = $dbh2->prepare($sqlx) ;
   $sthx->execute() ;
print "Empty table ucl - MySQL...\n"; 


print "Test date: $lastweek\n";
 
$dbh->do("use ucl") or die "Couldn't execute statement: " . $dbh->errstr;
...
$result = $sth->execute or die "Couldn't execute statement: " . $sth->errstr;
#print "Result: $result \n";
...
while (my ($custnum,$status,$fn,$ln,$email,$ssn,$trantype,$dob,$datepaid) = $sth->fetchrow_array) {
		...
		white($ssn);
   	    ...
   	    {$dob="";}
     		...
     		$sql =  qq{insert into ucl (birthdate,email,lastname,firstname,ssnumber,status)  values ("$dob","$email","$fn","$ln","$ssn","$status")};
 	 		#print "$sql\n";
