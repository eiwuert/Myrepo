...
my $sqlx = "TRUNCATE TABLE pcl";
my $sthx = $dbh2->prepare($sqlx) ;
   $sthx->execute() ;
print "Empty table pcl - MySQL...\n"; 
...
while (my ($custnum,$status,$fn,$ln,$email,$ssn,$trantype,$dob,$datepaid) = $sth->fetchrow_array) {
		...
		white($ssn);
   	    ...
   	    {$dob="";}
     		...
     		$sql =  qq{insert into pcl (birthdate,email,lastname,firstname,ssnumber,status)  values ("$dob","$email","$fn","$ln","$ssn","$status")};
 	 		#print "$sql\n";
