...
my $sqlx = "TRUNCATE TABLE ca";
my $sthx = $dbh2->prepare($sqlx) ;
   $sthx->execute() ;
print "Empty table ca - MySQL...\n"; 



print "$lastweek\n";
 
$dbh->do("use cashadvance");
...
while (my ($custnum,$status,$fn,$ln,$email,$ssn,$trantype,$dob,$datepaid) = $sth->fetchrow_array) {
		...
		white($ssn);
   	    ...
   	    {$dob="";}
     		...
     		$sql =  qq{insert into ca (birthdate,email,lastname,firstname,ssnumber,status)  values ("$dob","$email","$fn","$ln","$ssn","$status")};
 	 		#print "$sql\n";
