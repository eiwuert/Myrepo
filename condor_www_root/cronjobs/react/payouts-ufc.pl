...
my $sqlx = "TRUNCATE TABLE ufc";
my $sthx = $dbh2->prepare($sqlx) ;
   $sthx->execute() ;
print "Empty table ufc - MySQL...\n"; 
		...
		white($ssn);
  	    ...
  	    if($dob >=1 )
   	    ...
   	    {$dob="";}
     		...
     		$sql =  qq{insert into ufc (birthdate,email,lastname,firstname,ssnumber,status)  values ("$dob","$email","$fn","$ln","$ssn","$status")};
 	 		#print "$sql\n";
