...
//	require_once("replace.php");
	...
	$send_to = "nathanielm@partnerweekly.com, pamelas@partnerweekly.com, jake.ludens@partnerweekly.com, carolinem@partnerweekly.com, jude.parman@partnerweekly.com, pennied@partnerweekly.com, brianr@sellingsource.com";
	...
	$start_date = mktime(0, 0, 0, $local_time['tm_mon'] + 1, $local_time['tm_mday'] - 7, $local_time['tm_year'] + 1900);
	...
	foreach (array_keys($agents) as $agent_id)
	{
		...
		for ($day = 0; $day < 7; $day++)
		{
			$query_start_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + $day, $start_localtime['tm_year']), 1);
			$query_start_localtime['tm_mon'] += 1;
			$query_start_localtime['tm_year'] += 1900;
			$query_end_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + $day + 1, $start_localtime['tm_year']), 1);
			...
			$agents[$agent_id][$day] = get_single_result($query);
		}
	}
	...
	for ($day = 0; $day < 7; $day++)
	{
		$header_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + $day, $start_localtime['tm_year']), 1);
		$header_localtime['tm_mon'] += 1;
		
		$day_headers[$day] = $header_localtime['tm_mon']."/".$header_localtime['tm_mday'];
	}
		
	$email_text = "<html><body>\n";
	$email_text .= "Top Performers 7 Day Unique Clicks\n";
	$email_text .= '<table cellspacing="2" cellpadding="2" border="1">'."\n";
	$email_text .= "<tr><td>Company Name</td><td>First Name</td><td>Last Name</td>";
	for ($i = 0; $i < 7; $i++)
	{
		$email_text .= "<td>$day_headers[$i]</td>";
	}
	...
	mail ($send_to, "Top Performers 7 Day Unique Clicks", $email_text, "From: stats@partnerweekly.com\r\nContent-Type: text/html; charset=\"iso-8859-1\"\r\n");
exit;
