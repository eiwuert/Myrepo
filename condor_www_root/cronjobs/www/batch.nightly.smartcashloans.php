...
$sql = "SELECT 
		...
		FROM 
			campaign_info as c 
		JOIN personal p USING (application_id)
...
$recipients = array(
	//array("email_primary_name" => "Test", "email_primary" => "stephan.soileau@sellingsource.com"),
...
if($res->Row_Count() > 0)
{
	$fp = fopen("/tmp/smartcashloans.csv","w");
	...
	$header = array("sender_name"       => "Selling Source <no-reply@sellingsource.com>",
				    "subject" 	        => "Apps for SmartCashLoans for " . date("m/d/y",$yesterday),
				    "site_name" 	    => "sellingsource.com",
				    "message"			=> "Apps for SmartCashLoans for " . date("m/d/y",$yesterday));

	$out = file_get_contents('/tmp/smartcashloans.csv');

	$attach = array(
		'method' => 'ATTACH',
		'filename' => 'smartcashloans.csv',
