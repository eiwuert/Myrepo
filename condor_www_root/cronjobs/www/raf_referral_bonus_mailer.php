...
/*
	cronjob to send invitations for cash4friends.com, to people who have received a loan
	meant to be run from the cmdline

	This is copied from olp_paperless_event_mailers.php and probably has some dead code left over
*/


define('RC', TRUE); // meant to be true for live
define('DEBUG', RC || !(bool)preg_match('/ws0*[12]/', trim(`hostname`)));
// if DEBUG is set to true, user's email will be replaced with this one
define('DEBUG_EMAIL',	'ben.upsavs@thesellingsource.com');
//define('DEBUG_EMAIL',	'debug@tssmasterd.com');
// who to email when the script fails?
define('BAIL_NOTIFY',	DEBUG_EMAIL); //'dona@sellingsource.com');
...
define('OLE_EVENT',     'RAF_NOTICE');

if (DEBUG)
{
	echo "OLE_EVENT: '" . OLE_EVENT . "'\n";
}
...
$companies = array(
		"CA"	=> array(
			"db"	=> "olp_ca_visitor_archive"
			,"db_type"	=> "DB2"
			,"id"	=> 1581
			),
		"PCL"	=> array(
			"db"	=> "olp_pcl_visitor_archive"
			,"db_type"	=> "DB2"
			,"id"	=> 3018
			),
		"UFC"	=> array(
			"db"	=> "olp_ufc_visitor_archive"
			,"db_type"	=> "DB2"
			,"id"	=> 17208
			),
		"D1"	=> array(
			"db"	=> "olp_d1_visitor_archive"
			,"db_type"	=> "DB2"
			,"id"	=> 17208
			),
		"UCL"	=> array(
			"db"	=> "olp_ucl_visitor_archive"
			,"db_type"	=> "DB2"
			,"id"	=> 1583
			),
		);
...
//	$latest = date("YmdHis", $target + (TIME_WINDOW * 60));
	...
	$earliest = date('YmdHis', mktime(0, 0, 0, 2, 3, 2005));
	$latest = date('YmdHis'); // Today
...
//	$latest = date("YmdHis", $target + (TIME_WINDOW * 60));
	...
	SELECT 
		...
		,rtrim(customer.name_first) AS name_first
		,rtrim(customer.name_last) AS name_last 
		...
		,stat_info.confirmed as CONFIRMED
		,login.crypt_password AS hashed_password
		,originating_source.name AS site_name
		,originating_source.license_key AS license_key
		,'" . $property_short . "' AS property_short 
	FROM 
		{$schema}.TRANSACTION AS transaction LEFT JOIN {$schema}.EXCEPTION as exception ON (exception.transaction_id=transaction.transaction_id) LEFT JOIN {$schema}.EXCEPTION_TYPE as exception_type ON (exception.exception_type_id=exception_type.exception_type_id) LEFT JOIN {$schema}.EMAIL AS email ON (email.email_id=transaction.active_email_id) LEFT JOIN {$schema}.CUSTOMER AS customer ON (customer.customer_id=transaction.customer_id) LEFT JOIN {$schema}.LOGIN AS login ON (login.customer_id = customer.customer_id), 
		{$schema}.ORIGINATING_SOURCE AS originating_source, 
		{$schema}.STAT_INFO AS stat_info
	WHERE originating_source.originating_source_id = transaction.originating_source_id 
	AND stat_info.transaction_id = transaction.transaction_id 
	AND ".fetch_app_type('db2')."
	AND exception.exception_id is NULL 
...
//		$data['email'] = $data['email_primary'] = DEBUG_EMAIL;
		...
		// I had to reset $mailer every time or php crashed
		unset($mailer);
		$mailer = NULL;
		$mailer = new prpc_client(OLE_SMTP_URL);
		...
		$sent[(boolean)$mail][] = intval($user["app_id"]);
		//$sent[(bool)is_array($mail)][] = intval($user["app_id"]); // Didn't work
