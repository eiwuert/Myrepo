...
//	define ("PASS", "");
	...
	// databases to be cleaned
	$property_databases = array('ca', 'pcl', 'ucl');

	foreach($property_databases as $current_property)
	{
		// set database to clean
		$visitor_db = 'olp_'.$current_property.'_visitor';
		...
		while ($stamp < strtotime("-1 day") && $day)
		{
			...
			echo date("m/d/y",$stamp)." ".$total_found."\n";
			...
			if(is_array ($delete_array))
			{
				...
				{
					$query = "delete from `session_site` where session_id = '".mysql_escape_string($ses_id)."'";
					$result = $sql->Query ($visitor_db, $query, Debug_1::Trace_Code (__FILE__, __LINE__));
					Error_2::Error_Test ($result, TRUE);
					echo $query."\n";
				}
		...
		$day = $session->mindate;
		$stamp = mktime(0,0,0,substr($day,4,2),substr($day, 6,2),substr($day, 0, 4));
		while ($stamp < strtotime("-2 weeks") && $day)
		{
			// Pull the user information
			$query = "select * from `session_site` where modifed_date between '".date("YmdHis", $stamp)."' AND '".date("YmdHis", strtotime("+1 day", $stamp))."'";
			$result = $sql->Query ($visitor_db, $query, Debug_1::Trace_Code (__FILE__, __LINE__));
			...
			while ($session = $sql->Fetch_Object_Row ($result))
			{
				...
				Partial_Insert_Database($sql, $current_property);
				$delete_array[] = $session->session_id;
			}
			//exit;
			if(is_array ($delete_array))
