...
class olp_insert{
	function olp_insert($db2,$sql,$session_id=NULL,$session_property=NULL,$session_info=NULL,$session_date=NULL)
	{
		$this->db2 = $db2;
		$this->sql = $sql;
		if (isset($session_id) && isset($session_property) && isset($session_info))
		{
			...
			$session_date = preg_replace("/-|\s|:/","",$session_date);
			list($insert_date,$insert_micro) = split("\.",$session_date);
			$this->stamp = $insert_date;
	...
	//	@session_start();
		...
		{
			$query = "INSERT INTO application (application_id,account_id,session_id,created_date,modified_date) 
						VALUES('','" . str_shuffle($this->session_id) ."','" .$this->session_id."','".$this->stamp."','".$this->stamp."')";
	...
	function delete_row()
	{
		$query = "delete from ".USER.".session where session_id='" . $this->session_id . "'";
		$del_row = $this->db2->Execute($query);
		Error_2::Error_Test($del_row,TRUE);
	}
}
// end classes
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	**/
	$connection_type = "live"; // local or live
	switch($connection_type)
	{
		...
		case "live":
		define ("USER", "session");
		define ("PASS", "session_scrub");
		define ("DBNAME","OLP");
		define ("MySQLHOST" , "selsds001");
		define ("MySQLUSER" , "sellingsource");
		define ("MySQLPASS" , "%selling\$_db");
		break;
	}
	...
	$sql = new MySQL_3();
	$db2 = new Db2_1(DBNAME,USER,PASS);
	Error_2::Error_Test ($db2->Connect (), TRUE);

	$query = "select min(date_modified) as mindate from ".USER.".session";
...
//echo $query."\n";
//exit;
//	$query = "select session_id,session_info from ".USER.".session where session_id='08caae67b4e49c4d74ec8761a9442b67'";
//	$query = "select session_id,session_info,date_created from ".USER.".sessiofetch first 500 row only";
	...
	//	echo "record\n";
		...
		else
		{
			...
			$sesinfo = new serialize_db2();
			$session_data = $sesinfo->unserialize_data($data->SESSION_INFO);
			$do_insert = new olp_insert($db2,$sql,$data->SESSION_ID,$session_data['config']->property_short,$session_data,$data->DATE_CREATED);
