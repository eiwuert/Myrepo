...
//	require_once("replace.php");
	...
	// build the end date
	$end_date = mktime(0, 0, 0, $local_time['tm_mon'] + 1, $local_time['tm_mday'], $local_time['tm_year'] + 1900);
	// build the start date
	$start_date = mktime(0, 0, 0, $local_time['tm_mon'] + 1, $local_time['tm_mday'] - 56, $local_time['tm_year'] + 1900);
	...
	foreach (array_keys($agents) as $agent_id)
	{
		...
		{
			$query_start_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + $week * 7, $start_localtime['tm_year']), 1);
			$query_start_localtime['tm_mon'] += 1;
			$query_start_localtime['tm_year'] += 1900;
			$query_end_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + ($week + 1) * 7, $start_localtime['tm_year']), 1);
			...
			$agents[$agent_id][$week] = get_single_result($query);
		}
	}
	...
	for ($week = 0; $week < 8; $week++)
	{
		$header_start_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + $week * 7, $start_localtime['tm_year']), 1);
		$header_start_localtime['tm_mon'] += 1;
		$header_end_localtime = localtime(mktime(0, 0, 0, $start_localtime['tm_mon'], $start_localtime['tm_mday'] + (($week + 1) * 7) - 1, $start_localtime['tm_year']), 1);
		$header_end_localtime['tm_mon'] += 1;
		
		$week_headers[$week] = $header_start_localtime['tm_mon']."/".$header_start_localtime['tm_mday']."-".$header_end_localtime['tm_mon']."/".$header_end_localtime['tm_mday'];
	}
		
	$email_text = "<html><body>\n";
	$email_text .= "Top Performers 8 Week Unique Clicks\n";
	$email_text .= '<table cellspacing="2" cellpadding="2" border="1">'."\n";
	$email_text .= "<tr><td>Company Name</td><td>First Name</td><td>Last Name</td>";
	for ($i = 0; $i < 8; $i++)
	{
		$email_text .= "<td>Week of $week_headers[$i]</td>";
	}
	...
	mail ($send_to, "Top Performers 8 Week Unique Clicks", $email_text, "From: stats@partnerweekly.com\r\nContent-Type: text/html; charset=\"iso-8859-1\"\r\n");
exit;
