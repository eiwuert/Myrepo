<?php
	...
	$result = $sql->Query ("olp_ucl_visitor", $query, Debug_1::Trace_Code (__FILE__, __LINE__));
	Error_2::Error_Test ($result, TRUE);
	...
	while ($session = $sql->Fetch_Array_Row ($result))
	{
		...
		if ($data["application_id"])
		{
			if($data["completed"]["page4"])
			{
				...
				$app_result = $sql->Query ("olp_ucl_visitor", $query, Debug_1::Trace_Code (__FILE__, __LINE__));
