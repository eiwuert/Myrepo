...
class olp_insert{
	...
	function run_insert()
	{
		...
		{
			$query = "INSERT INTO application (application_id,account_id,session_id) 
						VALUES('','" . str_shuffle($this->session_id) ."','" .$this->session_id."')";
...
//	$query = "select session_id,session_info from ".USER.".session where session_id='08caae67b4e49c4d74ec8761a9442b67'";
	$query = "select session_id,session_info from ".USER.".session fetch first 2000 row only";
	$sql_select = $db2->Query($query);
	Error_2::Error_Test($sql_select,TRUE);
	$db2_select = $db2->Execute($query);
