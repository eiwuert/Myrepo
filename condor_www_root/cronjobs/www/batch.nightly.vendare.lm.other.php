<?php
	...
	if ( MODE == MODE_LEGACY )
	{
		$st = date("Y-m-d", strtotime("-90 days"));
		$end = date("Y-m-d");
		$fn = "VENDARE_OTHER_LIST_LEGACY.TXT";
	}
	else if ( MODE == MODE_NORMAL || MODE==MODE_TEST )
	{
		$st = date("Y-m-d", strtotime("-1 day"));
		$end = date("Y-m-d");
		$fn = "VENDARE_OTHER_LIST_$st.TXT";		
	}
	...
	$Result_olp_bb_visitor=$SQL->query("lp","
		SELECT
		first_name,
		last_name,
		email,
		ip_address,
		landing_page_id, 
		date_format(created_date,'%m/%d/%Y') cdt
		FROM visitors
		where created_date between '$st' and '$end'
		and landing_page_id in (2,6)
		AND first_name!=''
		AND first_name NOT LIKE '%TEST'
		AND first_name NOT LIKE 'TEST%'
		AND first_name NOT LIKE '%SHIT%'
		AND first_name NOT LIKE '%SPAM%'
		AND first_name NOT LIKE '%FUCK%'
		AND first_name NOT LIKE '%BITCH%'
		AND last_name!=''
		AND last_name NOT LIKE '%TEST'
		AND last_name NOT LIKE 'TEST%'
		AND last_name NOT LIKE '%SHIT%'
		AND last_name NOT LIKE '%SPAM%'
		AND last_name NOT LIKE '%FUCK%'
		AND last_name NOT LIKE '%BITCH%'
		AND email!=''
		AND email NOT LIKE '%TEST'
		AND email NOT LIKE 'TEST%'
		AND email NOT LIKE '%SHIT%'
		AND email NOT LIKE '%SPAM%'
		AND email NOT LIKE '%FUCK%'
		AND email NOT LIKE '%BITCH%'
		AND email NOT LIKE '%ABUSE'
		AND email NOT LIKE '%INTERNIC%'
		AND email NOT LIKE '%NETWORKSOLUTIONS%'
		AND email NOT LIKE '%TSSMASTERD%'
		group by email
	...
	$rst = $SQL->query("pw_visitor","
		select
		first_name,
		last_name,
		email,
		ip_address,
		1337 landing_page_id,
		date_format(created_date, '%m/%d/%Y') cdt
		from thegameplayer
		where created_date between '$st' and '$end'
				AND first_name!=''
				...
				AND email LIKE '%@%'
				", Debug_1::Trace_Code(__FILE__,__LINE__));
	Error_2::Error_Test($rst,TRUE);
	
	print "\nTGP DB count - " . $SQL->row_count($rst);
	$total_records += $SQL->Row_Count($rst);
	while ( $object = $SQL->Fetch_Object_Row($rst) )
	{
		$object_collection[] = $object;
	}

	$rst = $SQL->query("tss_visitor","
		select
		first_name,
		last_name,
		email,
		...
		where created_date between '$st' and '$end'
				...
				AND email NOT LIKE '%INTERNIC%'
				AND email NOT LIKE '%NETWORKSOLUTIONS%'
				AND email NOT LIKE '%TSSMASTERD%'", Debug_1::Trace_Code(__FILE__,__LINE__));
	Error_2::Error_Test($rst,TRUE);
	
	print "\nOLD LPTP count - " . $SQL->row_count($rst);
	$total_records += $SQL->Row_Count($rst);
	while ( $object = $SQL->Fetch_Object_Row($rst) )
	{
		$object_collection[] = $object;
	...
	print "\nOLD LPTPNY count - " . $SQL->row_count($rst);
	$total_records += $SQL->Row_Count($rst);
	while ( $object = $SQL->Fetch_Object_Row($rst) )
	{
		$object_collection[] = $object;
...
";
	...
	{
		switch ( $row->landing_page_id )
		{
			case 400:
			case 2: $lp = "greatweboffers.com/trimpatch"; break;
			case 401: $lp = "greatweboffers.com/trimpatch_newyear"; break;
			case 6: $lp = "familyhs.com"; break;
			case 1337: $lp = "thegameplayer.com"; break;
			
			default : break;
		}
		fwrite($fp_csv,"{$row->email}\t{$row->first_name}\t{$row->last_name}\t{$row->ip_address}\t$lp\t{$row->cdt}\r\n");
