...
*******************/
	...
	$olp->Connect();
	$csv = '';
	$file = 'grab_olpbbp_fa_'.date("YmdHis").'.csv';
	$putfile = "full_".date("YmdHis").'.csv';
	$localpath = '/tmp/'.$file;
	$stat_file = '/tmp/grab_olpbbp_fa_stat.log';
	...
	if ($total>=100000) {
		$msg = 'limit reached for grab_fullapp.php.  Remove from cron';
	...
	$olpr = $olp->Query('olp', $olpq);
	$count = $olp->Row_Count($olpr);
	if ($count<5000) {
		$msg = "Grab Full App Data below minimum count (".$count.")";
