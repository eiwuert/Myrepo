...
var holidays;
var CurrentPayment;
// There is too much interest retrieval done at page initialization, so I disable it for a couple seconds.
var interest_retrieve_disabled = true;
setTimeout(function () {
		interest_retrieve_disabled = false;
	}, 2000);

// The holiday list needs to be loaded and the most direct way is to do an api call.
//setTimeout(function () {
//	ecash_calculator({
//		id: 469, 
//		params:[{
...
{       
	...
	opts += ',height=380,left=50,top=0,screenX=25,screenY=0';
...
{       
	var opts = 'toolbar=no,location=no,directories=no,status=no,menubar=no';
	opts += ',scrollbars=yes,resizable=yes,copyhistory=no,width=506';
	opts += ',height=500,left=200,top=200,screenX=200,screenY=200';
...
function ToggleScheduled(chkbox)
{
	...
	table = document.getElementById("schedule_rows_table");
...
//	}
	...
	if(document.getElementById(base+"_date_"+(num_rows_visible-1)))
	{
		...
		{
			current_date = new Date(document.getElementById(base+"_date_"+(i)).value);
		...
		var last_date = document.getElementById(base+"_date_"+(latest_idx)).value;
	}
...
// While just calling "showCalendar" is easier, sometimes the position is jacked up b/c
// the element coordinates are taken from the container, which is a scrollable viewport
// in this case. Most of the code here is from "showCalendar", "showAtElement" and "showAt"
...
function PopCalendar(target, x, y, fund_date, pastDates, pt_dropdown, end_date, allowHolidays)
{

//	error_overlay(holidays);
	...
	var isACH = false;
	var typeInput = document.getElementById(target).parentNode.parentNode.firstChild.nextSibling.firstChild;
	if (typeInput != null && typeInput.selectedIndex != null) {
		var type = typeInput.options[typeInput.selectedIndex].value;
	...
	if (calendar != null)
	{
//		if (calendar.cachekey != pastDates + isACH)
//		{
			calendar.destroy();
			calendar = null;
//		} 
//		else 
//		{
//			calendar.onSelected = selectHandler;
//			calendar.hide();
//			calendar.setDate(calendar.parseDate(el.value));
//		}
	}
	if (calendar == null)
	{
		var cal = new Calendar(true, null, selectHandler, closeHandler);
		calendar = cal;
...
//				 Never on the weekends
				if (!hasHolidays && thisdate.getDay() == 6 || thisdate.getDay() === 0) {
					return true;
				}
				if (pastDates) {	
...
//				 Check if this day is a holiday
				...
				{
					if (isACH) {
						if (thisdate.toString() == today.toString()) {
							return true;
						}
					}
				}
...
function checkDate(input, past_date, pt_dropdown) {
...
{
	var ctrls = document.forms[base+'_payment_form'].getElementsById("INPUT");
...
function SaveAdjustment()
{
	...
	var desc = document.getElementById("adjustment_description").value;
...
// Our keyup handler.  Don't run a recalc immediately, but hang tight for a couple seconds to see if they enter more.
...
function AdjustArrangedSettlementAmounts(base, event, idx)
{
	...
	// If the event isn't null and the id passed the regexp test, we need to change a
	// "pure" value
	if ((event != null) && (event.target.id.match(re_amt)))
	{
	...
	var fee_balance     = parseFloat(document.getElementById(base+"_fee_balance").value);
	var interest_balance = parseFloat(document.getElementById(base+"_interest_balance").value);
...
function save_next_adjustment(application_id, company_id)
{
		var req = ecash_get_schedule({
			id: 0, 
			params:[
...
/* callback is what get_interest should call when its gotten the interest amount.  it will be called per this signature.
row, base, and principal are passthroughs the correponding values it was called with
callback(String base, String row, Float interest, Float principal);
*/
...
function populate_next_payment_adjustment (schedule) 
{
	...
	type_element.value = schedule[0].type;
	date_element.value = schedule[0].event_date;
	var total = 0;
	var i;
	for (i = 0; i < schedule.length && schedule[0].event_date == schedule[i].event_date; i ++) 
	{
	...
	for(i=i;i<schedule.length;i++)
	{
		...
		var type_element = document.getElementById("next_payment_adjustment_payment_type_" + i);
		var date_element = document.getElementById("next_payment_adjustment_date_" + i);
		var amount_element = document.getElementById("next_payment_adjustment_amount_" + i);
		var actual_amount_element = document.getElementById("next_payment_adjustment_actual_amount_" + i);
		if (!(type_element && date_element && amount_element && actual_amount_element)) {
			alert("Missing element" + type_element + ' - ' + date_element + ' - ' + amount_element + ' - ' + actual_amount_element );
		}
		type_element.value = arrangements[i].type;
		date_element.value = arrangements[i].date;
		amount_element.value = arrangements[i].amount;
...
function populate_payment_arrangements (arrangements) 
{
	...
	{
		//if its an adjustment_internal type transaction, that means its the discount!
		if(arrangements[i].type == 'adjustment_internal')
		{
			...
			var discountamt = (Math.abs(arrangements[i].amount)/principal_balance )*100;
		...
		else
		{
			...
			if (elements.type && elements.date && elements.amount && elements.actual_amount && elements.interest_range_begin && elements.interest_range_end) 
			{
				...
				if(gpo("payment_arrangement_display_action_effective").value == 'effective')
				{
					elements.date.value = arrangements[i].date;
				}
				else
				{
					elements.date.value = arrangements[i].event_date;
				}
...
function get_payout_total(base, row, principal, first_date, payment_date) {
	...
	var svc_charge_balance = parseFloat(document.getElementById("service_charge_balance_pending").value);
...
// Update a row.  Our principal WAS 'principal' and our interest charge IS 'interest' on ROW 'row' 
function AdjustArrangedSettlementAmountsRecursive(base, row, interest, previous_principal) 
{
	...
	var current_value;
	var re_amt = new RegExp(base+"_amount_(\\d+)");
	var re_act_amt = new RegExp(base+"_actual_amount_(\\d+)");
	...
	for (i = 0; i < num_rows_visible; i ++) {
		...
		current_interest = parseFloat(gpo(base+"_payment_row_"+i+"_interest").innerHTML);
...
//alert('fee paid off ' + remaining_sc + ' - ' + arranged_sc);
				...
				//	total_arranged += current_value;
					...
					temp = remaining_princ - current_value;
...
//alert('interest paid ' + current_value + ' - ' + arranged_interest);
		...
		// Don't let them over or underpay for payment arrangements.  Only act on the last row displayed.
		//if (FormatAmount(current_value) > 0 && summary_balance != 0)  // imprecision leads to format.  
		if (base == 'payment_arrangement') {
			if (num_rows_visible == i + 1) {
				// Adjust for discount
				var discount_amount = get_discount_amount(base, principal_balance);

				// not exact paying?  Not legal.  You can't submit unless the balance is zero, you know.
	...
	if (discount_amount > 0.0)
	{
		...
		UpdateDiscountDate(base);
//		document.getElementById("payment_discount_date").value = dates[last_val].value;
...
function SaveSinglePayment(paytype)
{
...
function SaveDebitPayments()
{
	...
	// GF #12858: If there's no debt consolidation companies, show an error [benb]
...
{	
	...
	for (var i = 0; i < num_rows_visible; i++)
	{
		total_balance += parseFloat(gpo(base+"_interest_amount_"+i).value);
		var date = new Date();
		...
		else
		{
			...
			// Added to ensure payment dates fall within a year of the current date.
...
/*				if (date > now)
				{
					var str = "Manual Payments must be in the past.";
					alert(str);
					return;
				}
*/			}
			else
			{
				...
				if (date < now)
				{
					var str = "All arranged dates must be\n";
				...
				if (date > year)
				{
					var str = "Arranged payment dates can not\n";
		...
		{
			var amount = parseFloat(document.getElementById(base+"_amount_"+i).value);
		...
		if ((base == 'payment_arrangement') || (base == 'manual_payment')) {
			...
			var amount = document.getElementById(base+"_amount_"+i).value;
			if (isNaN(amount) || amount <= 0) {
				var str = "The amount for all intended \npayments";
...
//	if ((total_arranged.toFixed(2) != total_balance) && (base == 'payment_arrangement'))
	if ((total_paid.toFixed(2) != total_balance.toFixed(2)) && (base == 'payment_arrangement'))
	{
		var str = "The exact amount of the balance (" + FormatAmount(total_balance) + " vs " + FormatAmount(total_paid) + ") must be paid\n";
...
{
//	alert(interest_accrued);
	
	//document.getElementById("interest_accrued").innerHTML = FormatAmount(interest_accrued);
	...
	//var debit_comp_base = document.getElementById("debt_consolidation_company").text;
	var debit_date_base = document.getElementById("debt_consolidation_date").value;
	...
	var debit_date = debit_date_base;
...
function ChangeRowDisplay(base)
{
	...
	if (el != null)
	{
		...
		set_payment_mode(base); // visibility of display:none elements doesn't persist, so we need to reset them.
		UpdateDiscountDate(base);
	}
}
...
function get_discount_amount (base, principal_balance)
{
	...
	var discount_display_element = gpo(base+'_discount_displayed');
	...
	discount_display_element.innerHTML = discount_amount.toFixed(2);
...
//		if (el != null) el.onchange = new Function("SetDiscountDisplay('"+base+"');");
		...
		for (var j = 0; j < ctrls.length; j++)
		{
			...
			{
				ctrls[j].onchange = new Function("UpdateDiscountDate('"+base+"');");
			}
		}
...
function CheckAccountEdit()
{
	...
	var as = document.getElementById("account_status").selectedIndex;
...
/*		var val = Date.parse(date);
		var now = new Date();
		if (isNaN(val) || 
		    (val >= (new Date(now.getFullYear(), now.getMonth(), now.getDate()))))
		{
			alert("For the status selected you must enter a past date.");
			return false;
		} */
	}
...
function Deceased_Unverified()
{
	...
	{
		var form = document.forms['deceased_unverified_form'];
		form.submit();
	}
}
...
function Deceased_Verified()
{
	...
	{
		var form = document.forms['deceased_verified_form'];
		form.submit();
	}
}
...
function Recall_2nd_Tier()
{
	...
	{
		var form = document.forms['recall_2nd_tier_form'];
		form.submit();
	}
}
