...
{
   	$bureau = new Bureau_Query($db);
	$rows = $bureau->Get_Newest_All($_REQUEST["app_id"], $_REQUEST["cid"]);
	if( is_array($rows) )
	{
		foreach($rows as $row)
		{
			...
			<td class='data'><pre>" . date( "r", $row->date_created) . "</pre></td></tr>\n";
			echo "</table><br><table border='1'>";
			//echo "<tr><td colspan=\"2\"><br /></td></tr>\n";

			if($row->name_short == 'datax')
			{
				Datax_HTML($row);
			}
			elseif($row->name_short == 'clverify')
			{
				CLV_HTML($row);
			}
			else
