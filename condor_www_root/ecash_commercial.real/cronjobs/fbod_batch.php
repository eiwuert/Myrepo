...
class FBOD_Batch
{
	...
	{
		$this->db = DB_DatabaseConfigPool_1::get(ECash_Models_WritableModel::ALIAS_MASTER);
	...
	private function sendBatch($file)
	{
		...
		$batch_login = eCash_Config::getInstance()->CERTEGY_BATCH_LOGIN;
		$batch_pass  = eCash_Config::getInstance()->CERTEGY_BATCH_PASS;

		try 
		{
			$transport_type   = eCash_Config::getInstance()->CERTEGY_TRANSPORT_TYPE;
			$transport_url    = eCash_Config::getInstance()->CERTEGY_BATCH_URL;
			$transport_server = eCash_Config::getInstance()->CERTEGY_BATCH_SERVER;
			$transport_port   = eCash_Config::getInstance()->CERTEGY_BATCH_SERVER_PORT;

			$transport = ACHTransport::CreateTransport($transport_type, $transport_server, $batch_login, $batch_pass, $transport_port);

			$filename =  eCash_Config::getInstance()->CERTEGY_BATCH_FILE;
	...
	{
		$recipients = (EXECUTION_MODE !== 'LOCAL') ? eCash_Config::getInstance()->ECASH_NOTIFICATION_ERROR_RECIPIENTS : '';
