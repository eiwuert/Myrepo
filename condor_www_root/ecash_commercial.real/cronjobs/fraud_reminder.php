...
class Fraud_Reminder
{
	...
	/* This is mostly garage to create a stub server class so this
	...
	public static function main($argc, $argv)
	{
		...
		$sql = ECash_Config::getMasterDbConnection();
        $query = "
                        SELECT company_id from company where name_short = '". self::RULE_COMPANY_SHORT . "'";
        $q_obj = $sql->query($query);
        $row = $q_obj->fetch(PDO::FETCH_OBJ);
		require_once(MY_CRON_DIR ."../utils/mini-server.class.php");
		$server = new Server(new Applog('fraud_reminder'), $sql, $row->company_id);
	...
	private function Load_Rules()
	{
		...
		$this->from_address = eCash_Config::getInstance()->COMPANY_SUPPORT_EMAIL;
	}
