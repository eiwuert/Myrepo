...
class Server
{
	...
	public function Set_Company($company_name)
	{
		...
		/**
		 * Load the Company config and the Base config
		 * 
		 * The Company config contains company specific information, 
		 * whereas the Base config contains the common values that all
		 * companies within an enterprise may have.
		 * 
		 * The useConfig() method allows stacking the two, one on top of
		 * the other in order of search preference.  E.g., When values are
		 * requested from the class, the Company class is searched first,
		 * and then the Base class.
		 */
		$enterprise_prefix = ECash_Config::getInstance()->ENTERPRISE_PREFIX;
		$config_filename = CUST_DIR . "code/{$enterprise_prefix}/Config/{$this->company}.php";
		
		if(class_exists(CUSTOMER . '_Config_' . EXECUTION_MODE . '_' . strtoupper($this->company)))
		{
			$base_config = CUSTOMER . '_Config_' . EXECUTION_MODE . '_' . strtoupper($this->company);
		}
		else
		{
		...
		if(file_exists($config_filename))
		{
			...
			eCash_Config::useConfig(strtoupper($this->company) . "_CompanyConfig", $base_config);
		}
		else
		{
...
//ach.class.php will probably have to be made CFE Friendly.
...
$selected_companies = getSelectedCompanies($companies, $company_arg);
