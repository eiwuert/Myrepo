...
class ACH_Return_Impact extends ACH_Return 
{
// Impact Return File Format
	protected $return_file_format = array(
						'recipient_id', //ECashID
					...
					if (strpos($response['received'], 'ER=') === false)
					{
						...
						foreach ($ach_report_data as $report_data)
						{
							...
							if (is_numeric($ach_id))
							{
								...
								{
									//removed cashline stuff, you wanna fight about it? [W!]
...
//											MySQLi_1e::Get_Instance()->Rollback();
	...
	public function Parse_Report_Batch ($intercept_return_file)
	{
		
		// Split file into rows
		$return_data_ary = explode(self::$RS, $intercept_return_file);
