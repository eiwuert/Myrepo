...
{	
	...
	{
		$return_val = array();
		$transport_type = eCash_Config::getInstance()->SECOND_TIER_INCOMING_TRANSPORT_TYPE;
		$batch_server   = eCash_Config::getInstance()->SECOND_TIER_INCOMING_SERVER;
		$batch_login    = eCash_Config::getInstance()->SECOND_TIER_INCOMING_LOGIN;
		$batch_pass     = eCash_Config::getInstance()->SECOND_TIER_INCOMING_PASS;
		$transport_port   = eCash_Config::getInstance()->SECOND_TIER_INCOMING_SERVER_PORT;
		...
		for ($i = 0; $i < 5; $i++) { // make multiple request attempts
			try {
				...
				{
					case "updates":	
						$prefix = eCash_Config::getInstance()->SECOND_TIER_INCOMING_URL_PREFIX;
						$suffix = eCash_Config::getInstance()->SECOND_TIER_INCOMING_URL_SUFFIX;
						$url = eCash_Config::getInstance()->SECOND_TIER_INCOMING_URL;
	...
	protected function Insert_Second_Tier_Incoming_Response($type, $filename = '', $extension = '', $contents, $start_date)
	{
		...
		$query = '-- /* SQL LOCATED IN file=' . __FILE__ . ' line=' . __LINE__ . ' method=' . __METHOD__ . " */
			...
			WHERE srd.sreport_type_id = ".ECash::getFactory()->getModel('SreportType')->getTypeId($type)."
