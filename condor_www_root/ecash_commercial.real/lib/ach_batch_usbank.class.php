...
class ACH_Batch_USBank extends ACH_Batch
{
	...
	protected function Build_ACH_File($ach_transaction_ary)
	{
		...
		$output['immediate_origin_name']		= substr(eCash_Config::getInstance()->ACH_ORIGIN_COMPANY_NAME, 0, 23);
		...
		$output['originating_dfi_identification'] = substr(ltrim(eCash_Config::getInstance()->ACH_DEBIT_BANK_ABA), 0, 8);
		...
		foreach ($ach_transaction_ary AS $record)
		{
			...
			$output['trace_number_prefix']			= substr(ltrim(eCash_Config::getInstance()->ACH_DEBIT_BANK_ABA), 0, 8);
			...
			if($record['ach_type'] === 'credit') $total_credit_amount += $output['amount'];
			if($record['ach_type'] === 'debit')  $total_debit_amount  += $output['amount'];
...
//		}
		...
		$this->credit_amount = ($total_credit_amount / 100);
		$this->debit_count   = $total_debit_count;
		$this->debit_amount  = ($total_debit_amount / 100);
	...
	{
		return eCash_Config::getInstance()->ACH_BATCH_URL;
	}
