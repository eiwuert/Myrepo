...
class eCash_Mail
{
	...
	public static function FBOD_CERTEGY_BATCH($recipients=array(),$data=array())
	{
		...
		$recipients = count($recipients)?$recipients:eCash_Config::getInstance()->CERTEGY_NOTIFICATION_LIST;
		//populate tokens
		$mail_data = array();
	...
	/**
	 * This function is used to mimic the mail() command simplicity for direct access to exception
	 * alerts.
	 *
	 ...
	 * @return boolean True on success, False on error.
	 */
	public static function sendExceptionMessage($recipients, $body, $subject = null) //mantis:7727 - $subject
	{
		$argc = func_num_args();
		$argv = func_get_args();
