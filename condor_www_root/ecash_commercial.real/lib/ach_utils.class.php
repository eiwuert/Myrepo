...
class ACH_Utils 
{
	...
	public function Insert_ACH_Row($table, $row_data)
	{
	...
	public function Update_ACH_Row($table, $ach_id, $status, $ach_trace_number=NULL, $ach_return_code=NULL, $ach_report_id=NULL)
	{
	...
	public function ACH_Deem_Successful($business_day)
	{
		...
		$ach_hold_days = eCash_Config::getInstance()->ACH_HOLD_DAYS;
		$ach_hold_expire_threshold_date = $this->paydate_obj->Get_Business_Days_Backward($business_day, $ach_hold_days);
		
		if(!empty(eCash_Config::getInstance()->CHECK_HOLD_DAYS))
		{
			$check_hold_expire_threshold_date = $this->paydate_obj->Get_Business_Days_Backward($business_day, eCash_Config::getInstance()->CHECK_HOLD_DAYS);
		}
		else
		{
		...
		try
		{
			...
			{
				$this->Update_ACH_Row('customer', $row['ach_id'], 'processed');
				Post_Transaction($row['application_id'], $row['transaction_register_id']);
				
				$application_status = Fetch_Application_Status($row['application_id']);
				if ($application_status['level0'] == 'current' && $application_status['level1'] == 'arrangements') {
					if (Application_Flag_Exists($row['application_id'], 'arr_incl_pend' )) {
	...
	public function getInterceptAuthCodes()
	{
			...
			$url = eCash_Config::getInstance()->ACH_AUTH_URL;
