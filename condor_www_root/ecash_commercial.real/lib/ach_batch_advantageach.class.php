...
class ACH_Batch_AdvantageACH extends ACH_Batch
{
	...
	public function __construct(Server $server)
	{
		...
		$this->confirm_data->acronym       = eCash_Config::getInstance()->ACH_BATCH_ACRONYM;
		...
		if (isset(eCash_Config::getInstance()->ACH_BATCH_NOTIFY_LIST))
		{
			$this->confirm_data->notify_list   = explode(',', eCash_Config::getInstance()->ACH_BATCH_NOTIFY_LIST);
	
			$this->confirm_data->notify_from   = (isset(eCash_Config::getInstance()->ACH_BATCH_NOTIFY_FROM) ? eCash_Config::getInstance()->ACH_BATCH_NOTIFY_FROM : "achbatch@sellingsource.com");
		}
		else
		{
	...
	{
		$phone_num = eCash_Config::getInstance()->COMPANY_PHONE_NUMBER;
		...
		$output['immediate_destination_name'] 	= eCash_Config::getInstance()->DEPOSITS_PROCESSOR_NAME;
		$output['immediate_destination']		= eCash_Config::getInstance()->ACH_DEBIT_BANK_ABA;
		...
		$output['originating_dfi_identification'] = substr(eCash_Config::getInstance()->ACH_DEBIT_BANK_ABA, 0, 8);
		...
		foreach ($ach_transaction_ary AS $record)
		{
			...
			$output['trace_number_prefix']			= substr(eCash_Config::getInstance()->ACH_DEBIT_BANK_ABA, 0, 8);
			...
			if(eCash_Config::getInstance()->ACH_USE_APP_ID_FOR_NAME === TRUE)
			{
		...
		if(eCash_Config::getInstance()->USE_ACH_ENTRY_DETAIL) 
		{
			$total_hash += $output['receiving_dfi_identification'];
			$entry_addenda_count++;
			$this->file .= $this->Build_Record(6, $output);
	...
	protected function Send_Batch ()
	{
		...
		try {
			...
			if ((isset(eCash_Config::getInstance()->ACH_BATCH_USE_PGP)) && eCash_Config::getInstance()->ACH_BATCH_USE_PGP == TRUE)
			{
				try
				{
			...
			if (is_array($this->confirm_data->notify_list) && !empty($this->confirm_data->notify_list))
			{
				...
				$mail_data['company_name']      = eCash_Config::getInstance()->COMPANY_NAME;
				$mail_data['company_acro']      = eCash_Config::getInstance()->ACH_BATCH_ACRONYM;
				...
				$mail_data['admin_contact_n']   = eCash_Config::getInstance()->ACH_ADMIN_CONTACT_NAME;
				$mail_data['admin_contact_p']   = eCash_Config::getInstance()->ACH_ADMIN_CONTACT_PHONE;
				$mail_data['admin_contact_e']   = eCash_Config::getInstance()->ACH_ADMIN_CONTACT_EMAIL;

				// Technical Contact
				$mail_data['tech_contact_n']    = eCash_Config::getInstance()->ACH_TECH_CONTACT_NAME;
				$mail_data['tech_contact_p']    = eCash_Config::getInstance()->ACH_TECH_CONTACT_PHONE;
				$mail_data['tech_contact_e']    = eCash_Config::getInstance()->ACH_TECH_CONTACT_EMAIL;
