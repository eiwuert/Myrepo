	...
	public function Get_Module_HTML()
	{
		...
		switch ($data->view) {
			case "new_queue":
					...
					{
						$module_string = "<option value=\"{$module[0]}\"" . ($module[0] == $data->req->section_id ? $selected : '') . ($module[2] ? '' : ' disabled' ) . ">{$module[1]}</option>\n";
					...
					{
						$group_listing .= "<option value=\"{$queue_group->Model->queue_group_id}\"" . ($queue_group->Model->queue_group_id == $data->req->queue_group_id ? $selected : '') . ">{$queue_group->Model->name}</option>\n";
					}
					...
					{
						$company_listing .= "<option value=\"{$company->company_id}\"" . ($company->company_id == $data->req->queue_company_id ? $selected : '') . ">{$company->name} ({$company->name_short})</option>";
					}
					...
					foreach ($ordered_queues as $name_short => $display_name)
					{
						...
						if($name_short == $data->req->queue_select)
						{
							$selected = ' selected';
						} else {
							$selected = '';
					...
					$limit_company_default = $config_model->loadBy(array("queue_id" => 0, 'config_key' => 'recycle_limit'))->config_value;
					$timeout_company_default = ECash::getFactory()->getModel('QueueConfig')->loadBy(array("queue_id" => 0, 'config_key' => 'recycle_time'))->config_value / 60;
					...
					$queue_type_list = '
										<option value="Queues_BasicQueue"' . ($data->req->queue_type == 'Queues_BasicQueue' ? ' selected' : '') . '>BasicQueue: Basic Queue</option>
										<option value="Queues_TimeSensitiveQueue"' . ($data->req->queue_type == 'Queues_TimeSensitiveQueue' ? ' selected' : '') . '>TimeSensitiveQueue: Basic Queue w/ Timezone Awareness</option>
										<option value="Queues_CollectionsQueue"' . ($data->req->queue_type == 'Queues_CollectionsQueue' ? ' selected' : '') . '>CollectionsQueue: Collections Queue</option>
					';

					$queue_timeout_options = '';
					foreach ($GLOBALS["DEFAULT_QUEUE_TIMEOUTS"] as $val => $nam) {
						$queue_timeout_options .= "<option value=\"{$val}\"" . ($val == $data->req->queue_timeout_limit ? ' selected' : '') . ">{$nam}" . ($val == 'COMPANY' ? " ({$timeout_company_default})" : '') . "</option>\n";
					}
					...
					return Display_Utility::Token_Replace($html,
						array(
							...
							'queue_name' => $data->req->queue_name,
							'queue_name_display' => $data->req->queue_name_display,
							'queue_name_short' => $data->req->queue_name_short,
							...
							'queue_function' => ucfirst($data->req->queue_function),
