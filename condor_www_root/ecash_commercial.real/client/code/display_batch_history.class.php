	...
	private function Format_Batch_List()
	{
		...
		foreach ($this->data->batchlist as $item)
		{
			...
			$output .= "  <td class=\"batch_mgmt\" style=\"text-align: center; text-decoration: underline\">" . 
															"<a href=\"{$url}\" onClick=\"return window.confirm('Are you sure you want to resend batch {$item['batch_id']}?');\" onMouseOver=\"tooltip(event, 'Click to re-send batch #{$item['batch_id']}.', 150, -30);\" onMouseOut=\"tooltip(null);\">{$item['batch_id']}</a></td>\n";
