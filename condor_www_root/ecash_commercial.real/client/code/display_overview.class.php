...
class Display_View extends Display_Parent
{
	...
	protected function Overview_HTML()
	{
		...
		$g00 = new Group(array ("position" => "absolute", "top" => "5px", "left" => "10px",
					"width" => "380px", "height" => "248px", "overflow" => "auto",
					));
		$g01 = new Group(array ("position" => "absolute", "top" => "5px", "left" => "401px",
					"width" => "380px", "overflow" => "auto", "height" => "440px",
					));
		$g02 = new Group(array ("position" => "absolute", "top" => "252px", "left" => "10px",
					"width" => "380px", "height" => "240px", "overflow" => "hidden", 
					));

		// Set up the layout 0 layers,  If conversion is for CLK, use the Conversion Notes, etc.
		// Layout 0, Group 0
		if(eCash_Config::getInstance()->CONVERSION_MODE === 'CASHLINE')
		{
		...
		$this->layouts[0]->Add_Groups($g00, $g01, $g02);
...
//		  	$this->data->current_module !== 'funding')
		...
		$this->data->date_hire       = date("m-d-Y", strtotime($this->data->date_hire));
		$this->data->conversion_info_button = '';
		$this->data->phone_cell_link = '';
		$this->data->phone_home_link = '';
		$this->data->phone_work_link = '';

		if ( isset($this->data->conversion_queue_reasons) )
		{
			if(eCash_Config::getInstance()->CONVERSION_MODE === 'CASHLINE')
			{
				$this->data->conversion_queue_reasons = $this->Build_Conversion_Info($this->data->conversion_queue_reasons);
				$this->data->conversion_info_button = '<td><a href="#" onClick="javascript:SetDisplay(0,0,5,\'view\', \'conversion_buttons\');"><nobr>[Conversion Info]</nobr></a></td>';
			}
		}
		...
		if($this->data->idv_trapped_error == "")
		{
		...
		if ((eCash_Config::getInstance()->INVESTOR_GROUP_TAGGING_ENABLED != NULL) && $this->data->is_react == 'yes')
		{
		...
		if(eCash_Config::getInstance()->USE_LOGIN !== FALSE && (! empty($this->data->crypt_password)))
		{
		...
		$this->data->receive_document_list = $this->build_display->Build_Receive_Document_List($this->data->receive_doc_list, $this->data->docs, 'email');
		 ...
		 */
		if(eCash_Config::getInstance()->CONDOR_DOC_URL !== NULL || eCash_Config::getInstance()->DOC_REPRINT_URL !== NULL)
		{
			//mantis:4922
			...
			$this->data->condor_doc_url = eCash_Config::getInstance()->CONDOR_DOC_URL . '&property_short='. ECash::getTransport()->company;
			$this->data->doc_reprint_url = eCash_Config::getInstance()->DOC_REPRINT_URL . '?database=WEB_' . ECash::getTransport()->company;
	...
	protected function Set_Read_Only_Fields()
	{
		...
		{
			$this->data->ssn_edit_field = $ssn_display . '<input type="hidden" id="ssn" name="ssn" value="' . $ssn_display . '" />';
		}
		...
		// If the field is marked read only, allow them to send the customer to rebiew
	...
	protected function Format_Fraud_Fields()
	{
		$this->data->fraud_rules_label = '';
		$this->data->fraud_rules_list = '';
		$this->data->fraud_fields_label = '';
		$this->data->fraud_fields_list = '';
		$this->data->risk_rules_label = '';
		$this->data->risk_rules_list = '';
		$this->data->risk_fields_label = '';
		$this->data->risk_fields_list = '';
		//$this->data->fraud_fields = '';
		
		//echo '<pre>'.print_r($this->data,true).'</pre>';
		if(!empty($this->data->fraud_rules))
		{
			$this->data->fraud_rules_label = "&nbsp;Fraud Rules:&nbsp;";
			$this->data->fraud_rules_list = addslashes(join("<br>", split(";", $this->data->fraud_rules)));
			$this->data->fraud_rules = join(", ", split(";", $this->data->fraud_rules));
		}

		if(!empty($this->data->fraud_fields))
		{
			$this->data->fraud_fields_label = "&nbsp;Fraud Fields:&nbsp;";
			$this->data->fraud_fields = join("<br> ", FraudColumns::formatForDisplay(split(",", $this->data->fraud_fields)));
			$this->data->fraud_fields_list =  $this->data->fraud_fields; //don't run format on these (they've already been formatted)
		}

		if(!empty($this->data->risk_rules))
		{
			$this->data->risk_rules_label = "&nbsp;High Risk Rules:&nbsp;";
			$this->data->risk_rules_list = addslashes(join("<br>", split(";", $this->data->risk_rules)));
		}

		if(!empty($this->data->risk_fields))
		{
			$this->data->risk_fields_label = "&nbsp;High Risk Fields:&nbsp;";
			$this->data->risk_fields = join("<br> ", FraudColumns::formatForDisplay(split(",", $this->data->risk_fields)));
			$this->data->risk_fields_list = $this->data->risk_fields; //don't run format on these (they've already been formatted)
		}
	}
 ...
 <td class=\"align_left{$alt}\">{$b}</td>
	...
	protected function BuildInvestorGroupDropdown()
	{
		...
		{
			if (! empty(eCash_Config::getInstance()->DEFAULT_INVESTOR_GROUP) && eCash_Config::getInstance()->DEFAULT_INVESTOR_GROUP == $group->tag_name)
			{
	...
	protected function Set_Dropdown_HTML($bank_account_type_is_read_only)
	{
		...
		if(($this->data->level1 == 'verification') || ($this->data->level1 == 'underwriting'))
		{
		...
		$this->data->legal_id_state_drop = $l_state_dd->State_Pulldown ("legal_id_state", 0, 0, isset($this->data->saved_error_data->legal_id_state) ? $this->data->saved_error_data->legal_id_state: $this->data->legal_id_state, true, "", 0, false, false, NULL, NULL, NULL,'legal_id_state');
	...
	protected function Are_Docs_Received_And_Status_Pending($status, $docs)
	{
		...
		if ($status == 'pending')
		{
			...
			{
				if ($value->name == 'ecash30_Loan Document')
				{
					$result = TRUE;
					break;
				}
	...
	protected function Set_Button_HTML()
	{
		...
		*/
		...
		// This hack was added to accommodate for the folks in Collections
...
//				break;
	...
	{
		$current_tenure = $this->data->job_tenure;
		$tenures = array(
						0 => '',
						1 => '0 to 6 months',
						2 => '6 to 12 months',
