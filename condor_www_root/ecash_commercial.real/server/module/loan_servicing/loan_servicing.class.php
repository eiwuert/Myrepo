...
class Loan_Servicing extends Base_Module
{
	...
	public function __construct(Server $server, $request, $mode, Module_Interface $module = NULL)
	{
		...
		//$this->non_ach = new Non_ACH_Actions($server);
	...
	public function Return_File_History()
	{
		...
		$columns = array('ach_report_id', 'date_request', 'date_created', 'report_status', 'ach_report_request');
		$report_list = ECash::getFactory()->getModel('AchReport')->loadHistory($columns, $start_date, $end_date);
	...
	{
		$cashline_view = new Cashline_View($server, $request);
		$cashline_view->Get_Customer();
	...
	public function Change_Status()
	{
		...
		switch(strtolower($this->request->submit_button))
		{
			...
			case "withdraw":
				...
				$action_result = $this->ld->Withdraw($this->request->application_id);
				ECash::getTransport()->Set_Levels('close_pop_up');

				// GF 10339: "Send Docs" checkbox was being ignored when an application was withdrawn.
				// Solution: Put the doc sending in a conditional based on whether 'document_list' is in
				// the request.
				if (isset($this->request->document_list))
				{
					require_once (LIB_DIR . "/Document/AutoEmail.class.php");
					eCash_Document_AutoEmail::Send($this->server, $this->request->application_id, 'WITHDRAWN_LETTER');
				}
			...
			case 'not_bankruptcy':
				...
				else
				{
					...
					if ($action_result) Schedule_Full_Pull($this->request->application_id,
	...
	public function Send_Docs()
	{
		if(!empty($this->request->document_list))
		{
			...
			$_SESSION['current_app']->docs = eCash_Document::Get_Application_History($this->server, $_SESSION['current_app']->application_id);
	...
	public function Add_Loan_Action($set_data = TRUE)
	{
		if ($set_data)
		{
			ECash::getTransport()->Set_Data($this->ld->Fetch_Loan_All($this->request->application_id, false));
			...
			if(is_array($this->request->loan_actions))
			{
				for($i=0; $i<count($this->request->loan_actions); $i++)
				{
					...
					Insert_Loan_Action($this->request->application_id,$loan_item,$status_id, $this->server->agent_id); //mantis:6807 - added $agent_id
				}
			}
			else
			{
				$loan_item = $this->request->loan_actions;
				$loan_actions_query->Insert_Loan_Action($this->request->application_id,$loan_item,$status_id, $this->server->agent_id); //mantis:6807 - added $agent_id
			}
		}
	}
	...
	public function Schedule_Payout()
	{
		...
		case 'fetch':
			list($status, $schedule) = $this->ld->Fetch_Schedule_Data($application_id,false,false);
			$data = Get_Transactional_Data($application_id);
			$interest = Interest_Calculator::getInterestPaidPrincipalAndDate($schedule);
	...
	public function RecoveryReversal()
	{
		...
		switch($this->request->action_type)
		{
			...
			case 'save':
				...
				$status 	= Fetch_Application_Status($app_id);

				$this->ld->Save_RecoveryReversal($this->request,$app_id);

				if($status['status'] === 'recovered')
				{
					$apps = Get_Other_Active_Loans(ECash_Config::getMasterDbConnection(), $app_id);
...
*/
	...
	public function Reminder_Remove()
	{
		...
		Insert_Loan_Action($this->request->application_id, $loan_action_id, $app_status['application_status_id'], $this->server->agent_id);
	...
	public function Place_In_Amortization()
	{
		...
		$biz_rules = new ECash_BusinessRulesCache(ECash_Config::getMasterDbConnection());
		...
		Follow_Up::Create_Follow_Up($application_id, 'amortization_start', $interval, $agent_id, $company_id, $comment, false);
