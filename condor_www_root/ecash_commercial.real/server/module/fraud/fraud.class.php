...
class Fraud extends Base_Module
{
	...
	{		
		...
		$this->customer_contact = new Customer_Contact(ECash_Config::getMasterDbConnection());
		if($mode == 'fraud_rules')
		{
...
// 	}
	...
	public function Unconfirm_Fraud($preview = TRUE)
	{
		...
		$checker = new FraudCheck(ECash_Config::getMasterDbConnection(), FraudRule::RULE_TYPE_FRAUD, TRUE, $rule);
	...
	public function Check_Fraud(FraudCheck $checker, ECashApplication $application, $preview = FALSE, $fraud_rule_off = FALSE)
	{
		$results = $checker->processApplication($application);
		$status = NULL;
		$current_status=Fetch_Status($application->application_id);
		//echo "<!-- RULES: ", print_r($results, TRUE), " -->";
		...
		if(empty($results[FraudRule::RULE_TYPE_FRAUD]))
		{
			...
			if($need_to_remove)
			{
				if(!$preview)
				{
					...
					{
						$prev_status = Get_All_Previous_Status($application->application_id);

						if($prev_status)
						{
						//	echo '<pre>' . print_r($prev_status,true) . '</pre><br>blah:' . $prev_status[0]->application_status_id;
						//  exit();
							$statuses = Fetch_Status_Map();
							$temp_status =explode('::',$statuses[$prev_status[0]->application_status_id]['chain']);
							$i=1;
							// while previous status is in fraud
							while($temp_status[1] == 'fraud')
							{
								$temp_status =explode('::',$statuses[$prev_status[$i]->application_status_id]['chain']);
								$i++;
							}
...
//					{
	...
	private function Load_Fraud_Business_Rules()
	{
		if(empty($this->settings))
		{
			...
			$this->settings[self::ECASH_ADDRESS] = eCash_Config::getInstance()->LOAD_BALANCED_DOMAIN;
			$this->settings[self::COMPANY_SUPPORT_EMAIL] = eCash_Config::getInstance()->COMPANY_SUPPORT_EMAIL;
		}
	...
	private function Fraud_Rule_Off(FraudRule $rule, $preview = TRUE)
	{
		...
		$checker = new FraudCheck(ECash_Config::getMasterDbConnection(), NULL, NULL, $rule);
//		echo '<pre>' . print_r($apps,true) .'</pre>';
	...
	{
		//here's a fun hack to make sure apps get moved to the right
		//queues, even if this app has been brought up by this agent
		//-- JRF
		unset($_SESSION['LOCK_LAYER']['App_Info'][$application_id]['date_modified']);
