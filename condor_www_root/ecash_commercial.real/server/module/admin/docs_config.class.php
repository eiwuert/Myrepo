...
//		var_dump($this->request);
	...
	public function RemoveEmailResponse()
	{
		...
		$res = ECash_Config::getMasterDbConnection()->exec($query);
	...
	{
		$client = new Prpc_Client(eCash_Config::getInstance()->PRINT_MANAGER_URL);
	...
	{
		$client = new Prpc_Client(eCash_Config::getInstance()->CONDOR_SERVER);
	...
	public function FetchEmailFooters()
	{
		...
		$result = ECash_Config::getMasterDbConnection()->query($query);
