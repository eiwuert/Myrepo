...
class Queue_Config
{
	...
	public function New_Queue()
	{
		...
		if(isset($this->request->go))
		{
			...
			&& empty($data->completed))
			{
				...
				else 
				{
					...
					$new_queue_displays = new DB_Models_ModelList_1($factory->getModelClass('QueueDisplay'), 
		...
		$db = DB_DatabaseConfigPool_1::getConnection(ECash_Models_DatabaseInstanceHandler::ALIAS_MASTER);
		...
		{
			if($group->getModel()->company_id == ECash::getCompany()->company_id || eCash_Config::getInstance()->MULTI_COMPANY_ENABLED)
	...
	public function Set_Recycle_Limit_Rule($queue_name_short, $count)
	{
		...
		$limit_company_default = ECash::getFactory()->getModel('QueueConfig')->loadBy(array("queue_id" => 0, 'config_key' => 'recycle_limit'))->config_value;
