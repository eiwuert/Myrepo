...
//        var_dump(eCash_Config::getInstance());
//        $sorts = eCash_Config::getInstance()->QUEUE_CONFIG->getSortOrders();
...
//                        $queue_routing = "module=collections";
                ...
                $found[$array_id] = ""
					...
					.		$row["application_id"]
					.		($row["waiting_for_recycle"] > 1 ? '(Waiting For Recycle)' : '')
					.		($row["final_queuing"] > 0 ? '(No More Recycles)' : '')
					.		($row["not_yet_available"] > 0 ? '(Not Yet Available)' : '')
					.		"</a>"
                    ...
                    .           "<li>"
                    .           "Expires: "
                    .           ( NULL === $row["date_unavailable"] ? "Never" : date('g:i:s A D M. jS, Y',$row["date_unavailable"]) )
                    .           "</ul>"
                    .       "</div>"
                    ;
