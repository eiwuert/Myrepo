...
class Return_Item_Summary_Report_Query extends Base_Report_Query
{
	...
	public function Fetch_Return_Item_Summary_Data($date_start, $date_end, $loan_type, $company_id)
	{
		...
		$query = "-- eCash 3.0, File: " . __FILE__ . ", Method: " . __METHOD__ . ", Line: " . __LINE__ . "
			SELECT
				...
				CONCAT(ach.bank_aba, ' / ', ach.bank_account) bank,
				CASE
					WHEN tt.name_short IN ('repayment_principal', 'payment_arranged_princ', 'payment_manual_princ')
					THEN 'principal'

					WHEN tt.name_short IN ('payment_service_chg')
					THEN 'service_charge'

					WHEN tt.name_short IN ('payment_fee_ach_fail', 'payment_arranged_fees', 'payment_manual_fees')
					THEN 'fee'
