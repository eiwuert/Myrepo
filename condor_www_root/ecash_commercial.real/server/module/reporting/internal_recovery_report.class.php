...
class Internal_Recovery_Report_Query extends Base_Report_Query
{
	...
	public function Fetch_Internal_Recovery_Data($date_start, $date_end, $company_id, $loan_type)
	{
		...
		while ($rowset = $st->fetch(PDO::FETCH_ASSOC))
		{
			...
			if(!isset($row[$keydate]))
			{
				$row[$keydate] = array();
				$row[$keydate]['company_name']			= $rowset['company_name'];
				$row[$keydate]['company_id']			= $rowset['company_id'];
				$row[$keydate]['trans_date'] 			= $keydate;
				$row[$keydate]['Total_Arrangments'] 	= 0;
				$row[$keydate]['Total_Amount'] 			= 0;
				$row[$keydate]['Total_App_IDS'] 		= array();
				$row[$keydate]['Failed_Arrangments'] 	= 0;
				$row[$keydate]['Failed_Amount'] 		= 0;
				$row[$keydate]['Failed_App_IDS'] 		= array();
				$row[$keydate]['Completed_Arrangments']	= 0;
				$row[$keydate]['Completed_Amount']		= 0;
				$row[$keydate]['Completed_App_IDS']		= array();
			}
			...
			if(in_array($rowset['tran_status'],array('complete','failed')))
			{
				...
				$row[$keydate]['Total_Arrangments']++;
				$row[$keydate]['Total_Amount'] 		= $row[$keydate]['Total_Amount'] + $rowset['amount'];
				$row[$keydate]['Total_App_IDS'][] 	= $rowset;

				switch($rowset['tran_status'])
				{
					case 'complete':
						$row[$keydate]['Completed_Arrangments']++;
						$row[$keydate]['Completed_Amount'] 		= $row[$keydate]['Completed_Amount'] + $rowset['amount'];
						$row[$keydate]['Completed_App_IDS'][] = $rowset;

						break;
					case 'failed':
						$row[$keydate]['Failed_Arrangments']++;
						$row[$keydate]['Failed_Amount'] 		= $row[$keydate]['Completed_Amount'] + $rowset['amount'];
						$row[$keydate]['Failed_App_IDS'][] 	= $rowset;
						break;
				}
			}
