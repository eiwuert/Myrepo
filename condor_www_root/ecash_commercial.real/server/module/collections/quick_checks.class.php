...
class Quick_Checks
{
	...
	public function __construct( Server $server )
	{
		...
		$this->execution_mode   = eCash_Config::getInstance()->QC_EXECUTION_MODE;

		$this->return_code = eCash_Config::getInstance()->QC_RETURN_CODE;

		$this->deposit_url      = eCash_Config::getInstance()->QC_DEPOSIT_URL;
		$this->deposit_userpass = eCash_Config::getInstance()->QC_DEPOSIT_USER .
							  ':'.eCash_Config::getInstance()->QC_DEPOSIT_PASS;
		$this->deposit_port		= eCash_Config::getInstance()->QC_DEPOSIT_PORT;
		$this->return_url       = eCash_Config::getInstance()->QC_RETURN_URL;
		$this->return_user      = eCash_Config::getInstance()->QC_RETURN_USER;
		$this->return_pass      = eCash_Config::getInstance()->QC_RETURN_PASS;
		$this->status_url       = eCash_Config::getInstance()->QC_STATUS_URL;
		$this->status_user      = eCash_Config::getInstance()->QC_STATUS_USER;
		$this->status_pass      = eCash_Config::getInstance()->QC_STATUS_PASS;		
...
\* *************************************************** */
	...
	{
		$co_phone_number	= eCash_Config::getInstance()->COMPANY_PHONE_NUMBER;
		$company_name 		= eCash_Config::getInstance()->COMPANY_NAME;
		$qc_company 		= eCash_Config::getInstance()->QC_COMPANY;
		$qoc 				= eCash_Config::getInstance()->QC_OWNER_CODE;
...
\* ***************************************** */
	...
	public function Get_Pending_Count()
	{
		...
		// QC Ready Status ID
		$status_id =  app_stat_id_from_chain($this->db, array('ready','quickcheck','collections','customer','*root'));
