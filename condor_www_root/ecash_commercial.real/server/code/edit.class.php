...
class Edit
{
	...
	{
		//die('<pre>'. print_r($this->request, TRUE));
...
//			$this->request = (object) array_merge((array)$this->request, (array)$this->val_obj->Get_Last_Normalized());
		...
		//	$data_obj = (object) array_merge((array)$data_obj, (array)$this->val_obj->Get_Last_Normalized());
			...
			try 
			{
				if($use_session_data !== FALSE || $application->isAltered())
				{
					...
					{
						$application->application_status_id = ECash::getFactory()->getReferenceList('ApplicationStatusFlat')->toId('agree::prospect::*root');
						//send an email
						require_once (LIB_DIR . "/Document/AutoEmail.class.php");
						eCash_Document_AutoEmail::Send($this->server, $this->request->application_id,'LOAN_NOTE_AND_DISCLOSURE');
	
						// We also need to send CSO docs if this is a CSO Loan
						// this autoemail business needs overhauled
						// hack
						if ($return_obj->business_rules['loan_type_model'] == 'CSO')
						{	
							eCash_Document_AutoEmail::Send($this->server, $this->request->application_id,'ONLINE_NOTE');
							eCash_Document_AutoEmail::Send($this->server, $this->request->application_id,'CREDIT_SERVICES_AGREEMENT');
						}
		...
		if( !count($validation_errors) )
		{
			...
			{	
				...
				if($application->isAltered())
				{
					if ($set_to_agree)
					{
						// GF #12201: Since the application's data is clearly altered to get to this point, it might be 
						// a good idea to save it before generating and autosending out docs. [benb]
						$application->save();
	...
	public function Save_Wizard()
	{
		...
		try
		{
			...
			$display->toModel($this->request, $data);
		}
		...
		try {
			...
			$app_status = Fetch_Application_Status($this->request->application_id);
			...
			if($affected_rows > 0 && ($_SESSION['current_app']->level2 == "applicant") ) 
			{
				...
				$gp = $pdc->Get_Calendar_Days_Forward(date('m/d/Y'), $rules['grace_period']);
                ...
                $business_rules = new ECash_BusinessRulesCache(eCash_Config::getMasterDbConnection());
                $rule_set_id = $business_rules->Get_Rule_Set_Id_For_Application($this->request->application_id);
				...
				unset($_SESSION['LOCK_LAYER']['App_Info'][$this->request->application_id]['date_modified']);
				Update_Status(NULL, $this->request->application_id, 'agree::prospect::*root');
			}
		}
		catch(Exception $e) 
		{
	...
	public function Customer_SSN_Change()
	{
		...
		$customer->loadBySSN($company_id, $new_ssn);
		...
		if(!empty($customer->customer_id))
		{
			if($customer->customer_id != $this->request->customer_id)
			{
				...
				$this->log->Write("Found customer id: $new_customer_id while searching with new SSN $new_ssn");

				$new_customer = new Customer($this->company_id, $customer->customer_id, $this->agent_id);
				$old_customer = new Customer($this->company_id, $this->request->customer_id);

				$data = new stdClass();
				$data->old_customer_id = $this->request->customer_id;
				$data->new_customer_id = $customer->customer_id;

				$data->old_applications = $old_customer->Get_Applications();
				$data->new_applications = $new_customer->Get_Applications();
		...
		$old_customer = new Customer($this->company_id, $old_customer_id, $this->agent_id);
		...
		if (count($old_customer->Get_Application_IDS()) == 1)
		{
	...
	public function Customer_SSN_Commit_Change()
	{
		...
		if(isset($this->request->new_customer_id))
		{
			...
			$new_customer = new Customer($this->company_id, $new_customer_id, $this->agent_id);
			$new_customer->Merge_Applications($applications);
		...
		{
			$new_customer = new Customer($this->company_id, $this->request->old_customer_id, $this->agent_id); //mantis:6569 $this->request->old_customer_id instead of null
		...
		if(count($applications) == count($old_customer->Get_Application_IDS()))
		{
	...
	{
		$customer = new Customer($this->company_id, $this->request->customer_id, $this->agent_id);
		...
		$all_applications = $customer->Get_Applications();
	...
	public function React_Verification_Action()
	{
		...
		switch($request->react_type)
		{
			...
			case 'same':
				...
				eCash_Document_AutoEmail::Send($this->server, $application_id, 'LOGIN_AND_PASSWORD');
				break;
			...
			case 'different':
				...
				$new_customer = new Customer($this->company_id, null, $this->agent_id, null, $this->request->application_id); //mantis:6569 - added null, $this->request->application_id
				if($new_customer_id = $new_customer->Create_Customer(array($request->application_id), $new_ssn))
				{
					eCash_Document_AutoEmail::Send($this->server, $application_id, 'NEW_USERNAME_PASSWORD');
      ...
      sample output (expressed in JSON syntax)
{
    ...
    */
	...
	public function Add_Follow_Up($application_id, $comment, $interval, $followup_date = null)
	{
		...
		{
			Follow_Up::Create_Follow_Up($application_id, $follow_up_type, $follow_up_time, $agent_id, ECash::getCompany()->company_id, $comment);
	...
	public function Modify_Transaction()
	{
		...
		case 'complete':
			...
			// Per Mantis: 4749 - If an account is in arrangements and the schedule does not
			// resolve to zero after a transaction is completed, the account should go into
			// the Collections Queue so an agent can make new arrangements.
			$status = Fetch_Application_Status($this->request->application_id);
			if (($status['level1'] == 'arrangements' && $status['level0'] == 'current') ||
			($status['level1'] == 'quickcheck' && $status['level0'] == 'arrangements'))
			{
				$schedule = Fetch_Schedule($this->request->application_id);
				$status = Analyze_Schedule($schedule, true);

				// If the schedule does not resolved to zero, notify an agent or throw the
				// account in a collection queue.
				if($status->running_total != 0 && $status->num_scheduled_events == 0)
				{
					// mantis 6506.  If the unfinished app has an agent affiliation, it should go back to that agent
					...
					if (!empty($agent)) 
					{
						$log->Write("[App:{$this->request->application_id}] In Made Arrangements but has an incomplete schedule, and has a current Agent Affiliation.  Renewing Agent Affiliation.");
						Follow_Up::createCollectionsFollowUp($this->request->application_id, date('Y-m-d H:i:s', time() + 60), $agent->getAgentId(), $this->company_id, "Application has completed an arrangement but the schedule does build to 0. Arrangements may need to be renegotiated.", NULL, 'arr_with_balance');
					}
					else 
					{
						$log->Write("[App:{$this->request->application_id}] In Made Arrangements but has an incomplete schedule.  Sending to Collections General Queue");
						$queue_manager = ECash::getFactory()->getQueueManager();
						$queue_manager->moveToQueue(new ECash_Queues_BasicQueueItem($this->request->application_id), "collections_general");
					}
		...
		case 'fail':
			...
			catch(Exception $e) 
			{
				...
				Set_Standby($this->request->application_id, 'reschedule');
				ECash::getTransport()->Set_Levels('exception');
				return;
			}
	...
	public function Id_Recheck()
	{
		...
		$timer_name = "ID_Verification Application ID " . $_SESSION['current_app']->application_id;
		ECash::getMonitoring()->getTimer()->startTimer($timer_name);
		
		ECash::getLog()->Write("[App_ID: ".$_SESSION['current_app']->application_id."] ID Verification Rechecked.");
		
		$received = $this->loan_data->bureau_query->Bureau_Call($_SESSION['current_app']->application_id, $this->server->company_id, $this->server->company, 'datax', $rule_sets['IDV_CALL'], $_SESSION['current_app']);
		$this->loan_data->bureau_handler->Format_IDV($_SESSION['current_app'], $received);
 ...
 */
				...
				$query = "
					...
					FROM
						...
						(	SELECT
								...
								DATE_FORMAT(tr.date_effective, '%m/%d/%Y') as date_effective,
	...
	public function Get_ACH_Return_Codes()
	{
		...
		$codes = ECash_Config::getMasterDbConnection()->query($query)->fetchAll(PDO::FETCH_OBJ);
		return $codes;
	}
	...
	public function ModifyDocument()
	{
		...
		if(eCash_Document::Check_Archive_IDs($this->server, $this->request->archive_id) != 0)
		{
	...
	public function resendDocument($method='email')
	{
		...
		$response = eCash_Document_DeliveryAPI_Condor::Prpc()->Send($this->request->archive_id, $rcpt, (strtolower($method) == 'fax') ? 'FAX': 'EMAIL');
		ECash::getTransport()->Set_Levels('close_pop_up');
	}
	...
	private function Check_Fraud($application_id)
	{
		...
		$checker = new FraudCheck(ECash_Config::getMasterDbConnection());
