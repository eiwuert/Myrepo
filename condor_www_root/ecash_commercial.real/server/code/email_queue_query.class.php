...
/*        if(file_exists(CUSTOMER_LIB . "application_restrict.func.php"))
        ...
        if(in_array($queue_name, array_keys(eCash_Config::getInstance()->QUEUE_CONFIG->getMultiCompanyQueues())))
		{
			$company_restrict = '';
		}
		else 
  ...
  */      
		...
		if(eCash_Config::getInstance()->MULTI_COMPANY_ENABLED != true)
		{
			$company_restrict = "				AND
					incoming_email_queue.company_id = '{$this->server->company_id}'";
		}
		...
		$query = "
            ...
            LEFT OUTER JOIN (SELECT application_id FROM application app  ";
		...
		$query .= ") app USING (application_id)
            WHERE
	...
	{
		$queue_name = $this->Get_Queue_Name();
		//$this->timeout = queue_get_config_timeout($queue_name);
		$this->timeout = 30;
/*
		if(file_exists(CUSTOMER_LIB . "application_restrict.func.php"))
        {
	...
	protected function Insert_Record($archive_id, $company_id, $queue_name, $is_failed, $application_id='0')
	{
		...
		if ( empty($queue_name) )
			$queue_name = eCash_Config::getInstance()->DEFAULT_INCOMING_EMAIL_QUEUE;
