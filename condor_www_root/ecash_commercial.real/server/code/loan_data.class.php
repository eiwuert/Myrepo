...
require_once(SERVER_CODE_DIR . "bureau_handler.class.php");
...
require_once(SQL_LIB_DIR . "conversion.func.php");
...
class Loan_Data
{
	...
	/**
	 * Beureau Handler Object
	 *
	 * @var Bureau_Handler
	 */
	public $bureau_handler;
	...
	public function __construct(Server $server)
	{
		...
		$this->bureau_handler = new Bureau_Handler(ECash::getACL(), $this->company_id);
		$this->bureau_query = new Bureau_Query($this->db);
	...
	public function Fetch_Loan_All($application_id, $set_td_lock = TRUE)
	{
		...
		// If we're unable to Fetch the Application, reset the screen to
		// the Search page and return false.

		//get model
		$trans_obj = new stdClass();
		//$trans_obj = Fetch_Application_Info($application_id, $set_td_lock, $this->server);
		//yes this will probably fetch twice
		// Force a reload here [#16320]
		...
		// Fetch the campaign info.  Currently used by Bureau Query's calls to DataX
		// and web_url is displayed in the Interface.
		$campaign = ECash::getFactory()->getData('Application')->getCampaignInfo($application_id);
		$trans_obj->web_url  = $campaign->url;
		$trans_obj->promo_id = $campaign->promo_id;
		$trans_obj->origin_url = $campaign->origin_url;
		...
		/** @todo: We pull the flag list, but where does it get used?  **/
		$application_flag_list = ECash::getFactory()->getModel('ApplicationFlagList')->loadByApplicationID($application_id, $trans_obj);

		$trans_obj->business_rules = $this->Load_Business_Rules($application->getModel()->rule_set_id);
		...
		//$trans_obj->BusinessRulesList = ECash::getFactory()->getModel('BusinessRulesList')->loadBy(array('rule_set_id' => $trans_obj->Application->rule_set_id));
 	...
 	//	$trans_obj->app_flags = $flags->Get_Application_Flags();
		...
		$trans_obj->company_id = $application->getCompanyId();
		$trans_obj->ssn = $application->Model->ssn;
 		...
 		if(is_null($this->customer))
		{
			...
			{
 				$this->customer = new Customer($trans_obj->company_id, $trans_obj->customer_id, $this->server->agent_id);
 			}
 			else
 			{
 				$this->customer = new Customer($this->server->company_id, NULL, $this->server->agent_id, $trans_obj->ssn, $application_id);
 			}
 		}

		$trans_obj->application_list = $this->customer->Get_Applications();
		...
		if ($trans_obj->date_hire && $trans_obj->date_hire != 'n/a')
		{
			$trans_obj->residence_duration = $this->getDurationTag($trans_obj->date_hire);
		}
		else
		{
			$trans_obj->date_hire = "";
			switch ($trans_obj->job_tenure)
			{
				case 1:
					$trans_obj->employment_duration="0 to 6 Months";
				break;
				case 2:
					$trans_obj->employment_duration="6 to 12 Months";
				break;
				case 3:
					$trans_obj->employment_duration="12+ Months";
				break;
				case 4:
					$trans_obj->employment_duration="Not Presently Employed";
				break;
				case 5:
					$trans_obj->employment_duration="Retired";
				break;
				default:
					$trans_obj->employment_duration = "n/a";
				break;
			}
		...
		if((is_array($trans_obj->references)) && (count($trans_obj->references) > 0))
		{
			...
			for ($i = 0; $i < 2; $i++)
			{
				...
				$trans_obj->$name = $trans_obj->references[$i]->full_name;
				$trans_obj->$phone = $trans_obj->references[$i]->phone;
		...
		$ecash_api = eCash_API_2::Get_eCash_API(ECash::getCompany()->name_short, $this->db, $application_id);

		if(method_exists($ecash_api, 'getDelinquencyDate'))
		{
			$trans_obj->delinquency_date = $ecash_api->getDelinquencyDate($application_id);
		}

		// Get the paydates array from the current application in the application list

//		$calc = $application->getPayDateCalculator();
//		$calc->rewind();
		/**
...
/*		try
		{
			for($i = 0; $i < 4; $i++)
			{
				$paydate = $calc->current();
				$paydates['paydates'][] = date('Y-m-d',$paydate);
				$trans_obj->{'paydate_' . $i} = date('Y-m-d', $paydate);
				$trans_obj->{'payday_' .$i} = date('D', $paydate);
				$calc->next();
			}
		}
		catch (Exception $e)
	...
	*/
		...
		$paydates = $trans_obj->application_list[$application_id]->paydates;
		...
		$trans_obj->due_date_offset = $trans_obj->business_rules['grace_period'];
		...
		$trans_obj->docs = eCash_Document::Get_Application_History($this->server, $application_id);

		//Add IDV stuff
		$received = $this->bureau_query->Get_Data($application_id, $trans_obj->company_id, true);
		/*
		 * Commented Out GF #22127

		if($received == NULL)
		{
			// If there wasn't any previously stored bureau inquiry result, we only do the external call (DataX)
			//	when the current application status belongs to prospect or applicant branch...
			// But we do not want to check these for Reacts
			if ((	in_array($trans_obj->level1, self::$status_to_call_datax) && $trans_obj->level2 == '*root'	||
					in_array($trans_obj->level2, self::$status_to_call_datax) && $trans_obj->level3 == '*root'	||
					in_array($trans_obj->level3, self::$status_to_call_datax) && $trans_obj->level4 == '*root'	||
					in_array($trans_obj->level4, self::$status_to_call_datax) && $trans_obj->level5 == '*root'
					) && $trans_obj->is_react == "no")

			{
				$timer_name = "ID_Verification Application ID " . $application_id;
				$this->server->timer->Timer_Start($timer_name);

				try
				{
					// Try to use a call type defined in the database first
					if(isset($trans_obj->business_rules['IDV_CALL']))
					{
						$call_type = $trans_obj->business_rules['IDV_CALL'];
					}
					else
					{	// Fall back to a generic idv call
						$call_type = 'idv';
					}

					$received = $this->bureau_query->Bureau_Call(   $application_id,
				...
				{
					$_SESSION['error_message'] .= "There were errors processing this DataX Call!  Reason: " .$e->getMessage() . "\n";
				}


				$this->server->timer->Timer_Stop($timer_name);

				//mantis:6106
				if(empty($received))
				{
					if($loan_action_id = Get_Loan_Action_ID('no_datax_call'))
					{
						$app_history_array = Get_Application_History($application_id);
						$app_history = $app_history_array[count($app_history_array)-1];
						$status_id = Get_Status_ID($app_history->status_history_id);

						Insert_Loan_Action($application_id, $loan_action_id, $status_id, ECash::getAgent()->getAgentId());
					}
				}
			}
			else
			{
				$received = array('external_call_suppressed' => 'loan decision already made on this application');
			}
		}
		*/
		try
		{
			$this->bureau_handler->Format_IDV_XML($trans_obj, @key($received), @current($received));
		}
		catch (Exception $e)
		{
			// Fixes mantis issue #4952
			$this->log->Write("Falling back to MiniXML on app #" . $trans_obj->application_id);
			$minidoc = new MiniXMLDoc();
			$xml = str_replace("-", "_", current($received));
			$minidoc->fromString($xml);
			$this->bureau_handler->Format_IDV($trans_obj, array(key($received), $minidoc->toArray()));
		}
...
/*
		...
		$trans_obj->application_list = $customer->getApplications();
		$trans_obj->dnl_info = ECash::getFactory()->getData('Customer')->getDoNotLoanSummary($trans_obj->ssn);
//		$trans_obj->is_dnl_set = $dnl->get();
//		$trans_obj->is_dnl_set_for_company = $dnl->getByCompany(ECash::getCompany()->company_id);
//		$trans_obj->is_dnl_set_for_other = $dnl->getByCompanyExclusion(ECash::getCompany()->company_id);
//		$trans_obj->is_override_dnl_set = $dnl->getOverride($this->company->company_id);
...
*/
	...
	//	$trans_obj->notifications = $this->application_field_attributes->Get_Attributes($application_id);
		...
		$trans_obj->esig_doc_list = eCash_Document::Get_Document_List($this->server,"esig");
		$trans_obj->packaged_doc_list = eCash_Document::Get_Document_List($this->server,"packaged");
		$trans_obj->send_doc_list = eCash_Document::Get_Document_List($this->server,"send");
		$trans_obj->receive_doc_list = eCash_Document::Get_Document_List($this->server,"receive");
		...
		if(isset($trans_obj->business_rules['service_charge']['svc_charge_type']) && $trans_obj->business_rules['service_charge']['svc_charge_type'] === 'Daily')
		{
			...
			// LOOK AT ME, I'm an interest calculator!
			...
			$daily_interest_amount = ($trans_obj->business_rules['service_charge']['svc_charge_percentage'] / 7) / 100 * $paid_to['principal'];
			if ($trans_obj->business_rules['loan_type_model'] == 'CSO')
			{
			...
			$trans_obj->interest_paid_to_date = preg_replace('%^(\d{4})-(\d{2})-(\d{2})$%', '$2/$3/$1', $this->pdc->Get_Last_Business_Day($paid_to['date']));
		}
		...
		{
			// If this is a collections follow_up and the agent_id is the same, or it's a funding
			// follow_up we'll update the follow_up to show it's been viewed.
			switch ($follow_up->type)
			{
				case 'collections':
				case 'servicing':
					if ($follow_up->agent_id == ECash::getAgent()->getAgentId())
					{
		...
		if ( $trans_obj->archive_cashline_id )
		{
			$conversion_info = Get_Conversion_Info_Data($application_id);

			$trans_obj->converted_transaction_count = $conversion_info->converted_transaction_count;
			$trans_obj->converted_notes_count = $conversion_info->converted_notes_count;
			$trans_obj->conversion_queue_reasons	 = $conversion_info->queue_reasons;
			$trans_obj->conversion_status = $conversion_info->conversion_status;
			$trans_obj->cashline_status = $conversion_info->cashline_status;
		}
		else
		{
		...
		/* sample input (expressed in JSON syntax)
{
   ...
   application_id:4,
		...
		*/
		...
		if ($structure->payment_type == 'manual_payment')
		{
			...
			else if ( Event_Type_Is_Scheduled($schedule, 'full_balance') )
			{
				...
				{
					if ('full_balance' == $scheduled_event->event_name_short
			...
			if(Check_Inactive($application_id))
			{
				...
				eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'ZERO_BALANCE_LETTER');
				return false;
			}
		...
		if (($structure->payment_type) == 'payment_arrangement')
		{
			...
			require_once (LIB_DIR . "/Document/AutoEmail.class.php");
			eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'ARRANGEMENTS_MADE');
	...
	protected function Handle_Actions_After_Partial($application_id, $date, $agent_id, $company_id)
	{
		...
		$loan_type_id = Fetch_Application_Info($application_id);
		$rule_set_id = $this->business_rules->Get_Current_Rule_Set_Id($loan_type_id->loan_type_id);
		$rules = $this->business_rules->Get_Rule_Set_Tree($rule_set_id);
	...
	{
		if(Check_For_Application_Modifications($this->db, $this->log, $application_id))
		{
	...
	public function Schedule_Payout($request)
	{
		...
		else
		{
			if (Set_Payout($application_id,$date) && isset($_SESSION['api_payment']))
			{
				...
				eCash_Document_AutoEmail::Send($this->server, $application_id, 'ONLINE_REQUEST_CONFIRMATION');
			}
	...
	public function Cancel_Loan($application_id)
	{
		...
		else
		{
			...
			if($principal_amount > 0)
			{
				...
				$new_schedule[] = Schedule_Event::MakeEvent($today, $next_day, $amounts, 'cancel', 'Cancel Request, Deduct principal','scheduled','cancel');
			}
	...
	public function Fund($application_id, $method)
	{
		if(Check_For_Transaction_Modifications($this->db, $this->log, $application_id))
		{
		...
		else
		{
			...
			if ($status)
			{
				...
				// Used to include any pre-existing fees for Title Loans
			 ...
			 $app_data = Fetch_Application_Info($application_id, FALSE);

			 switch ($app_data->loan_type)
			 {
 			...
 			try
			{
			 	eCash_Document_AutoEmail::Send($this->server,$application_id, $document_name);
 			}
 			catch (Exception $e)
 			{
	...
	public function toPreFund($application_id)
	{
		if(Check_For_Application_Modifications($this->db, $this->log, $application_id))
		{
			$_SESSION['error_message'] = "Unable to approve this applicant, this application has been modified.";
		}
		else
		{
			$this->log->Write("[Agent:{$_SESSION['agent_id']}][AppID:{$application_id}] Moving Application to Pre-Fund Status");
			return Update_Status($this->server, $application_id, 'approved::servicing::customer::*root');
		}
	}
	...
	{
		if($prev_app_status_id = Return_To_Previous_Status($application_id))
		{
			$queue_manager = ECash::getFactory()->getQueueManager();
			$queue_manager->getQueueGroup('automated')->remove(new ECash_Queues_BasicQueueItem($application_id));
	...
	public function Update_Schedule($application_id, $start_date=null, $made_arrangement)
	{
		...
		else
		{
			...
			Update_Schedule($application_id, $schedule, $made_arrangement);
		}
	}
	...
	public function Hold_Status ($hold_type)
	{
		...
		switch($hold_type)
		{
			...
			case 'previous_status':
				if($previous_status = Get_Previous_Status($application_id))
					Update_Status($this->server, $application_id, intval($previous_status));
				break;
		}
	}
	...
	*/
	...
	public function Add_Fee($application_id, $type)
	{
		...
		get_log()->Write("[Agent:{$_SESSION['agent_id']}] Adding Fee of type: $type to $application_id");
		...
		switch ($type)
		{
			case 'assess_fee_lien':
				$fee_amount = Vehicle_Data::fetchLienFee($application_id);
				break;
		...
		if($fee_amount > 0.0)
		{
			...
			$event   = Schedule_Event::MakeEvent($date, $date, $amounts, $type,
					     "Agent " . ECash::getAgent()->AgentId . " initiated Fee",'scheduled','manual');
		...
		{
			get_log()->Write(__FUNCTION__ . " No fee amount!");
		}
