<?php
	 ...
	 * validation.
	 * 
	 * @TODO surely I added some things that didn't need to be here during a merge [JustinF]
	 */

	/*
	 *  This was ripped from acl.3 (stupid)
	 */
	...
	class Server_Web extends Server
	{
		...
		public function __construct($session_id)
	  {
	    ...
	    if (eCash_Config::getInstance()->COOKIE_DOMAIN != NULL)
	    {
	      session_set_cookie_params(0, '/', ECash_Config::getInstance()->COOKIE_DOMAIN);
	    }
	    ...
	    if ($this->php_memory_usage > PHP_MEMORY_USE_THRESHOLD)
	    {
			...
			// It's arguable to construct the ACL here, but I don't see how it could be a security problem. -- MarcC
		//self::Set_ACL(new ECash_ACL());
    ...
    public function Process_Data()
    {
        ...
        catch( Exception $e)
        {
		    ...
		    if(strtoupper(EXECUTION_MODE) === 'LOCAL' || strtoupper(EXECUTION_MODE) === 'RC')
            {
		    ...
		    $recipients = (strtoupper(EXECUTION_MODE) !== 'LOCAL') ? eCash_Config::getInstance()->ECASH_NOTIFICATION_ERROR_RECIPIENTS : '';
    ...
    protected function Security_Check ()
    {
    	...
    	else
    	{
    		if( isset(ECash::getRequest()->page) && ECash::getRequest()->page == 'login' && isset(ECash::getRequest()->abbrev) )
    		{
    			...
    			$sys_name = ECash_Config::getInstance()->SYSTEM_NAME;
    			...
    			{
    				$this->setAgent($security->getAgent());
	...
	private function setCompanyID($company_id)
	{
		...
		//		if(isset($this->company_list[$company_id]))
		//		{
		//			$company->name = $this->company_list[$company_id]['name'];
		//			$company->name_short = $this->company_list[$company_id]['name_short'];
		//		}
    ...
    {
    	if(eCash_Config::getInstance()->DEFAULT_MODULE)
    	{
    		$default_modules = explode("::",eCash_Config::getInstance()->DEFAULT_MODULE);
    ...
    protected function Setup_PBX ()
    {
    	...
    	*/
    	...
    	ECash::getTransport()->pbx_enabled = (ECash_Config::getInstance()->PBX_ENABLED && $this->company_list[$this->company_id]['pbx_enabled'] === true && $this->agent_phone != NULL) ? ECash_Config::getInstance()->PBX_ENABLED : false;
    ...
    public function Load_Company_Config($company_id)
    {
    	...
    	try
    	{
    		...
    		ECash_Config::useConfig($company . '_CompanyConfig',$base_config);
    	}
    	catch(Exception $e)
    	{
    	...
    	ECash::getTransport()->new_app_url = ECash_Config::getInstance()->NEW_APP_SITE;
    }
	...
	public function __isset($name)
	{
		...
		{
			case 'agent_id':
				return (ECash::getAgent() !== NULL && ECash::getAgent()->getAgentId() !== eCash_Config::getInstance()->DEFAULT_AGENT_ID);
				break;
