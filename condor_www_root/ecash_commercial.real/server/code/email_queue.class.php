...
class Incoming_Email_Queue extends Email_Queue_Query
{
	...
	public function Get_Email_Response_Data()
	{
		...
		$data = eCash_Document::Get_Application_Data($this->server, $this->request->application_id);
		$data->tokens = eCash_Document_DeliveryAPI_Condor::Map_Data($this->server, $data);
	...
	public function Send_Email_Response()
	{
		...
		$document_name = array( eCash_Config::getInstance()->EMAIL_RESPONSE_DOCUMENT );
		$request_obj = new stdclass();
		foreach($this->request as $key => $value)
		{
			$request_obj->$key = $value;
		...
		$doc = eCash_Document::Singleton($this->server, $request_obj)->Get_Documents_By_Name($document_name);
		...
		try
		{
			...
			$request_obj->GenericSubject = $request_obj->response_subject;
			$request_obj->GenericMessage = str_replace("\n", "<br />", stripslashes($request_obj->response_message) );
			$request_obj->SenderName = $this->Get_Sender_Name_By_Queue($request_obj->email_queue_record->queue_name);
			ECash::getTransport()->Set_Data($request_obj);
	...
	public function File_Email_Document($archive_id)
	{
		...
		$document_name = eCash_Config::getInstance()->EMAIL_RECEIVE_DOCUMENT;
	...
	protected function Fetch_Next_Record()
	{
		...
		if($record && $record->company_id != $this->server->company_id)
		{
	...
	public function Add_To_Email_Queue($company_id, $archive_id, $is_failed, $rec_email_address, $orig_email_address)
	{
		...
		$queue_name = $this->Get_Queue_By_Email_Address($company_id, $rec_email_address);
