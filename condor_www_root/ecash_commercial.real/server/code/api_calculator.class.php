...
//						$data = $calc->get_Holiday_List($input->dateformat);
//						break;
					case 'isBusinessDay':
						$data = $calc->isBusinessDay($input->timestamp);
						break;
//					case 'dateDiff':
//						$data = $calc->dateDiff($input->date_a, $input->date_b);
//						break;
					default:
			...
			case 'interest':
				...
				$ecash_api = eCash_API_2::Get_eCash_API($this->server->company, eCash_Config::getMasterDbConnection(), $application_id);
				$delinquency_date = $ecash_api->getDelinquencyDate($application_id);
				...
				switch ($input->function)
				{
    				case 'calculateDailyInterest':
						...
						if($input->use_next_business_day_payment == 1)
						{
							$last_date = $calc->Get_Next_Business_Day($last_date);
						}

						if($input->use_next_business_day_first == 1)
						{
							$first_date = $calc->Get_Next_Business_Day($first_date);
						}
    					
						$data = Interest_Calculator::calculateDailyInterest($rules, $input->amount, $first_date, $last_date, NULL, $delinquency_date);
    				...
    				case 'getInterestPaidPrincipalAndDate':
						$data = Interest_Calculator::getInterestPaidPrincipalAndDate($input->schedule, $input->include_scheduled);
						break;
  					case 'scheduleCalculateInterest':
						...
						break;
					case 'dateDiff':
						$date_a = date('Y/m/d', strtotime($input->date_a));
						$date_b = date('Y/m/d', strtotime($input->date_b));
						$data = Interest_Calculator::dateDiff($date_a, $date_b);
						break;
					default:
						throw new Exception("Unknown paydate function {$input->function}");
				}
