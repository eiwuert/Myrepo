...
<?php
	 ...
	 * eCash Database
	 */
	$db_host = ECash_Config::getInstance()->DB_HOST;
	$db_user = ECash_Config::getInstance()->DB_USER;
	$db_pass = ECash_Config::getInstance()->DB_PASS;
	$db_name = ECash_Config::getInstance()->DB_NAME;
	$db_port = ECash_Config::getInstance()->DB_PORT;
	$ecash_db = new MySQLi_1($db_host, $db_user, $db_pass, $db_name, $db_port);
	
	/**
	 * Site Management Database
	 */
	$mgmt_db_host = ECash_Config::getInstance()->STAT_MYSQL_HOST;
	$mgmt_db_user = ECash_Config::getInstance()->STAT_MYSQL_USER;
	$mgmt_db_pass = ECash_Config::getInstance()->STAT_MYSQL_PASS;
	...
	class StatAuditReport
	{
		...
		private function getStatProCredentials()
		{
			...
			$site_name = ECash_Config::getInstance()->COMPANY_DOMAIN;
