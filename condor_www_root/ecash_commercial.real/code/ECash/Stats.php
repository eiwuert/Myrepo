...
class ECash_Stats
{
	...
	private function getSetStat(ECash_Application $application)
	{
		...
		$campaign = ECash::getFactory()->getData('Campaign')->getConfigInfoRow($application->application_id);
		...
		if ($campaign == null) {
			get_log()->Write(__METHOD__ . "(): Unable to locate campaign info for {$application->application_id}. No stat call can be made.");
