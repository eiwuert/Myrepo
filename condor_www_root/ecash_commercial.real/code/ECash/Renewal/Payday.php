...
Class ECash_Renewal_Payday extends ECash_Renewal_Renewal implements ECash_Renewal_RenewalInterface 
{
	...
	public function requestRollover($application_id, $paydown_amount = 0)
	{
		...
		{
			//Email the customer
			eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'RENEWAL_REQUEST');
	...
	public function createRollover($application_id, $paydown_amount = 0)
	{
		...
		if($eligibility['eligible'] == FALSE)
		{
			...
			eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, $document);
			//eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'HAHA_YOU_SUCK');
			return $results;
		}
		...
		eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'RENEWAL_SUCCESS');
		return $results;
