...
Class ECash_Renewal_CSO extends ECash_Renewal_Renewal implements ECash_Renewal_RenewalInterface
{
	...
	public function getRolloverEligibility($application_id)
	{
		...
		//Verify that there isn't a payment due today
		
		if($status->next_action_date == date('m-d-Y'))
		{
			$eligibility['eligible'] = FALSE;
			$eligibility['reason'] = 'Payment is due today';
		}
	 ...
	 */
	public function getCSOFeeDescription($fee_name,$application_id,$company_id = null)
	{
		$business_rules = new ECash_BusinessRulesCache(ECash_Config::getMasterDbConnection());
	...
	public function getCSOFeeAmount($fee_name,$application_id,$start_date = null, $end_date = null, $payment_amount = null, $principal_override = null)
	{
		$application =  ECash::getApplicationByID($application_id);
		$business_rules = new ECash_BusinessRulesCache(ECash_Config::getMasterDbConnection());
		$rule_set_id = $business_rules->Get_Rule_Set_Id_For_Application($application_id);
		...
		$principal_balance = $principal_override ? $principal_override : $principal_balance;
		$rule = $rules[$fee_name];
		$amount_type     = $rule['amount_type'];
		$fixed_amount    = $rule['fixed_amount'];
		$percentage_type = $rule['percent_type'];
		...
		if($percentage_type === 'apr')
		{
			...
			$term = Interest_Calculator::dateDiff($start_date, $end_date);
