...
class IT_Settlement implements IT_Settlement_Interface 
{
	...
	public function __construct(Server $server)
	{
		...
		$this->transport_type     = eCash_Config::getInstance()->IT_SETTLEMENT_TRANSPORT_TYPE;
		$this->settlement_server  = eCash_Config::getInstance()->IT_SETTLEMENT_SERVER;
		$this->settlement_port    = eCash_Config::getInstance()->IT_SETTLEMENT_SERVER_PORT;
		$this->settlement_url     = eCash_Config::getInstance()->IT_SETTLEMENT_URL;
		$this->settlement_login   = eCash_Config::getInstance()->IT_SETTLEMENT_LOGIN;
		$this->settlement_pass    = eCash_Config::getInstance()->IT_SETTLEMENT_PASS;
		
		$this->summary_file       = eCash_Config::getInstance()->IT_SETTLEMENT_SUMMARY_FILE;
		$this->summary_extension  = eCash_Config::getInstance()->IT_SETTLEMENT_SUMMARY_EXTENSION;
		$this->details_file       = eCash_Config::getInstance()->IT_SETTLEMENT_DETAILS_FILE;
		$this->details_extension  =	eCash_Config::getInstance()->IT_SETTLEMENT_DETAILS_EXTENSION;
		$this->customer_file      =	eCash_Config::getInstance()->IT_SETTLEMENT_CUSTOMER_FILE;
		$this->customer_extension =	eCash_Config::getInstance()->IT_SETTLEMENT_CUSTOMER_EXTENSION;
...
//		
		...
		try 
		{
			foreach ($reports as $report)
			{
				...
				{			
					...
					require_once(LIB_DIR . "achtransport_sftp_agean.class.php");
	...
	protected function insertFile($report_id,$file,$type,$filename,$extension)
	{
		...
		$report_data->sreport_type_id = ECash::getFactory()->getModel('SreportType')->getTypeId($type);
	...
	public function fetchReportsList($hide_obsolete = false, $start_date = "'1982-11-09'", $end_date = 'now', $search_field = 'sreport_date')
	{
		...
		$query = '-- /* SQL LOCATED IN file=' . __FILE__ . ' line=' . __LINE__ . ' method=' . __METHOD__ . " */
				...
				JOIN
					sreport_type ON sreport_data.sreport_type_id = sreport_type.sreport_type_id
				WHERE
					sreport_type.name_short IN ('it_settlement_summary', 'it_settlement_details', 'loan_customer_table')
				AND
