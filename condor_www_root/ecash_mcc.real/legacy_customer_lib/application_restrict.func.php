...
function getSearchControlRestrictions()
{
	...
	if (method_exists(ECash::getACL(),'Get_Control_Info'))
	{
		$control_options = ECash::getACL()->Get_Control_Info($agent_id, $company_id);
	
		if(!empty($control_options) && eCash_Config::getInstance()->HAS_LOANTYPE_RESTRICTION)
		{
			foreach(ECash::getACL()->getAllowedCompanyIDs() as $company_id) 
			{
				if ($company_id != NULL)  {
					$control_options = ECash::getACL()->Get_Control_Info($agent_id, $company_id);
		...
		{
			if(ECash::getACL()->getAllowedCompanyIDs())
			{
				$control_wheres['company_id'] = "lt.company_id IN ('". implode("','", ECash::getACL()->getAllowedCompanyIDs()) . "')";
			}
		}
