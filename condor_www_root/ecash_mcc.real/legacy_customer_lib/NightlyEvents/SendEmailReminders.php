		...
		private function Send_Email_Reminders(Server $server, $run_date)
		{
			...
			SELECT
				...
				FROM
		            ...
		            application as app
		        WHERE
		        	et.event_type_id = es.event_type_id
		       	AND
		        	app.application_id = es.application_id
				AND
					es.event_status = 'scheduled'
				AND 
					et.name_short IN ('payment_service_chg', 'repayment_principal')
				AND
			...
			while($row = $result->fetch(PDO::FETCH_OBJ))
			{
				...
				eCash_Document_AutoEmail::Queue_For_Send($server, $row->application_id, 'PAYMENT_REMINDER');
			}
		}
	}
