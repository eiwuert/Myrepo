	...
	class ECash_NightlyEvent_CSOAssessLateFee extends ECash_Nightly_Event
	{
		...
		private function assessLateFees(Server $server, $run_date)
		{
			...
			while($row = $result->fetch(PDO::FETCH_OBJ))
			{
				...
				$failure = ECash_CSO::getDefaultingFailure($application_id);
				...
				if (strtotime($assessment_date) <= strtotime($run_date)) 
				{
					...
					//if so go to town!
					$fee = ECash_CSO::getCSOFeeAmount('cso_assess_fee_late',$application_id,null,null,$failure['default_amount']);
					$amounts = array();
