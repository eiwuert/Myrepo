...
class ReschedulingDFA extends DFA
{
	const NUM_STATES = 41;
	...
	function __construct($server)
	{
		...
		$this->final_states = array(3,4,7,8,9,10,11,23,24,25,26,27,29,31,32,36,37,39,45,47);
		...
		$this->transitions = array ( 
			 ...
			 14 => array( 0 => 10, 1 => 11),
			 15 => array( 0 => 43, 1 => 41), // decide: Was there a fatal ach in our history?
			 43 => array( 0 => 16, 1 => 46), 
			 44 => array( 0 => 24, 1 => 45),
			 ...
			 22 => array( 0 => 14, 1 => 29),
			 46 => array( 0 => 44, 1 => 47),
			 40 => array( 0 => 43, 1 => 42), // decide: was there a fatal on our current account?
	...
	function return_grace_period($parameters)
	{
		//$parameters->rules['lender_default'] = 5;
		if($parameters->rules['lender_default'] > 0)
		{
			// Check to see if this return is from an attempt to 'true up'
			foreach($parameters->status->fail_set as $f)
			{
				//if it is return 0
				if($f->origin_id != null)
				{
					$this->Log("Has a failed attempt to 'true up'.  Following Default process");
					return 0;
				}
	...
	function State_45($parameters)
	{
		$this->Log("Loan is now past due");
		//////////////////////////////////////
		...
		try 
		{
			...
			{
				//Get Fees
				$bank_fee = ECash_CSO::getCSOFeeAmount('lend_assess_fee_ach',$parameters->application_id,null,null,$failure_amount);
				...
				//Customer has to call in and "true up" to set the payment for this transaction
				
						
			}
	
			// Add all the reattempts
			//Ha Ha, we don't do reattempts for CSO!
//			foreach($parameters->status->fail_set as $f) 
//			{
//				$ogid = -$f->transaction_register_id;
//				Reattempt_Event($parameters->application_id, $f, $date_event, $ogid);
//			}
			$db->commit();
		} 
		catch (Exception $e) 
	...
	//	eCash_Document_AutoEmail::Queue_For_Send($this->server, $parameters->application_id, 'RETURN_LETTER_2_SECOND_ATTEMPT', $parameters->status->fail_set[0]->transaction_register_id);
		...
		// Change the status to Past Due... Let's hope that the customer "trues up"...
...
//		$qm->moveToQueue($queue_item, 'collections_new');				
	...
	function is_early_payment($parameters)
	{
		...
		{
			//determine whether or not the ach failure is a paydown/payout/manual payment
...
//		$qm->moveToQueue($queue_item, 'collections_general');
		...
		Remove_Agent_Affiliation($application_id, NULL, 'Watch', 'Owner');
	...
	function State_24($parameters) 
	{
		...
		if($parameters->rules['loan_type_model'] === 'CSO' && !ECash_CSO::hasDefaulted($application_id))
		{
			$failure_amount = null;
			//Get Fees
			$bank_fee = ECash_CSO::getCSOFeeAmount('lend_assess_fee_ach',$application_id,null,null,$failure_amount);
	...
	//
	//Action: Because MCC is from the moon, we are sending a document and rebuilding the schedule as if nothing happened.  
	function State_47($parameters)
	{
		$this->Log("Early Payment returned");
		//////////////////////////////////////
		$holidays = Fetch_Holiday_List();
		$pdc = new Pay_Date_Calc_3($holidays);
		...
		//Now we AREN'T assessing any fees!!!!!
		
		//Send Condor doc to notify customer
		eCash_Document_AutoEmail::Queue_For_Send($this->server, $application_id, 'RETURN_LETTER_EARLY_PAYMENT', $parameters->status->fail_set[0]->transaction_register_id);
		...
		//Add note?
		$comments->add('Early Payment failure',Fetch_Default_Agent_ID());
	}
	...
	function State_26($parameters) 
	{
		...
		// Get the reattempt date
		if (! empty($parameters->next_due_date))
		{
			$date_event = $parameters->next_action_date;
			$this->Log("Date Event is Next Action Date: " . $parameters->next_action_date);
		}
		else
		{
			$date_pair = Get_Next_Payday(date("Y-m-d"), $parameters->info, $parameters->rules);
			$date_event = $date_pair['event'];
			$this->Log("Date Event is Next Pay Date: {$date_event}");
		}
		$db = ECash_Config::getMasterDbConnection();
		...
		{			
			...
			foreach($parameters->status->fail_set as $f)
			{
				$this->Log("Reattemping {$f->tranaction_register_id} on $date_event");
				$ogid = -$f->transaction_register_id;
				Reattempt_Event($application_id, $f, $date_event, $ogid);
			}			
			$db->commit();					     
		} 
		catch (Exception $e) 
	...
	function State_31($parameters) 
	{
		...
		try 
		{
			...
			if ($parameters->status->ach_fee_count == 0) 
			{
				...
				$amounts = array();
				$amounts[] = Event_Amount::MakeEventAmount('fee', -intval($rules['return_transaction_fee']));
				$e = Schedule_Event::MakeEvent($date_event, $date_effective, $amounts, 'payment_fee_ach_fail', 'ACH fee payment');
