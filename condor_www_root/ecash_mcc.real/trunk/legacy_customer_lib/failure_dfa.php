...
class FailureDFA extends DFA {
	...
	function run ($parameters) 
	{
		...
		$log->Write("Current Schedule Stopping Location:");
		$log->Write(var_export($status->stopping_location, true));
		$log->Write(var_export($current_schedule[$status->stopping_location], true));
	...
	function is_inactive_paid($parameters) 
	{
		...
		$chain = $status_util->Get_Status_Chain_By_ID($parameters->application_status_id);
		return ($chain == 'paid::customer::*root');
	}

	function send_to_collections($parameters) 
	{
        Update_Status(null, $parameters->application_id, 'queued::contact::collections::customer::*root', 'NOW()', NULL, false);
