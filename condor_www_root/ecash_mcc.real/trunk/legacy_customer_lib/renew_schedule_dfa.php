...
class RenewScheduleDFA extends DFA {
	...
	function getCSOFeeEvent($parameters, $action_date, $due_date, $rule_name, $fee_name, $fee_description)
	{
		...
		$fee_amount = ECash_CSO::getCSOFeeAmount($rule_name,$parameters->application_id,$action_date,$due_date,null,$this->principal_balance);
		if(! empty($fee_amount))
		{
	...
	function num_sc_only_rollovers_max($parameters) 
	{
		...
		$rollovers = ECash_CSO::getRolloverTerm($parameters->application_id) - 1;
