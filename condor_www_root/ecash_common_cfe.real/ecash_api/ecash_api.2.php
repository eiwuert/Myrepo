...
class eCash_API_2
{
	 ...
	 */
	private function _Get_Rule_Set()
	{
		try
		{
	...
	{
		$query = "SELECT company_id FROM application WHERE application_id = '{$this->application_id}' ";
	 ...
	 * @param int $application_id
	 */
	public function Get_Last_Document_Id($document, $application_id = null)
	{
		...
		$query = "
			...
			WHERE d.application_id = {$application_id}
				AND d.document_event_type != 'failed'
	 ...
	 * 
	 * This uses the LoanAmountCalculator class which uses the 
	 * applicant's business rules for rates.
	 * 
