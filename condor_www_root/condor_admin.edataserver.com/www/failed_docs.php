<?php
	...
	$file_name = CACHE_DIR."failed_{$start_date}_{$end_date}_{$company_id}.png";
	clearstatcache();
	...
	// of "tomorrow"
	$report = new Interval_Report('Failed Sends', $start_date, $end_date, '1 hour', 'gA');
	
	$range = $report->Default_Range();
	$range2 = $report->Default_Range();
	
	$source = Load_Source('email_failed');
	$range->Source($source);
	$report->Add_Source($range, 'Email');
	
	$source2 = Load_Source('fax_failed');
