    ...
    switch (strtolower($filetype))
    {
      case "image":
        $C_MaxSize=$MaxImageSize;
			...
			$C_AbsolutePath=ServerMapPath($filepath,$AbsoluteImageGalleryPath,$ImageGalleryPath);
        break;
      case "flash":
        $C_MaxSize=$MaxFlashSize;
		    $Filter_Array=array(".swf",".flv");
			$C_AbsolutePath=ServerMapPath($filepath,$AbsoluteFlashGalleryPath,$FlashGalleryPath);
        break;
      case "media":
        $C_MaxSize=$MaxMediaSize;
		    $Filter_Array=explode(",",strtolower($MediaFilters));
			$C_AbsolutePath=ServerMapPath($filepath,$AbsoluteMediaGalleryPath,$MediaGalleryPath);
        break;
      case "template":
        $C_MaxSize=$MaxTemplateSize;
		    $Filter_Array=explode(",",strtolower($TemplateFilters));
			$C_AbsolutePath=ServerMapPath($filepath,$AbsoluteTemplateGalleryPath,$TemplateGalleryPath);
        break;
      case "document":
        $C_MaxSize=$MaxDocumentSize;
		    $Filter_Array=explode(",",strtolower($DocumentFilters));
			$C_AbsolutePath=ServerMapPath($filepath,$AbsoluteFilesGalleryPath,$FilesGalleryPath);
        break;
      default:
        break;
    }     
