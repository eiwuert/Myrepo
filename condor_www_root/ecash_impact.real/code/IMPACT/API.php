...
class IMPACT_API extends ECash_API
{
	/**
	 ...
	 */
	protected $ar;
	const ENTERPRISE = 'IMPACT';

	public function __construct($mode = 'RC', $company_short = NULL)
	{
		parent::__construct();
		/*$this->addRequiredIndex(array(
			self::INDEX_BUREAU_INQUIRY, //this should be required, yes/no?
			self::INDEX_PERSONAL_REFERENCE,  // freakin sweet fix for [#15163]
			self::INDEX_STATUS_HISTORY,
			self::INDEX_CUSTOMER, // this is not strictly neccessary (may already exist)
			self::INDEX_CARD // this is optional too
			));*/
		$this->loadConfig($mode, self::ENTERPRISE, $company_short);
	}
	 ...
	 */
	public function loadConfig($mode, $enterprise, $company = NULL)
	{
		$name = NULL;
		...
		/**
		 * IIC uses a separate database instance and therefore requires different
		 * configuration files for the environment.
		 */
		if (!empty($company) && file_exists(dirname(__FILE__)."/Config/$name".'_'.strtoupper($company).".php"))
		{
			$name  .= "_".strtoupper($company);
		}
