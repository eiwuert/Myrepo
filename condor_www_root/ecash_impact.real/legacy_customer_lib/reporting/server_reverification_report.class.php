...
require_once(SERVER_MODULE_DIR."/reporting/reverification_report.class.php");
...
class Customer_Report extends Report
{
	...
	public function Generate_Report()
	{
		...
		try
		{	
			$this->search_query = new Customer_Reverification_Report_Query($this->server);
...
class Customer_Reverification_Report_Query extends Base_Report_Query
{
	...
	public function Fetch_Reverification_Report_Data($date_start, $date_end, $loan_type, $company_id)
	{
		...
		$query = "
			...
			SELECT
				...
				concat(lower(a.name_last), ', ', lower(a.name_first)) AS agent_name,
				temp.application_id,
				c.comment
			 FROM
				loan_type AS lt,
				...
				(SELECT	shv.company_id,
					...
					(SELECT
					  ...
					  FROM
						comment sc
					  WHERE
						sc.company_id     = shv.company_id
					   AND	sc.application_id = shv.application_id
			 ...
			 LEFT OUTER JOIN comment   c  ON temp.comment_id =  c.comment_id
			 WHERE
