<?php
// This report hacked by benb to account for massive amounts of data
// and buffering for slow clients.
...
class Queue_Report extends Report_Parent
{
	...
	protected function Format_Field( $name, $data, $totals = false, $html = true )
	{
		switch( $name )
		{
			case 'ssn':
				if ($html === true)
					return htmlentities(substr($data,0,3)."-".substr($data,3,2)."-".substr($data,5));
				else
	...
	public function Download_XML_Data()
	{
		...
		// PHP not to reach its memory limit, resulting in blank windows when clicking the excel report, etc
