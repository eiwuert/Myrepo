...
require_once(SERVER_MODULE_DIR."/reporting/loan_activity_report.class.php");
...
class Customer_Report extends Report_Generic
{
	...
	public function Generate_Report()
	{
		...
		try
		{
			$this->search_query = new Customer_Loan_Activity_Report_Query($this->server);
			...
			$_SESSION['reports']['cfs_loan_activity']['report_data'] = new stdClass();
			$_SESSION['reports']['cfs_loan_activity']['report_data']->search_criteria = $data->search_criteria;
			$_SESSION['reports']['cfs_loan_activity']['url_data'] = array('name' => 'CFS Loan Activity', 'link' => '/?module=reporting&mode=cfs_loan_activity');
		...
		ECash::getTransport()->Set_Data($data);
		$_SESSION['reports']['cfs_loan_activity']['report_data'] = $data;
	}
}

class Customer_Loan_Activity_Report_Query extends Base_Report_Query
{
	private static $TIMER_NAME    = "CSF Loan Activity Report Query";
	 ...
	 * @returns array
	 */
	public function Fetch_Loan_Activity_Data($start_date, $end_date, $loan_type, $company_id, $date_type = "transaction_date")
	{
	//	$this->timer->Timer_Start(self::$TIMER_NAME);
		...
		$query = "
			...
			SELECT
					...
					app.ssn                                     AS ssn,
					app.customer_id                             AS customer_id,
			...
			JOIN
				    application_tags at on (app.application_id = at.application_id)	
			JOIN
					application_tag_details atd using (tag_id)
			LEFT JOIN
					agent ag ON (ag.agent_id = tr.modifying_agent_id)
			WHERE
					tr.transaction_status IN ('complete', 'failed')
			AND
					atd.tag_name in ('IG_CSHF')
			AND
					lt.name_short IN ({$loan_type_list})
			AND 	app.application_status_id not in ($this->fund_failed)
					{$extra_sql}
				";
	...
	//	$this->timer->Timer_Stop(self::$TIMER_NAME);
	//	echo '<pre>' . $query;
		return $data;
	}
}
