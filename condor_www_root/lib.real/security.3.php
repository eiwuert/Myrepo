<?php
	...
	*/
	...
	class Security_3
	{
		...
		function Security_3 (&$sql_object, $database, $table)
		{
			$this->sql = &$sql_object;
		...
		function Logout_User ($temp_pass, $trace_code = NULL)
		{
			...
			// Replacement temp
			$new_temp = $this->_Mangle_Password (crypt_3::Random_Char());
		...
		function Create_Customer_Account ($account_id, $active_application_id, $login, $trace_code = NULL)
		{
			...
			$query = "
				...
				values 
					('".$account_id."', '".$active_application_id."', '".strtolower($login)."', ".$password_expire_date.", ".$account_expire_date.", '".$hash_pass."',  '".$hash_temp."','TRUE', '0', 'CUSTOMER')";
			$result = $this->sql->Query ($this->database, $query, $trace_code.Debug_1::Trace_Code (__FILE__, __LINE__));
		...
		function _Un_Mangle_Password ($mangled_password)
		{
			...
			$password->message = "The login cannot be reveresed because it is an MD5 hash.";
