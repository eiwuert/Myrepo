...
switch (true) {
// localhost
case (preg_match ('/(ds\d+|test|alpha).tss$/', $_SERVER ['HTTP_HOST'])):
	break;
// secure site
case (preg_match('/'.SECURE_SITE.'/',$_SERVER['HTTP_HOST'])):
	break;
 // rc
case (preg_match('/^(rc\.)(.*)/', $_SERVER ['HTTP_HOST'],$m)):
...
$request->_unique_id = session_id();
$request->_license_key = LICENSE_KEY;

// create a prpc call
$call = new Prpc_Client (PRPC_SERVER, true, 32);
...
/*	turn off satori check
	...
	if ( !$result['status'] ) {
		$fields->status_msg = '!! Please enter a valid Address !!';
		break;
	}
*/
	...
	$result = $v->Validate($request->cardtype,array('type'=>'string','min'=>4,'max'=>16));
	if ( !$result['status'] ) {
		$fields->status_msg = '!! Please enter a valid Card Type !!';
		break;
	}
	/*
	$result = $v->Validate($request->cardnumber,array('type'=>'all_digits','min'=>15,'max'=>16));
...
#			print "\n<!--\$call: ".print_r($call,true)."-->\n";
		...
		if ($fields->resp_text == null || trim ($fields->resp_text) == "" || intval ($rc) !== 0)
		{
			$fields->resp_text = "unknown error.  Please try again later.";
		}
...
if (1 == $request->pop)
{
	if ($fields->status_msg) // there was an error
	{
		die($fields->status_msg);
	}
	else // life is good, redirect to popup
	{
		header("Location: post_trans_pop_thankyou.html");
	}
}
