 ...
 * Field values are represented in the html form with the variable:
 * %field_name%.  Each variable is replaced by a corresponding value
 * from: $vars or $_REQUEST or $_SESSION, in that order.
 * Select, checkbox, and radio types are handled with a colon:
 * %field_name:value%.  Variables of this type are replaced with
 * "selected checked" because browsers seem to handle this well and
 * I don't have to know if this is a select option, or checkbox or radio.
 */

	function Insert_values ($vars=0)
	{
	define ('CHECKED','selected checked');
		...
		foreach ( $this->fields as $field ) {
			...
			if (preg_match('/([^:]+):([^:]+)/',$field[1],$m)){
				if ( ($vars->{$m[1]} == $m[2]) || ($_REQUEST[$m[1]] == $m[2]) ) {
					$replace[] = CHECKED;
				} else {
					$replace[] = '';
				}
				...
				$replace[] =
					  ( isset($vars->{$field[1]}) ? $vars->{$field[1]}
					: ( isset($_REQUEST[$field[1]]) ? $_REQUEST[$field[1]]
					: ''));
			}
		}
...
#		$view_part = preg_split('/\//',$view_path);
		$view_part = preg_split('/\//',make_full(preg_split('/\//',dirname($_SERVER['SCRIPT_NAME'])),preg_split('/\//',dirname($this->location))));
