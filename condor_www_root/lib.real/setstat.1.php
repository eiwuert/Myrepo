<?php
	...
	*/
	...
	class Set_Stat_1
	{
		...
		function _Get_Block($date, $site_id, $vendor_id, $page_id, $promo_id, $promo_sub_code, $sql_object, $database_name, $promo_status, $batch_id = NULL)
		{
			...
			$query = $sql_object->Query($database_name,$sql, Debug_1::Trace_Code (__FILE__, __LINE__));
     			Error_2::Error_Test ($query, TRUE);
		...
		function _Get_Table($date, $block_id, $page_id, $sql_object, $database_name)
		{

			//echo "Set_Stat_1->_Get_Table($date, $block_id, $page_id, $sql_object, $database_name)";
   			...
   			if (!$table_list->$stats_table_name)
			{
				...
				{
					// We have no idea what to do, fail gracefully
					$error = new Error_2 ();
					$error->fatal = FALSE;
					$error->trace_code = $trace_code.Debug_1::Trace_Code (__FILE__, __LINE__);
					$error->message = "Template table (" . $template_table_name . ") does not exist in the database (" . $database_name . ")";
					$error->stats_table = $stats_table_name;
					$error->template_table = $template_table_name;
					$error->table_list = isset($table_list) ? $table_list : 'Empty';
				...
				// Run Query
				$create = $sql_object->Query($database_name, $sql, Debug_1::Trace_Code (__FILE__, __LINE__));
				Error_2::Error_Test ($create);
			}
		...
		function Set_Stat ($block_id, $table_name, $sql_object, $database_name, $column, $increment='1', $stat_model='BOTH') {
			...
			if (($stat_model=='NEW') || ($stat_model=='BOTH')) {
				...
				if ($statpro_key && $statpro_pass) {
					...
					ob_end_clean();
					/*
					// lets check to see if we should send out an email
					if( strlen($crap) > 0 )
					{	
						@mail (
						'mike.genatempo@thesellingsource.com',
						'SetStat::_Crap_Mai::'.session_id(),
		...
		// Write the requested record to the database
		function _Abs_Set_Stat ($block_id, $table_name, $sql_object, $database_name, $data)
		{
		...
		function _Replace_Stat ($block_id, $table_name, $sql_object, $database_name, $data)
		{
			$data = is_object ($data) ? get_object_vars ($data) : $data;
		...
		function Setup_Stats ($site_id, $vendor_id, $page_id, $promo_id, $promo_sub_code, $sql_object, $database_name, $promo_status, $batch_id = NULL)
		{
      		return Set_Stat_1::_Setup_Stats (date ("Y-m-d"), $site_id, $vendor_id, $page_id, $promo_id, $promo_sub_code, $sql_object, $database_name, $promo_status, $batch_id);
		}

		// Wrapper function to run on session start to set initial settings.
		function _Setup_Stats ($date, $site_id, $vendor_id, $page_id, $promo_id, $promo_sub_code, $sql_object, $database_name, $promo_status, $batch_id = NULL)
		{
			
			//Get Block ID
			$block_id = Set_Stat_1::_Get_Block($date, $site_id, $vendor_id, $page_id, $promo_id, $promo_sub_code, $sql_object, $database_name, $promo_status, $batch_id);

			//Get Table Name
			$table_name = Set_Stat_1::_Get_Table($date, $block_id, $page_id, $sql_object, $database_name);
