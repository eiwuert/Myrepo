...
/*
	Modified by Chris Stephens on 2005-04-04
	
	This nice little script has all of the basic jpgraph functionality only it is a lot smaller and cleaner.  This made the following
	functionality a lot easier to add:
		
	1.  addSeriesAssoc -- Use an associate array and plot the X cords based on the tick for that key.
	2.  plotString -- Returns a binary string of the image
*/
...
class series
{
	public $m_values, $m_seriesTitle, $m_strokeColor, $m_fillColor;
	public $m_chart, $m_type;
	public $mode_assoc;
	
	public function __construct(&$chart, $chartType, &$values, $title, $style, $strokeColor, $fillColor)
	{
		$this->m_chart = &$chart;
		...
		$this->mode_assoc = FALSE;
	}
	
	public function setModeAssoc()
	{
		$this->mode_assoc = TRUE;
	}
}

class chart
{
	private $m_title, $m_width, $m_height;
	private $m_strokeColor, $m_backgroundColor, $m_fillColor, $m_fontColor, $m_fontWidth, $m_fontHeight;
	private $m_maxFontWidth, $m_maxFontHeight;
	private $m_minValue, $m_maxValue;
	private $m_minCount, $m_maxCount;
	public $m_image, $m_series;
	private $m_labels, $m_labelsTextColor, $m_labelsFont, $m_labelsFontWidth , $m_labelsFontHeight, $m_labelsDirection;
	private $m_gridHColor, $m_gridVColor, $m_showHGrid, $m_showVGrid, $m_showXAxis, $m_showYAxis;
	private $m_numberOfDecimals, $m_thousandsSeparator, $m_decimalSeparator;
	private $m_style;
	private $m_withLegend, $m_legendStyle, $m_legendStroke, $m_legendFill, $m_legendFont;
	private $label_x;
	
	// Chart constructor
	public function __construct($width, $height, $margin, $backgroundColor)
	{
		$this->m_title = "";
		$this->m_width = $width;
	...
	public function setFormat($numberOfDecimals, $thousandsSeparator, $decimalSeparator)
	{
	...
	public function setLegend($position, $borderStyle, $borderColor, $fillColor, $font)
	{
		//$this->m_legendPosition
		$this->m_legendStyle = $borderStyle;

		$vStrokeColor = _decode_color($fillColor);
	...
	public function setTitle($title, $textColor, $font)
	{
		$this->m_title = $title;
	...
	public function setPlotArea($style, $strokeColor, $fillColor)
	{
		$this->m_style = $style;
		if($strokeColor)
		{
	...
	public function setXAxis($color, $style, $font, $title)
	{
		if(strlen($color) > 0)
		{
	...
	public function setYAxis($color, $style, $font, $title)
	{
		if(strlen($color) > 0)
		{
	...
	// Set grid attributes
	public function setGrid($colorHorizontal, $styleHorizontal, $colorVertical, $styleVertical)
	{
	...
	// Add new series
	public function addSeries($values, $plotType, $title, $style, $strokeColor, $fillColor)
	{
	...
	// Add new series
	public function addSeriesAssoc($values, $plotType, $title, $style, $strokeColor, $fillColor)
	{
		$temp_series = new series($this, $plotType, $values, $title, $style, $strokeColor, $fillColor);
		$temp_series->setModeAssoc();
		
		$this->m_series[] = $temp_series;
	...
	// Set X labels
	public function setLabels(&$labels, $textColor, $font, $direction)
	{
	...
	public function plot($file = '')
	{
		...
		/*
		if( isset($m_withLegend) && $m_withLegend)
		{
		...
		*/
		...
		// marginx --  Margin from the bottom up?
		// dx -- Space between x cords?  $w = $width / ($this->m_maxCount+0.2); $dx = $w * 0.8;
		// sx -- Perhaps the actual graph width?	$w = $width / ($this->m_maxCount+0.2); $dx = $w * 0.8; $sx = $w - $dx;
		// w -- $w = $width / ($this->m_maxCount+0.2); $dx = $w * 0.8;
		...
		for($i=0; $i<count($this->m_labels); $i++)
		{
			...
			if($len > 0)
			{
				$x1 = $dx / 2 + $sx + $marginx + $i * $w;
				
				$this->label_x[$this->m_labels[$i]] = $x1;
				...
				{				
					imagestring ($this->m_image, 
						$this->m_labelsFont,
						$x1 - $len*$this->m_labelsFontWidth/2, 
						$marginy+4+$height, 
						$this->m_labels[$i],
						$this->m_labelsTextColor);	
				}
		...
		foreach($this->m_series as $series)
		{
			...
			switch($series->m_type)
			{
				// LINE PLOT
				case "line":
					reset($series->m_values);
					_set_style($this->m_image,$series->m_style,$series->m_strokeColor, $this->m_fillColor);
					$startx = $marginx + $dx /2 + $sx;
					$starty = $marginy + $height - $dy * ($series->m_values[key($series->m_values)]-$min);
					
					if($series->mode_assoc)
					{	
						foreach($series->m_values as $tick => $m_value_y)
						{
							$x = $this->label_x[$tick];
							$y = $marginy + $height - $dy * ($m_value_y - $min);
	...
	function plotString()
	{
		ob_start();
		$this->plot();
		$binary_image_string = ob_get_contents();
		ob_end_clean();
...
function _imageline($img,$x0,$y0,$x1,$y1)
{
