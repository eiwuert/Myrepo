<?php
    ...
    Reported_Exception::Add_Recipient('email', 'condor-alerts@dev.amgsrv.com');
    
    //Parse command line options
    $args = Console_Getopt::readPHPArgv(); 
    $short_opts = "m::u::p::v::l::";
    $long_opts = array('mode=','user=','pass=','verbose','log');
    ...
    if($parsed_opts['l']) {
        ...
        $log->lfile('/tmp/get_new_emails_'.date('Y-m-d_H-i-s-T').'_'.$my_pid.'.log');
        
        $log->lwrite("Log File get_new_emails_".date('Y-m-d_H-i-s-T').'_'.$my_pid." Started");
    }
    ...
    if(file_exists(LOCK_FILE)) {
        if($DEBUG_MODE) fwrite(STDOUT,"Another process may be running\n");
        if($LOG_MODE) $log->lwrite("Another process may be running\n");
        // Check to see if the lock file is legit
        $checkpid = file_get_contents(LOCK_FILE);
    
        if($DEBUG_MODE) fwrite(STDOUT,"Checking to see if PID {$checkpid} is running (and is killable)\n");
        if($LOG_MODE) $log->lwrite("Checking to see if PID {$checkpid} is running (and is killable)\n");
        $running = posix_kill($checkpid, 0);
    
        $posix_ret = posix_get_last_error();
        if($DEBUG_MODE) fwrite(STDOUT,"posix_get_last_error() ". var_export($posix_ret, TRUE). "\n");
        if($LOG_MODE) $log->lwrite("posix_get_last_error() ", var_export($posix_ret, TRUE), "\n");
        ...
        if ($posix_ret == 0) {
            if (file_exists(WARN_FILE) || filemtime(LOCK_FILE) + (10 * 60 /* 10 minutes */) > time()) {
                if($DEBUG_MODE) fwrite(STDOUT,"Lock file still open\n");
                if($LOG_MODE) $log->lwrite("Lock file still open\n");
                die(WARN_FILE);
                exit(2);
            } else {
                if($DEBUG_MODE) fwrite(STDOUT,"Condor lock file has been open for too long\n");
                if($LOG_MODE) $log->lwrite("Condor lock file has been open for too long\n");
            ...
            if($DEBUG_MODE) fwrite(STDOUT,"Process not running, removing stale lock file\n");
            if($LOG_MODE) $log->lwrite("Process not running, removing stale lock file\n");
            // Stale lock file
            unlink(LOCK_FILE);
    ...
    $my_pid = getmypid();
    if($DEBUG_MODE) fwrite(STDOUT,"My PID {$my_pid}\n");
    if($LOG_MODE) $log->lwrite("My PID {$my_pid}\n");
    ...
    switch($parsed_opts['m']) {
        case "rc":
            ...
            break;
        case "live":
            $url = 'condor.loanservicingcompany.com/condor_api.php';
            break;
    }
    if(strlen($url) > 0) {
    ...
    if(!is_object($condor)) {
        ...
        if($LOG_MODE) $log->lwrite("Could not create condor object.\n");
    }
    ...
    if(!is_numeric($user_id) || !is_numeric($company_id)) {
        ...
        if($LOG_MODE) $log->lwrite("The company id ($company_id) or user_id ($user_id) did not exist.\n");
    }
    $mail_servers = $condor->Get_All_Pop_Accounts('INCOMING');
    if(!is_array($mail_servers)) {
        if($LOG_MODE) $log->lwrite("NO mailservers\n");
        die("NO mailservers");
        return;
    }
    ...
    foreach($mail_servers as $mail_settings) {
        ...
        //Get Message Count
        if($DEBUG_MODE) fwrite(STDOUT,"Getting message count\n");
        if($LOG_MODE) $log->lwrite("Getting message count\n");
        $msg_count = $mail->messageCount();
    
        if($msg_count == 0) {
            if($DEBUG_MODE) fwrite(STDOUT,"No Messages\n");
            if($LOG_MODE) $log->lwrite("No Messages\n");
            continue;
        } else {
            if($DEBUG_MODE) fwrite(STDOUT,"There's $msg_count messages\n");
            if($LOG_MODE) $log->lwrite("There's $msg_count messages\n");
        }
    
        // Used for calculating whether an email has reached deletion age
        $two_days_ago = strtotime('-2 days');
        if($msg_count == 0) {
		if($DEBUG_MODE) fwrite(STDOUT,"Old messages delete date: ".date("F j, Y, g:i a",$two_days_ago)."\n");
		if($LOG_MODE) $log->lwrite("Old messages delete date: ".date("F j, Y, g:i a",$two_days_ago)."\n");
	}
        ...
        for($mid = 1; $mid <= $msg_count; $mid++) {
            ...
            $message_delivered_to = clean_email($structure->headers['delivered-to']);
            ...
            if($DEBUG_MODE) {
                ...
                fwrite(STDOUT,"\n From: ".$message_from);
                fwrite(STDOUT,"\n Type: ".$message_type);
                fwrite(STDOUT,"\n Date: ".date("F j, Y, g:i a",$message_date));
                fwrite(STDOUT," \n ");
            }
...
/*    
            $blacklist_addresses = array(
                'info@realestateworldnews.net',
                'support@dodeals.com',
                'Mykids27407@yahoo.com',
            );
    
            foreach($blacklist_addresses as $ba)
            {
                if(strcasecmp($message_from, $ba) == 0)
                {
                    var_dump($structure->headers);
                    $mail->delete($mid);
                    echo "Deleting blacklisted email from $ba\n";
                    continue;
                }
            }
*/    
    
//    		if(strcasecmp($message_from, 'cbowden7@verizon.net') == 0)
//    		{
//    			echo "Deleting incoming mail for cbowden7@verizon.net\n";
//    			$mail->delete($mid);
//    			continue;
//    		}
            ...
            if($condor->Does_Unique_Id_Exist($message_id)) {
                if($DEBUG_MODE) fwrite(STDOUT,"Existing message found in condor: ID {$message_id}\n");
                if($LOG_MODE) $log->lwrite("Existing message found in condor: ID {$message_id}\n");
                //Delete old email
                if($message_date <= $two_days_ago) { //2 Days
                    $mail->delete($mid);
                    if($DEBUG_MODE) fwrite(STDOUT,"Deleted {$mid}\n");
                    if($LOG_MODE) $log->lwrite("Deleted {$mid}\n");
                }
                continue;
            }
            //Document and Parts
            ...
            if(isset($structure->parts)) {
                ...
                for($x=0; $x < count($parts); $x++) {
                    ...
                    // [benb]
if($DEBUG_MODE) fwrite(STDOUT,"\n part ".$x."  type=".$parts[$x]['TYPE']." \n ");
if($LOG_MODE) $log->lwrite("\n part ".$x."  type=".$parts[$x]['TYPE']." \n ");
//if($DEBUG_MODE) print_r($parts[$x]['CONTENT']);

                    if(strtolower($parts[$x]['TYPE']) == 'text/plain' || strtolower($parts[$x]['TYPE']) == 'text/html') {
                        $t0 = trim($parts[$x]['CONTENT']);
                        if (empty($t0)) $parts[$x]['CONTENT'] = 'empty';
                ...
                if(!isset($document['FROM'])) {
                    if ($DEBUG_MODE) fwrite(STDOUT,"Document has no main body!\n");
                    if ($LOG_MODE) $log->lwrite("Document has no main body!\n");
                    $document = array("FROM"     => $message_from,
                                      "TO"       => $message_to,
                                      "TYPE"     => 'text/plain',
                                      "CONTENT"  => 'empty',
                ...
                if(isset($structure->headers['content-transfer-encoding'])
                    && $structure->headers['content-transfer-encoding'] == 'quoted-printable') {
            ...
            //Insert Document
            if($DEBUG_MODE) fwrite(STDOUT," Inserting Document: \n");
            if($LOG_MODE) $log->lwrite(" Inserting Document: \n");
            ...
            if($DEBUG_MODE) fwrite(STDOUT," Archive ID: " . $a_id . " \n ");
            if($LOG_MODE) $log->lwrite(" Archive ID: " . $a_id . " \n ");
            //Insert Parts
            foreach($parts as $part) {
                if($DEBUG_MODE) {
                    fwrite(STDOUT, "  Inserting Attachment:  ");
                    fwrite(STDOUT, "  ".$part['TYPE']." \n ");
                }
                if($LOG_MODE) {
                    $log->lwrite("  Inserting Attachment:  ");
                    $log->lwrite("  ".$part['TYPE']." \n ");
                }
                if($condor->Create_As_Email_Attachment($a_id, $part['TYPE'], $part['CONTENT'], $part['SUBJECT'])) {
                    if($DEBUG_MODE) fwrite(STDOUT,"Inserted successfully.\n");
                    if($LOG_MODE) $log->lwrite("Inserted successfully.\n");
                } else {
                    if($DEBUG_MODE) fwrite(STDOUT,"Not so successful.\n");
                    if($LOG_MODE) $log->lwrite("Not so successful.\n");
                }
            }
        }
    ...
    function Get_Parts($message) {
        ...
        foreach($message->parts as $part) {
            ...
            if(isset($part->parts) && !empty($part->parts)) {
                ...
                if(!isset($part->d_parameters['filename']) && 
                   ($part_type == 'text/plain' || $part_type == 'text/html')) { //Plain Text
                    ...
                    if(isset($message->headers['content-transfer-encoding'])
                        ...
                        $document_content = quoted_printable_decode($document_content);
                    }
                } else { //Binary
                ...
                $part_type = Infer_MIME_Type($document_content, $part_type);
    ...
    /**
     * This will attempt to 'infer' a MIME type for the given content.
     *
     * @todo When PHP 5.3 is available OR if the fileinfo package is available.
     * 
     * @param string $content
     * @param string $default_type
     * @return string
     */
    function Infer_MIME_Type($content, $default_type) {
    ...
    function Get_Mail_Object($config) {
        ...
        $mconnect = 'pop3://'.$config->mail_user.':'.
            $config->mail_pass.'@'.$config->mail_server.':'.
            $config->mail_port.'/'.$config->mail_box.$options;
    
        if($DEBUG_MODE) fwrite(STDOUT,"Trying to connect to {$mconnect}...");
        if($LOG_MODE) $log->lwrite("Trying to connect to {$mconnect}...");
        if(!$mail->connect($mconnect)) {
            echo "Mail server error:\n";
            echo "Account: {$config->mail_user} @ {$config->mail_server}:{$config->mail_port}\n";
        ...
        if($DEBUG_MODE) fwrite(STDOUT,"Success\n");
        if($LOG_MODE) $log->lwrite("Success\n");
        return $mail;
    }
?>
