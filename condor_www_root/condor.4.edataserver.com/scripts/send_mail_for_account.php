...
require_once('../lib/PriorityServerList.php');
...
class Send_Mails
{
	...
	public function __construct($mode,$account_id)
	{
		...
		$priority_server_list = $this->Load_Priority_Servers($account_id);
		...
		$opts = Array(
				...
				'persist' => FALSE,
				'PriorityServerList' => $priority_server_list
			);
	...
	public function Get_New_Mails()
	{
		...
		if (is_array($mails)) foreach($mails as $mail)
		{
			$this->Send_Mail($mail);
		}
	}
	...
	public function Mark_Old_Failed()
	{
		...
		if ($mails) foreach($mails as $key=>$mail)
		{
	...
	private function Send_Mail($obj)
	{
		...
		if(is_string($to) && strpos($to,'@') !== FALSE)
		{
			...
			self::Display(" document found: {$obj->document_id}");

			}
			catch (Exception $e)
			{
self::Display(" document not found: {$obj->document_id}");

				// HACK HACK HACK HACK
				// This is a temporary fix that keeps nita from bitching about the queues backing up
				// but the packing routines in prpc are messed up, hopefully I'll get some time allocated to fix this
			...
			{
				$mime = new Mail_mime("\n");
				 self::Display(" starting to send: {$obj->document_id}");
				...
				$head = Array(
					...
					'X-xsMessageId' => $this->Normalize_Custom_Headers($obj->dispatch_id),
					'X-xsMailingId' => $this->Normalize_Custom_Headers($obj->template_name) . '-' . $obj->template_id
				);
				...
				try 
				{
					...
					$ret = $this->smtp->Send($to, $head, $body, $mail_from, $obj->send_priority);
			...
			else 
			{
				...
				self::Display("fail 1: {$obj->document_id}");

			}
		}
		else 
		{
self::Display("fail 2: {$obj->document_id}");
	...
	private function Get_Company_Data($company_id)
	{
		...
		try 
		{
			...
			if(($row = $res->Fetch_Object_Row()))
			{
				...
				$this->condor_api = new PRPC_Client($prpc_url);
			}
		}
		catch (Exception $e)
		{
	...
	private function Load_Pop_Account_By_Id($id)
	{
		...
		try 
		{
			...
			if(($row = $res->Fetch_Object_Row()) && (isset($row->mail_pass)) && (!empty($row->mail_pass)))
			{
	...
	/**
	 * Load the priority servers from the database.
	 * 
	 * @param int $account_id
	 */
	private function Load_Priority_Servers($account_id)
	{
		...
		$query = "
			SELECT
				...
				minimum_priority
			FROM
				priority_smtp_config
			WHERE
				account_id = {$account_id}
		";
		
		$server_list = new PriorityServerList();
		...
		try
		{
			...
			{
				$server_list->add(PriorityServer::newInstance($row->host, $row->port, $row->minimum_priority));
			}
		}
		catch (Exception $e)
		{
