...
/**
 * Since the PEAR SMTP mail class hates me and doesn't let 
 * you set the mail-from seperate from the From: Header, here's
 * my very similar version, that allows the mail from stuff to
 * be used.
 */
/* Changelog:
 *
 * 2008-09-19 - Fixed Issue where there was a fatal error when a
 *              fatal error occurred because the exception referenced
 *              a object that wasn't created yet. [benb]
 */
...
class SMTP_Mail
{
	...
	/**
	 * @var PriorityServerList
	 */
	private $priority_server_list;
	 ...
	 * @param int $priority
	 */
	public function Send($recipients, $headers, $body, $mail_from = NULL, $priority = PriorityServer::MAX_PRIORITY)
	{
		try
		{
	...
	/**
	 * Sets the priority server list.
	 * 
	 * @param PriorityServerList $list
	 */
	public function Set_PriorityServerList(PriorityServerList $list)
	{
		$this->priority_server_list = $list;
	}
	...
	{
		$server = $this->priority_server_list->get($priority);
		if ($server == FALSE)
		{
			return array($this->host, $this->port);
		}
		
		return array($server->getHost(), $server->getPort());
	}
