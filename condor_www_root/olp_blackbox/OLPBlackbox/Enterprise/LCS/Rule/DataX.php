...
/**
 * OLPBlackbox_Enterprise_LCS_Rule_DataX class file.
 * GForge Ticket #9883
 * 
 * @package OLPBlackbox
 * @author  Adam Englander <adam.englander@sellingsource.com>
 *  
 * @desc CLS Specific Rules copied directly from AALM as DataX calls are identical.
 * 
 */
class OLPBlackbox_Enterprise_LCS_Rule_DataX extends OLPBlackbox_Rule_DataX
{
	const TYPE_PERF = 'lcs-perf';
	...
	protected function onInvalid(Blackbox_Data $blackbox_data, Blackbox_IStateData $state_data)
	{
		...
		{
			case OLPBlackbox_Enterprise_LCS_Rule_DataX::TYPE_PERF:
	...
	protected function findDecision(Blackbox_Data $blackbox_data, Blackbox_IStateData $state_data)
	{
		...
		{
			$this->hitSiteStat('aa_lcs_cra_denial', $blackbox_data, $state_data);
		}
	}
	...
	protected function getSourceID()
	{
		...
		switch ($this->call_type)
		{
			case self::TYPE_PERF: $source_id = 15; break;
		}
	...
	protected function hitCustomStats($valid, Blackbox_Data $blackbox_data, Blackbox_IStateData $state_data)
	{
		...
		$stat[] = ($valid) ? 'lcs_perf_pass' : 'lcs_perf_fail';
