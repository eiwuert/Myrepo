...
class OLPBlackbox_Factory_Legacy_SuppressionList
{
	 ...
	 * 
	 * The array passed to this function is expected to have keys with the ID's of the suppressions lists
	 * with values of the type of list (EXCLUDE, VERIFY, etc...).
	 *
	 * @param array $lists the lists to create
	 * @param array $allowed_fields an array of strings containing fields that we'll restrict lists to
	 * @return void
	 */
	public function getSuppressionLists(array $lists, array $allowed_fields = NULL)
	{
		$db = OLPBlackbox_Config::getInstance()->olp_db;
		$db_name = OLPBlackbox_Config::getInstance()->olp_db->db_info['db'];
		...
		foreach ($lists as $id => $type)
		{
			...
			switch ($list_type)
			{
				case 'EXCLUDE':
					...
					break;
				case 'VERIFY':
					$list_rule = new OLPBlackbox_Rule_Suppression_Verify($list);
					break;
				case 'CATCH':
