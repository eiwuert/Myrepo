...
class OLPBlackbox_Factory_Legacy_OLPBlackbox
{
	...
	public function __construct()
	{
		...
		// Stat limits need to be incremented in some places
		$this->config->stat_limits = OLPBlackbox_Factory_Legacy_LimitCollection::getStatLimits();
	}
	...
	protected function addLegacyStateCheckRules(Blackbox_RuleCollection $rules)
	{
		if (OLPBlackbox_Config::getInstance()->blackbox_mode ==  OLPBlackbox_Config::MODE_BROKER
       	...
       	{
			$rules->addRule(new OLPBlackbox_Rule_LegacyStateExclude('VA'));
			$rules->addRule(new OLPBlackbox_Rule_LegacyStateExclude('WV'));
			$rules->addRule(new OLPBlackbox_Rule_LegacyStateExclude('GA'));
		}
	}
	...
	protected function getBlackboxData()
	{
		...
		if (isset($config->bb_sequential_preferred)
			&& $this->config->blackbox_mode ==  OLPBlackbox_Config::MODE_BROKER)
		{
		...
		$query = self::getCampaignQuery($tier_clause);
	 ...
	 */
	public static function getCampaignQuery($clause)
	{
		return "
			SELECT
	...
	protected function restrictAndExclude(array $data)
	{
		...
		if (!empty($restricted_targets) || !empty($excluded_targets))
		{
			...
			{
				$final_data = $this->sequentialRestrictAndExclude($data, $final_data);
			}
	...
	protected function restrictForceWinners(array $data)
	{
		...
		if (!empty($force_winner_targets))
		{
			...
			if (!empty($data['sequential']))
			{
				foreach ($data['sequential'] as $target)
				{
	 ...
	 * @return unknown
	 */
	protected function sequentialRestrictAndExclude(array $data, array $final_data)
	{
		$restricted_targets = $this->debug->getFlag(
			OLPBlackbox_DebugConf::TARGETS_RESTRICT
		);
		
		$final_data['sequential'] = array();
	...
	private function addTargets($parent_target_collection, $targets_to_add)
	{
		...
		foreach ($targets_to_add as $target_row)
		{
			...
			else
			{
				...
				$target->getStateData()->list_mgmt_nosell = (strcasecmp($target_row['list_mgmt_nosell'], 'TRUE'));
			...
			// If this target has withheld targets, get that info into the state data.
			if ($target_row['withheld_targets'])
			{
				// Since coming out of the db they are comma and space separated
				// we need to strip out any spaces otherwise our array ends up with
				// spaces that would likely throw off checks being performed.
				$withheld_targets = explode(',', str_replace(' ', '', $target_row['withheld_targets']));
				$target->getStateData()->withheld_targets = $withheld_targets;
			}

			// Create the campaign and add set this target inside it.
			$weight = ($target_row['percentage']) ? (int)$target_row['percentage'] : (int)$target_row['priority'];
