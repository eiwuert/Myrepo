 ...
 */
class OLPBlackbox_Rule_Suppression_RestrictTest extends PHPUnit_Framework_TestCase
{
	/**
	...
	public function testIsValid($list_return, $expected_result, $expected_valid)
	{
		...
		$rule->expects($this->once())->method('hitRuleEvent')
			->with($expected_result, $this->bb_data, $this->state_data);
	...
	public function testIsValidCache($list_return, $expected_result, $expected_valid)
	{
		...
		$rule->expects($this->exactly(2))->method('hitRuleEvent')
			->with($expected_result, $this->bb_data, $this->state_data);
