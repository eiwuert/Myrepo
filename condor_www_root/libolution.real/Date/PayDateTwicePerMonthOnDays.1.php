...
class Date_PayDateTwicePerMonthOnDays_1 extends Date_PayDateModel_1
{
	...
	private function normalizeMonth($timestamp, $dom)
	{
		...
		$calc_date = strtotime("{$date_info['month']} {$dom}");

		$calc_info = getdate($calc_date);
		if($date_info['month'] != $calc_info['month'])
		{
			//calculate the last day of $timestamp's month
			return strtotime("{$calc_info['month']} 1 -1 day");
		}
		return $calc_date;
	}
}
