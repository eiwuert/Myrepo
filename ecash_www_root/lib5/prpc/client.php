...
class Prpc_Client extends Prpc_Base
{
	...
	function _Prpc_Set_Url ($url = NULL)
	{
		...
		$this->_prpc_url_user = $m[2] ? $m[2] : NULL;
		$this->_prpc_url_pw = $m[3] ? $m[3] : NULL;
		$this->_prpc_url_host = $m[4];
		$this->_prpc_url_port = $m[5] ? $m[5] : $this->_prpc_url_scheme === 'tls' ? 443 : 80;
		$this->_prpc_url_path = $m[6];
	...
	{
		$sock = @fsockopen (($this->_prpc_url_scheme=='tls' ? 'tls://' : '').$this->_prpc_url_host, $this->_prpc_url_port, $errno, $errstr);
		if (! $sock)
		{
		...
		$head =
			...
			"Host: ".$this->_prpc_url_host.(($this->_prpc_url_port != 80 && $this->_prpc_url_port != 443) ? ':'.$this->_prpc_url_port : '')."\r\n".
