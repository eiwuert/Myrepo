...
class CreateScheduleDFA extends DFA {
	...
	public function run($parameters) 
	{
		...
		if($flags->get('card_disbursement') && $parameters->may_use_card_schedule)
		{
			$this->disbursement = 'card_disbursement';
		}
		else
		{
			$this->disbursement = 'loan_disbursement';
		}
		...
		// Include the reaction due date for the grace period for react apps funded in the future
		if ($info->is_react && (strtotime($start_date) > time())){
		...
		$gp = $parameters->pdc->Get_Calendar_Days_Forward($start_date, $grace_days);
...
//error_log('looking for refi balance in: '.__METHOD__.' || '.__FILE__);
//error_log(' parameters: '.print_r($parameters,true));
		$refi_totals = 0;

		foreach ($parameters->schedule as $e)
		{
			if (in_array($e->type, array('loan_disbursement', 'card_disbursement', 'moneygram_disbursement', 'check_disbursement'))
				&& in_array($e->status, array('scheduled','pending','complete')))
			{
				 return 1;
			}
			if (in_array($e->type, array('converted_principal_bal')))
			{
			    foreach ($e->amounts as $p)
			    $refi_totals += $p->amount;
			}

		}
//error_log(' refi totals: '.$refi_totals);
		$parameters->fund_amount -= $refi_totals;
	...
	{
//error_log('Funding in: '.__METHOD__.'  ||  '.__FILE__);
//error_log(' parameters: '.print_r($parameters,true));
		$fund_date_stamp =  strtotime($parameters->fund_date);
		if($parameters->pdc->isBusinessDay($fund_date_stamp) && Has_Batch_Closed($parameters->company_id))
		{
			$allow_batch = false;
			/*
			...
			*/
			if(($parameters->rules['ach_weekend_batch']['allow_holiday'] == 'Yes' && $parameters->pdc->Is_Holiday(strtotime($parameters->pdc->Get_Calendar_Days_Forward($parameters->fund_date, 1)))) ||
				($parameters->rules['ach_weekend_batch']['allow_sun'] == 'Yes' && date("D", $fund_date_stamp) == 'Sat') ||
				($parameters->rules['ach_weekend_batch']['allow_sat'] == 'Yes' && date("D", $fund_date_stamp) == 'Fri'))
			{
				$action_date = $parameters->pdc->Get_Calendar_Days_Forward($parameters->fund_date, 1);
				$allow_batch = true;
			}
			/*
			...
			*/
			elseif($parameters->rules['ach_weekend_batch']['allow_sun'] == 'Yes' && date("D", $fund_date_stamp) == 'Fri')
			{
				$action_date = $parameters->pdc->Get_Calendar_Days_Forward($parameters->fund_date, 2);
				$allow_batch = true;
			...
			*/
			...
			{
				$action_date = $parameters->pdc->Get_Next_Business_Day($parameters->fund_date);
				$due_date = $parameters->pdc->Get_Next_Business_Day($action_date);
			...
			*/
			if($allow_batch)
			{
				...
				{
					$action_date     = $parameters->pdc->Get_Business_Days_Forward($action_date, $parameters->rules['ach_weekend_batch']['action_forward']);
					$due_date = $parameters->pdc->Get_Business_Days_Forward($action_date, $parameters->rules['ach_weekend_batch']['effective_forward']);
				}
				else
				{
					$action_date     = $parameters->pdc->Get_Calendar_Days_Forward($action_date, $parameters->rules['ach_weekend_batch']['action_forward']);
					$due_date = $parameters->pdc->Get_Calendar_Days_Forward($action_date, $parameters->rules['ach_weekend_batch']['effective_forward']);					
				}
			}

			
		}
		else if(! $parameters->pdc->isBusinessDay($fund_date_stamp))
		{
			if(($parameters->rules['ach_weekend_batch']['allow_holiday'] == 'Yes' && $parameters->pdc->Is_Holiday($fund_date_stamp)) ||
				($parameters->rules['ach_weekend_batch']['allow_sun'] == 'Yes' && date("D", $fund_date_stamp) == 'Sun') ||
				($parameters->rules['ach_weekend_batch']['allow_sat'] == 'Yes' && date("D", $fund_date_stamp) == 'Sat'))
			{
			...
			{
				$action_date     = $parameters->pdc->Get_Business_Days_Forward($parameters->fund_date,1);
		...
		$parameters->fund_date = $action_date;
		...
		else
		{
			$this->new_events[] = Schedule_Event::MakeEvent($action_date, $due_date,
					  				$amounts, $this->disbursement,'Fund of Loan');
		}
		...
		foreach ($parameters->schedule as $e)
		{
			...
			{
				foreach ($e->amounts as $p)
					$this->principal_balance += $p->amount;
			}
		}
	...
	public function add_interest_payment($parameters)
	{
		...
		{
			if(($parameters->rules['ach_weekend_batch']['allow_holiday'] == 'Yes' && $parameters->pdc->Is_Holiday(strtotime($parameters->fund_date))) ||
				($parameters->rules['ach_weekend_batch']['allow_sun'] == 'Yes' && date("D", strtotime($parameters->fund_date)) == 'Sun') ||
				($parameters->rules['ach_weekend_batch']['allow_sat'] == 'Yes' && date("D", strtotime($parameters->fund_date)) == 'Sat'))
			{
				$sc_date = 	$parameters->pdc->Get_Business_Days_Forward($parameters->fund_date, $parameters->rules['ach_weekend_batch']['effective_forward']);
			}
			else
			{
				$sc_date = $this->fund_effective;
			}
			
		}
		else
		{
			$sc_date = $this->last_date;
		}

		$this->last_date = $this->dates['effective'][0];

		$days   = Interest_Calculator::dateDiff($sc_date, $this->last_date);
		$amount = Interest_Calculator::calculateDailyInterest($this->rules, $this->principal_balance, $sc_date, $this->last_date);

		$first_date_display = date('m/d/Y', strtotime($sc_date));
