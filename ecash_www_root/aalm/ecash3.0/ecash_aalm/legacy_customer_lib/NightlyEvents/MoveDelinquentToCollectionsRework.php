	...
	private function Move_Delinquent_To_Collections_Rework($start_date, $end_date)
	{
		...
		$query = "
			...
			WHERE
				...
				(
					SELECT
						if (icpr.fatal_fail = 1,'yes','no')
					FROM
						card_process icp
					JOIN
						card_process_response icpr ON (icpr.reason_code = icp.reason_code)
					WHERE
						icp.application_id = app.application_id
					AND
						DATEDIFF(NOW(), icp.date_modified) > {$days}
					ORDER BY icp.reason_code DESC
					LIMIT 1
				) = 'yes'
