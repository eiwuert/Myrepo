...
class QuickCheckDFA extends DFA
{
	...
	function has_fatal_code($parameters) 
	{
		...
		if (($parameters->arc_map[$f->ach_return_code_id]) || ($parameters->crc_map[$f->card_return_code_id]))
		{
			if (($parameters->arc_map[$f->ach_return_code_id]['is_fatal'] == 'yes') || ($parameters->crc_map[$f->card_return_code_id]['is_fatal']))
			return 1;
		}
		else
		{
	...
	function State_5($parameters) 
	{
		...
		$queue_log->Write(__FILE__.":".'$Revision: 1069 $'.":".__LINE__.":".__METHOD__."()",LOG_NOTICE);
	...
	function State_6($parameters) 
	{
		...
		try 
		{
			$db->beginTransaction();
			Remove_Standby($parameters->application_id);
			Set_Standby($parameters->application_id, $parameters->company_id, 'qc_ready');
			Update_Status(null, $parameters->application_id, "return::quickcheck::collections::customer::*root", NULL, FALSE);
