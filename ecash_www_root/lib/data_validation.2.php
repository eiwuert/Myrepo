	...
	public function Normalize_Engine($field, $param)
	{
		...
		*/
			switch ($param["type"])
			{
				...
				case "date":
					$field = preg_replace("~(\d{1,2})[/-](\d{1,2})[/-](\d{4})~", "$3-$1-$2", $field);
				break;
				...
				case 'real_dollar_amount':
					/// remove dollar sign at the beginning and  anything after the decimal point
                    // everything else should be considered an error
                    $field = preg_replace('/^\$|(\..*)|,/', '', $field);
					break;
				...
				case 'phone_number_full':
					$field = preg_replace('/^1(.{10,})$/', '$1', $field); // Note: no 'break' statement
				case 'phone_number':
					$field = preg_replace('/[^\d]/', '', $field);
					break;
