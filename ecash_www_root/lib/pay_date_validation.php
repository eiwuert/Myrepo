...
class Pay_Date_Validation
{
   ...
   private $record_bad_paydate;
	...
	/**
	 * Validates paydate information. Returns an array with errors and warnings.
	 *
	 * @param bool $allow_holidays
	 * @param bool $allow_weekends
	 * @param bool $allow_dates_in_past
	 * @return array
	 */
	public function Validate_Paydates($allow_holidays = FALSE, $allow_weekends = FALSE, $allow_dates_in_past = FALSE)
	{
		$twice['00'] = 0;
		$twice['01'] = 0; 
		$twice['02'] = 0;
		...
		if (isset($this->pay_stamp_1)
			&& (
				($this->_Is_Holiday($this->pay_stamp_1) && !$allow_holidays)
				|| ($this->_Is_Weekend($this->pay_stamp_1) && !$allow_weekends)
			)
		)
		{
		...
		if (!$this->bypass_error_checking)
		{
			if (isset($this->pay_stamp_2)
				&& (
					($this->_Is_Holiday($this->pay_stamp_2) && !$allow_holidays)
					|| ($this->_Is_Weekend($this->pay_stamp_2) && !$allow_weekends)
				)
			)
			{
		...
		{	
			...
			if (isset($this->pay_stamp_3)
				&& (
					($this->_Is_Holiday($this->pay_stamp_3) && !$allow_holidays)
					|| ($this->_Is_Weekend($this->pay_stamp_3) && !$allow_weekends)
				)
			)
			{
			...
			if (isset($this->pay_stamp_4)
				&& (
					($this->_Is_Holiday($this->pay_stamp_4) && !$allow_holidays)
					|| ($this->_Is_Weekend($this->pay_stamp_4) && !$allow_weekends)
				)
			)
			{
		...
		if (!count($this->errors))
		{
			...
			{
				switch (strtoupper($this->form_data["income_frequency"]))
				{
					case "WEEKLY":
					...
					case "TWICE_MONTHLY":
						...
						{
							$this->warnings[] = 'bad_twice_monthly';
						}
		...
		return array("errors" => $this->errors, "pay_dates" => $this->pay_dates, "warnings" => $this->warnings );
	}
			...
			switch ($this->form_data["income_frequency"] )
			{
				...
				case "FOUR_WEEKLY":
					if (isset($this->pay_dates['pay_date2']))
					{
						$this->pay_dates["pay_date3"]
							= date("Y-m-d",
									strtotime($this->pay_dates["pay_date2"]) + (1209600 * 2)); //+4 Week
						$this->pay_dates["pay_date4"]
							= date("Y-m-d",
									strtotime($this->pay_dates["pay_date2"]) + (2419200 * 4)); //+8 Week
					}
				break;
				case "TWICE_MONTHLY":
					if (isset($this->pay_dates['pay_date1'])
	...
	public function _Gen_Date_Checks()
	{
		...
		if (isset($this->form_data["income_frequency"]))
		{
			...
			{	
				...
				case "FOUR_WEEKLY":
					if($test && $this->pay_stamp_2 != $this->pay_stamp_1 + (1209600 * 2))
					{
						$this->pay_stamp_1 = $test;
					}
					...
					{
						$ix4 = $i*4;
						$this->checks[$i] = strtotime ("+$ix4 weeks", $this->pay_stamp_1);
					}
					foreach ($this->checks as $key => $date)
					{
						if ($this->_Is_Weekend($date) || $this->_Is_Holiday($date))
	...
	public function _Is_Valid($date)
	{
		...
		do
		{
			$date_forward =strtotime("+1 day", $date_forward);
		} while( $this->_Is_Weekend($date_forward) || $this->_Is_Holiday($date_forward));
		
		do
		{
			$date_backward =strtotime("-1 day", $date_backward);
	...
	public function _Near_Holiday($date)
	{
		...
		$date_forward = strtotime('+1 day', $date);
		$date_backward = strtotime('-1 day', $date);
	...
	public function _Check_vs_Input()
	{
		...
		switch ($this->form_data["income_frequency"])
		{
			...
			case "BI_WEEKLY":
				...
				{
					if (!in_array ($this->pay_stamp_3, $this->checks))
						$this->errors[] = "pay_date3_biweekly";
					if (!in_array ($this->pay_stamp_4, $this->checks))
			...
			case "FOUR_WEEKLY":
				if (!in_array ($this->pay_stamp_2, $this->checks) && !$this->bypass_error_checking )
					$this->errors[] = "pay_date2_fourweekly";
				
				if( $this->validate_4_paydates )
				{
					if (!in_array ($this->pay_stamp_3, $this->checks))
						$this->errors[] = "pay_date3_fourweekly";
					if (!in_array ($this->pay_stamp_4, $this->checks))
						$this->errors[] = "pay_date4_fourweekly";
				}
			break;
	...
	public function _Advanced_Payday( )
	{
		if (isset($this->form_data["income_frequency"]) && !$this->bypass_error_checking)
		{
			switch ( $this->form_data["income_frequency"] )
			{
				...
				break;
				case "FOUR_WEEKLY":
					if($this->pay_stamp_1 > mktime(0,0,0)+(1382400*2)) //+32 Days
						$this->errors[] = "pay_date1_weekly_far";
				break;
			}
		}
