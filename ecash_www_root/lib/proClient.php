<?php
	...
	class proClient
	{
		...
		function openJournal ()
		{
			...
			for ($i = 0, $f = false ; $i < 100 && !$f ; $i++)
			{
				foreach ($glob as $journalName)
				{
					...
					if ( ($this->journalFlock = @fopen($this->journalLock, 'x')) !== FALSE && flock($this->journalFlock, LOCK_EX) )
					{
			...
			if (! $f)
			{
				throw new Exception("openJournal failed: ".implode(', ', $glob));
			}
		}
		...
		function closeJournal ()
		{
			...
			fclose($this->journalFlock);
			unlink($this->journalLock);
		}
