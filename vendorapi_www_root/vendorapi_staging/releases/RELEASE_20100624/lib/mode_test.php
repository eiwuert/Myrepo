...
class Mode_Test {
	...
	{
		//
		// Ermmmm... Regex has its own OR operator 
		// All of these calls can be reduced to 1 preg_match
		//
		if (preg_match('/^(rc|qa|aqa)\d*\./i', $_SERVER['SERVER_NAME'])
			|| preg_match('/^[^\.]+\.qa\./i', $_SERVER['SERVER_NAME'])
			|| preg_match('/^demo./i', $_SERVER['SERVER_NAME'])
			|| preg_match('/(?:wk\d*\.sellingsource\.local|\.dev\d\.clkonline\.com)$/i', $_SERVER['SERVER_NAME'])
