...
class Prpc_Client extends Prpc_Base
{
	...
	function _Prpc_Call ($call)
	{
		...
		{
			if ($this->die) die('WARN: fsockopen of '.$this->_prpc_url_host.' failed '. $errno. ' - '. $errstr ."\n");
			return FALSE;
		}
		...
		while ($num_bytes > 0)
		{
			...
			{
				if ($this->die) die('WARN: fwrite to '.$this->_prpc_url_host.' failed '."\n");
				return FALSE;
			}
			$num_bytes -= $rc;
		...
		if (! is_a ($rpc, 'Prpc_Message'))
		{
			 ...
			 {
			 	print_r ($pack); flush();
			 	die ("FATAL: Did not receive a Prpc_Message\n");
			 }
			 else return false;
		}
