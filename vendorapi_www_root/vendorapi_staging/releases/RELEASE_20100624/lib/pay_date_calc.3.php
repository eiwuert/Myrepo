...
class Pay_Date_Calc_3
{
	...
	{
		$this->setHolidayArray($holiday_array);
	...
	/**
	 * Sets the holiday array and verifies that it is in the correct format
	 * of Y-m-d.
	 *
	 * @param array|ECash_Models_Reference_List $holiday_array
	 * @return void
	 */
	public function setHolidayArray($holiday_array)
	{
		...
		{
			throw new InvalidArgumentException('Holiday list is not an array. Type passed in: ' . gettype($holiday_array));
		}
		...
		foreach ($holiday_array AS $holiday)
		{
			...
			{
				throw new InvalidArgumentException('Invalid holiday format: ' . $holiday);
			}
		}
	 ...
	 * @return array
	 */
	public function Calculate_Pay_Dates($model_name, $model_data, $direct_deposit = TRUE, $num_dates = 52, $start_date = "now", $forward = TRUE)
	{
		...
		while (count($this->pay_dates) < $num_dates)
		{
			...
			{
				//#17795 - First business day
				if(is_object($model_data) && $model_data->day_int_one == "33"
				   ||
				   !is_object($model_data) && isset($model_data['day_int_one']) && $model_data['day_int_one'] == "33"
				  )
				{
					while ( $this->Is_Holiday($mp) || $this->Is_Weekend($mp) )
					{
						$mp = strtotime("+1 day", $mp);
					}
				}
	...
	{
		//#17795
		if($this->model_data['day_int_one'] == "33")
		{
			$this->model_data['day_int_one'] = "1";
		}
	...
	private function Validate_Field($field)
	{
		...
		switch($field)
		{
			...
			case "day_int_one":
			case "day_int_two":
				if( !is_numeric($this->model_data[$field]) || $this->model_data[$field] < 1 || $this->model_data[$field] > 33 )
