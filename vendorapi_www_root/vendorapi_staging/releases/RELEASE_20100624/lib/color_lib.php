...
This library contains:
	...
	(int) norm ($suspect_int, $max_val=255, $min_val=0);
		returns an integer normalized to within the max and min parameters.

	(obj) fetch_color ($color_name, $image_obj=false, $image_alpha=false, $delta=false)
		when passed a valid color name, returns either a color object containing hex color
		strings and integer values for the base color, a shadow and hilight color, or if
		passed a gd object in the second argument allocates the color and returns an integer
		reference to it.

	(obj) fetch_color_by_hex ($hex_str, $image_obj=false, $image_alpha=false, $delta=false)
		when passed a valid color hex string returns exactly what fetch_color does.  (The image
		object part of this code is currently untested.)

	(arr) interpolate_color ($color_1, $color_2, $steps)
		returns an array containing hex string colors at regular $steps intervals
		along a line between the two colors in rgb color space

	(obj arr) fetch_color_interpolation ($color_name_1, $color_name_2, $steps, $image_obj=false, $image_alpha = false, $delta=false)
		works exactly like interpolate_color, but contains the same color objects as returned by
		the fetch_color and fetch_color_by_hex functions.  (The image object part of this code is
		currently untested)
*/

function norm ($suspect_int, $max_val=255, $min_val=0)
{
	$n = $suspect_int < $min_val ? $min_val : $suspect_int;
	return $n > $max_val ? $max_val : $n;
}
	...
	if (isset ($colors[$color_name]))
	{
		...
		if ($image_obj)
		{
			if (is_resource ($image_obj))
			{
				...
				if ($image_alpha)
				{
					$image_alpha = norm (intval ($image_alpha), 127);
...
// fetch_color_by_hex
// when given a valid hex color string, returns objects or integers exactly like fetch_color.  The "list"
// option in fetch_color has no meaning here, and will cause the function to return false.

function fetch_color_by_hex ($hex_str, $image_obj=false, $image_alpha = false, $delta=false)
{
	$c = new stdClass ();

	$c->hex = substr ($hex_str, 0, 1) == "#" ? $hex_str : "#".$hex_str;
	if (strlen ($c->hex) != 7)
	{
		return false;
	}

	$c->r = hexdec (substr ($c->hex, 1, 2));
	$c->g = hexdec (substr ($c->hex, 3, 2));
	$c->b = hexdec (substr ($c->hex, 5, 2));
...
function interpolate_color ($color_1, $color_2, $steps)
{
	...
	$interpolated_arr = array ($color_1);
	for ($i=1; $i<$steps-1;$i++)
	{
		$new_color = str_pad (dechex ($r1 + ($r_step * $i)), 2, "0", STR_PAD_LEFT);
		$new_color .= str_pad (dechex ($g1 + ($g_step * $i)), 2, "0", STR_PAD_LEFT);
		$new_color .= str_pad (dechex ($b1 + ($b_step * $i)), 2, "0", STR_PAD_LEFT);
		$interpolated_arr[] = strtoupper ($new_color);
	}
	$interpolated_arr[] = $color_2;

	return $interpolated_arr;
}

function fetch_color_interpolation ($color_name_1, $color_name_2, $steps, $image_obj=false, $image_alpha = false, $delta=false)
{
	$c1 = fetch_color ($color_name_1, $image_obj, $image_alpha, $delta);
	$c2 = fetch_color ($color_name_2, $image_obj, $image_alpha, $delta);

	$color_arr = interpolate_color ($c1->hex, $c2->hex, $steps);
	$color_arr[0] = $c1;
	for ($i=1; $i<$steps-1; $i++)
	{
		$color_arr[$i] = fetch_color_by_hex ($color_arr[$i], $image_obj, $image_alpha, $delta);
	}
	$color_arr[$steps-1] = $c2;
	return $color_arr;
}
