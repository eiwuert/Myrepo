...
class MySQL_4
{
	...
	* @desc Run the given query against the given database
	...
	public function Query($database, $query)
	{
        ...
        if ($this->debug)
        {
			$log = '/* '.__CLASS__.'::'.__FUNCTION__.'() called from '.__FILE__.', line '.__LINE__." */\n";;
	...
	* @desc Return the last inserted id for the given id
	 ...
	 * @param mixed		$column		the column index to pull, can be an index offset or associative index
	 * @return string				returns the data in the column or FALSE on failure
	 */
	public function Fetch_Column($result_id, $row, $column = 0)
	{
		return($result_id ? @mysql_result($result_id, (int)$row, $column) : FALSE);
	}
	...
	* @desc Return a list of databases
	...
	/**
	 * Pings the server connection to see if the connection is still valid.
	 *
	 * If $reconnect is TRUE, Ping will try and reconnect to the database using the connection parameters originally
	 * passed.
	 *
	 * @param bool $reconnect a boolean to reconnect the connection if ping fails
	 * @return bool Returns true on a good connection, false otherwise
	 */
	public function Ping($reconnect = FALSE)
	{
		$ping = mysql_ping($this->link_id);
		
		if (!$ping && $reconnect)
		{
			if (FALSE === ($this->link_id = mysql_connect($this->host, $this->user, $this->password, TRUE)))
			{
				throw new MySQL_Exception("database connection failed to {$this->host} as {$this->user}");
			}
	...
	public function escape($string)
	{
		if (!$this->link_id)
		{
			throw new MySQL_Exception('attempted to escape a string without having a connection');
		}
