...
*/
		function MySQL_1 ($read_host, $write_host, $login, $password, $database, $port = 3306, $trace_code = NULL)
		{
			...
			$this->Get_Tables ($trace_code."\t".__FILE__."->".__LINE__."\n");
...
*/
		...
		function Get_Tables ($trace_code = NULL)
		{
			$query = "show tables";
			$this->Query ($query, "", $trace_code."\t".__FILE__." -> ".__LINE__."\n");
...
*/
		...
		{
			$err_from = "errors@sellingsource.com";
			$err_to = "errors@sellingsource.com";
			$err_subject = "A database error has occured on ".$SERVER["SERVER_NAME"];
			$err_send = FALSE;
			$err_show = TRUE;

			// Generate an error report and then act according to configuration
			$err_text = "An error has been generated by the server: ".$_SERVER["SERVER_NAME"].".\n".
			...
			if ($err_show)
			{
				echo "<html><head><title>Server Error</title></head><body><pre>".$err_text."</pre></body></html>";
			}
			else
			{
				echo "<html><head><title>Server Error</title></head><body><pre>An error has occured on the server.  Please use your back button to try again.</pre></body></html>";
			}
