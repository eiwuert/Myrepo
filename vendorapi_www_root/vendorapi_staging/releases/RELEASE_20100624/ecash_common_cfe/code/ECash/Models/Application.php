...
class ECash_Models_Application extends ECash_Models_ObservableWritableModel implements ECash_Models_ICustomerFriend
{
	...
	public function loadLegacyAll($application_id, &$response)
	{
		...
		/**
		 * The fetchAll call gets everything and caches it.
		 *
		 * We'll grab the individual bits out through the separate calls
		 * that get data from the cache.
		 */
		//$app_client->fetchAll($application_id);
	...
	protected function getNativeColumnData($application_id)
	{
		...
		$row['income_monthly'] = floor($employment_info->income_monthly);
		...
		$row['page_id'] = 0;
		$row['campaign_info'] = array(
			array(
				'promo_id' => $applicant_info->promo_id,
				'promo_sub_code' => $applicant_info->promo_sub_code,
	...
	protected function getNativeColumnData_ldb($application_id)
	{
		...
		$row['esig_ip_address'] = empty($application_info["esig_ip_address"]) ? NULL : $application_info["esig_ip_address"];
