<?php
	...
	{
		const INDEX_REGISTER = 'transaction_register';
	   	...
	   	public function __construct(DB_IConnection_1 $db)
		{
			...
			$this->transaction = ECash::getFactory()->getModel('TransactionRegister');
		...
		public function createInstance(array $db_row)
        {
			...
			return array(
				self::INDEX_REGISTER => $transaction,
		...
		public function loadBy(array $where_args = array())
		{
			...
			$query = '
				...
				left JOIN transaction_ledger on (transaction_ledger.transaction_register_id = transaction_register.transaction_register_id)
				' . self::buildWhere($where_args, FALSE) . ' ORDER BY transaction_register.transaction_register_id';
