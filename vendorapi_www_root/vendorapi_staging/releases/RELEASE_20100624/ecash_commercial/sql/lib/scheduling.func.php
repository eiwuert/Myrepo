...
function Add_Paydown($application_id, $request)
{
	...
	$paydown = Schedule_Event::MakeEvent($action_date, $due_date, $amounts, 'paydown', $comment,'scheduled','paydown');
...
function Set_Refund($application_id, $request, $schedule)
{
	...
	if ($amount != 0.00)
	{
		...
		$e = Schedule_Event::MakeEvent($today, $next_day, $amounts, 'refund', $description, null, 'manual');
		$evids[] = Record_Event($application_id, $e);
	}
...
function Set_Chargeback($application_id, $request, $schedule)
{
	...
	$e = Schedule_Event::MakeEvent($today, $today, $amounts, $request->action, $description, 'registered','manual'); //mantis:4337 - add 'registered','manual'
...
function Analyze_Schedule($schedule, $verify=false, $rules=NULL)
{
	...
	foreach ($schedule as $e)
	{
		...
		if ($e->type != 'cancel') $non_cancel_trans = true;
		...
		if($e->type === 'repayment_principal' && $e->context === 'manual') $status->has_manual_renewals = TRUE;
			...
			case 'failed':
				{
					...
					//add check if fatal is on current bank acct

					if(isset($e->ach_return_code_id) && $failure_map[$e->ach_return_code_id]['is_fatal'] === 'yes' && $e->bank_aba == $e->current_bank_aba && $e->bank_account == $e->current_bank_account) $status->num_fatal_failures++;
...
function Update_Schedule($application_id, $schedule, $made_arrangement = false)
{
	...
	if($schedule != NULL)
	{
		...
		foreach ($schedule as $event)
		{
			if(!$arrangement_set && $made_arrangement && $event->type == 'repayment_principal' && ECash::getConfig()->HANDLE_SHIFTS_AS_ARRANGEMENTS)
			{
	...
	foreach($current_schedule as $e)
	{
		...
		if ($e->status === 'scheduled')
		{
			...
			elseif((empty($e->origin_id)) && (!in_array($e->type, $ignored_events)) && !$e->is_shifted)
			{
				...
				if( $e->event_type == 'payout' || $e->context == 'generated')
					continue;
			...
			elseif($e->type === 'repayment_principal' && $e->context === 'manual')
			{
				$e->REASON = 'D';
				$classifications->special_payments[] = $e;
			}
			...
			if(((! empty($e->origin_id)) || ($e->type == 'assess_fee_ach_fail') || ($e->type == 'payment_fee_ach_fail')) && empty($preview_schedule))
			{
...
$origin_group_id, $date_event, $date_effective, $trace_data="", $context, $source_name = null, $is_shifted = false)
{
	...
	try
	{
		...
		if($context == "arrangement" || $context == "partial")
		{
...
function Record_Current_Scheduled_Events_To_Register ($date_current, $application_id=NULL, $event_schedule_id=NULL, $mode='all', $do_not_use=NULL)
{
	/*
	...
	*/
	...
	$ach_event_types = Load_ACH_Event_Types();
	$reverse_map = Load_Reverse_Map($company_id);
	...
	if ( !in_array($mode, array('all','ach','adjustment', 'external', 'accrued charge','landmark')))
	{
	...
	try
	{
		...
		foreach($query_rows as $row1)
		{
			...
			if ($row1->event_schedule_id != $event_schedule_id_prev)
			{
				...
				if (($row1->context == 'arrangement' || $row1->context == 'partial') && $row1->clearing_type !== 'ach')
				{
...
function Remove_And_Suspend_Events_From_Schedule($application_id)
{
	...
	try
	{
		...
		$sql = '-- /* SQL LOCATED IN file=' . __FILE__ . ' line=' . __LINE__ . ' method=' . __METHOD__ . " */
				...
				AND et.name_short NOT IN ('paydown','payout')
