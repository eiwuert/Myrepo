...
class eCash_AgentAffiliation_Legacy
{
	...
	private static function createMaxDateCreatedsFromApplicationServiceTempTable(array $application_ids)
	{
		...
		{
			$application_list = "";
			$insert_list = array_chunk($application_list, self::INSERT_LIMIT);
			foreach($insert_list as $reduced_list)
			{
				foreach($reduced_list as $application_id)
				{
					$application_list .= "$application_id,";
				}
			}
			unset($insert_list);

			$query = "EXECECUTE sp_fetch_last_date_of_status_history (".substr($application_list,0,-1)."); ";
