		...
		{	    
			...
			{				
				try 
				{
					...
					{
						$progress->update('Sending Combined batch', 60);
					...
					{
						$progress->update('Sending Credit batch', 50);
						// Already initialized from its initial __construct
						...
						$progress->update('Sending Debit batch', 60);
					...
					$progress->update('Account Statuses have been update', 90);
					...
					{
						$progress->update('Creating Daily Cash Report', 97);
	...
	public function Status_Approved_Move_To_Active()
	{
		 try
		 {
			...
			$query = "
					...
					WHERE application_id IN (".implode(',', array_keys($application_list)) .")
