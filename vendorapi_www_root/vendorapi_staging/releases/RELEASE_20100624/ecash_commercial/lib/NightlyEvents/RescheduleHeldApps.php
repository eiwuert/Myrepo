	...
	class ECash_NightlyEvent_RescheduleHeldApps extends ECash_Nightly_Event
	{
		...
		private function fetchApplicationDataByStatusNotIn(array $statuses)
		{
			...
			$query = "
				...
				SET @sqlQuery = N'
					SELECT a.authoritative_id application_id
					FROM application a INNER JOIN application_status aps ON a.application_status_id = aps.application_status_id
					WHERE aps.application_status_name NOT IN ($statuses) AND is_watched = 1;';
