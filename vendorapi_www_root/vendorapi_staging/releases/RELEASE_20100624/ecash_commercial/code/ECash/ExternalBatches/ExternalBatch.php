	...
	public function preprocess($limit = NULL)
	{
		...
		if(!empty($application_list))
		{
			$query = "
				SELECT
					es.application_id,
				    sum(if(es.event_status = 'scheduled', 1, 0)) as 'events',
	    			count(tr.transaction_register_id) as 'transactions',
    				count(ec.ext_collections_id) as 'external_batches'
				FROM
					event_schedule es 
				LEFT JOIN
					ext_collections ec ON es.application_id = ec.application_id 
				LEFT JOIN
					transaction_register tr ON tr.application_id = es.application_id AND tr.transaction_status = 'pending'
				WHERE
					es.application_id in ({$application_list})
				GROUP BY es.application_id
				HAVING events = 0 AND transactions = 0 AND external_batches = 0
			";
