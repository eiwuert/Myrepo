...
class ECash_VendorAPI_ApplicationFactory implements VendorAPI_IApplicationFactory
{
	...
	{
		$query = "SELECT MAX(application_id) AS application_id FROM application WHERE ssn = '".$ssn."' AND dob = '".$dob."';";
		$st = $this->db->prepare($query);
		$st->execute();
		$result = $st->fetch(PDO::FETCH_ASSOC);

		$version_model = $this->getApplicationVersionModel($result['application_id']);
		$persistor->setVersion($version_model->version);

		$app = $persistor->loadBy(
			$this->getApplicationModel(),
			array('application_id' => $result['application_id'],
	...
	protected function getQualify(ECash_Models_Application $app)
	{
        ...
        if ($app->olp_process == 'ecashapp_react') {
            if ($application_data->getNumberActiveApplicationsBySSN($app->ssn, $app->company_id)>0) {
                $num_paid_apps = $num_paid_apps + 1;
            }
        }
		...
		$qualify = $this->driver->getQualify($app->loan_type_id);
	...
	public function populateServerIp($application_id)
	{
		$config = new VendorAPI_Blackbox_Config();
		if ($config->run_tribal)
		{
			$tribal_url = ECash::getConfig()->TRIBAL_URL;
			$server_ip = gethostbyname($tribal_url);
			...
			*/
			...
			$app_server_model = $this->factory->getModel('ApplicationServer', $this->db, FALSE);
			$app_server_model->date_created = date("Y-m-d H:i:s", time());
			$app_server_model->application_id = $application_id;
			$app_server_model->server_ip = $server_ip;
			$app_server_model->setInsertMode(DB_Models_WritableModel_1::INSERT_STANDARD);
			$app_server_model->insert();
		}
	}
}
