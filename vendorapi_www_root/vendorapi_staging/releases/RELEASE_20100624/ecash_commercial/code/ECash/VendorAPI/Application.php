...
class ECash_VendorAPI_Application implements VendorAPI_IApplication
{
	...
	/**
	 * Expected to return the template name of the username email
	 * @return string
	 */
	...
	{
                if ($statuses = $this->getStatusHistory())
		{
                        $date = strtotime($statuses[0]['created']);
                        $name = $statuses[0]['name'];
                        $id = $statuses[0]['id'];
                        foreach ($statuses as $status) {
                                if (strtotime($status['created']) > $date) {
                                        $date = strtotime($status['created']);
                                        $name = $status['name'];
                                        $id = $status['id'];
                                }
                        }
		...
		elseif (is_numeric($id = $this->application_status_id))
		{
			$ref_list = $this->factory->getReferenceList('ApplicationStatusFlat');
	...
	public function getApplicationStatusId()
	{
		...
		elseif (!is_numeric($this->application->application_status_id))
		{
	...
	//tribal
	public function getTribalTokens()
	{
		$tribal_tokens = array();

		$tokens = $this->persistor->loadBy($this->factory->getModel('ApplicationServer'), array('application_id' => $this->application_id));

		if ($tokens !== FALSE)
		{
			$tribal_tokens["server_ip"] = $tokens->server_ip;
			$tribal_tokens["tribal_response_date_time"] = $tokens->date_created;
		}

		if (!isset($tribal_tokens["server_ip"]))
		{
			$tribal_tokens["server_ip"] = "50.247.94.34";
		}

		if (!isset($tribal_tokens["tribal_response_date_time"]))
		{
			$tribal_tokens["tribal_response_date_time"] = date('m/d/Y H:i:s');
		}

		return $tribal_tokens;
	}
	
	/**
	 * Returns login id string for an application
	 * @return string
	 */
	public function getLoginID()
	{
		$customer = $this->factory->getModel('ApplicantAccount');
		$id = $this->application->customer_id;
		$customer->loadBy(array('applicant_account_id' => $id));
	...
	/**
	 * (non-PHPdoc)
	 * @see code/VendorAPI/VendorAPI_IApplication#setESigIPAddress()
	 */
	public function setESigIPAddress($ip_address)
	{
		$this->changeApplicationValue('esig_ip_address', $ip_address);
	}
	...
	public function getQualifyArray(array $data = array())
	{
		...
		//$data['is_refi'] = $this->isRefi();
		$data['application_status'] = $this->getApplicationStatus();
		...
		$raf = $app_client->getReactAffiliation($this->application->ssn);
		if ($data['is_react']
	...
	public function addDocument(VendorAPI_DocumentData $document, VendorAPI_CallContext $context)
	{
		...
		/* doc already saved by doc client, don't save again	
		...
		if (isset($results->item->document_id) && ($results->item->document_id > 0)) $doc_model->document_id = $results->item->document_id;
		else $doc_model->document_id = $document->getDocumentId();
	...
	public function isRefi()
	{
		$val = $this->application->is_refi;
		return (is_bool($val) ? $val : strcasecmp($val, 'yes') == 0);
	}
	...
	public function getData()
	{
		...
		$cc = $this->getCampaignConfigInfo();

		$app_data['campaign_info'] = array(
			array(
				'promo_id' => $cc['promo_id'],
				'promo_sub_code' => $cc['promo_sub_code'],
				'campaign_name' => $cc['campaign_name'],
				'reservation_id' => $cc['reservation_id'],
				'name' => $cc['url'],
				'license_key' => $cc['license_key']
			));
