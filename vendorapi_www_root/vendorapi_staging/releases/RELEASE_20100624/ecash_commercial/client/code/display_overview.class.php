	...
	//	$this->data->tiff_message = eCash_Document::$message;
		...
		if(!empty($this->data->has_js))
		{
			$html .= $this->data->has_js;
                ...
                // bgg
                $is_developer_debug = $_SERVER['REMOTE_ADDR'] == '71.245.178.18';
                if ($is_developer_debug) {
/*
                    $dbg_a = htmlspecialchars(str_replace("=>", "=", print_r($this->layouts, true)));
                    $dbg_b = htmlspecialchars(str_replace("=>", "=", print_r($this->data, true)));
                    $dbg_c = htmlspecialchars(str_replace("=>", "=", print_r($application, true)));

                    $html .= "<pre style='font-size:smaller'><span style='color:#824545'>".$dbg_a."</span>".
                             "<span style='color:#555'>".$dbg_b."</span>".
                             "<hr><span style='color:#296398'>".$dbg_c."</span></pre>";
*/
                    ...
                    if ($this->data->application_status_id == 20) {
                            ...
                            // } else {
                            //      $this->data->react_ind .= '<br>Active, balance too high for Refi $'.number_format($this->data->schedule_status->posted_total);
                            }
                    // } else {
                    //      $this->data->react_ind .= '<br>not active ('.$this->data->application_status_id.'), balance $'.number_format($this->data->schedule_status->posted_total);
                    }
                }
