...
//			$this->data->posted_total = $this->data->schedule_status->posted_total;
		...
		case "fraud_risk_rules":
			...
			{
				$data->fraud_rules_list = addslashes(join("<br>", split(";", $data->fraud_rules)));
			}
			...
			{
				$data->risk_rules_list = addslashes(join("<br>", split(";", $data->risk_rules)));
			}
	...
	protected function Render_Transaction_Details()
	{
		...
		if (isset($e->transaction_register_id))
		{
			if (($history = Get_Transaction_History($e->transaction_register_id)) && $e->transaction_status != 'new')
			{
				...
				$history_str .= "<td>{$agent_name}</td></tr>";
				
				while ($row = $history->fetch(PDO::FETCH_OBJ)) 
				{
	        		 $history_str .= '<tr><td>' . ucfirst($row->status_before) . '</td>';
	        		 $history_str .= '<td>' . ucfirst($row->status_after) . '</td>';
	        		 $history_str .= '<td>' . date('m/d/Y H:i:s', strtotime($row->date_created)) . '</td>';
	        		 $history_str .= '<td>' . $agent_name . '</td></tr>';
	    		}
	    		$history_str .= '</table>';
	    		$history->closeCursor();
