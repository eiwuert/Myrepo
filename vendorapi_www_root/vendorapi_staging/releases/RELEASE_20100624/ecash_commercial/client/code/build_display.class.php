...
class Build_Display
{
	...
	public function __construct(stdClass &$data, &$acl, $mode, $module_name, &$data_format)
	{
		...
		$this->build_id_credit = new Build_ID_Credit($acl, $data->company_id, $this->data, new ECash_DataX_Responses_Generic());
	...
	public function Build_Loan_Actions($loan_actions)
	{
		...
		if(count($loan_actions))
		{
			foreach($loan_actions as $row)
			{
				...
				$html .= "<span class=\"{$td_class}\" style=\"text-align:left; width: 70px; overflow: hidden;\" onMouseOver=\"tooltip(event, '" . addslashes($row->agent_name) . "', null, -30);\" onMouseOut=\"tooltip(null);\"><nobr>{$row->agent_name_short}</nobr></span>";
	...
	public function Build_Receive_Document_List($document_list, $sent_documents)
	{
		...
		foreach($document_list as $doc)
		{
			$row = $doc->getModel();
				...
				$html .= "</nobr></div></td><td class=\"{$td_class}\">";
				$html .= "<input type=\"checkbox\" name=\"document_list[{$id}]\" value=\"{$row->name_short}\" onClick=\"doc_send_recv_ot.toggle(this); \">";
				$html .= "</td></tr>\n";
				$alt = !$alt;
		}
		$html .= "</table>\n";
