...
class Build_ID_Credit
{
	 ...
	 * Object Constructor
	 */
	public function __construct($acl, $company_id, stdClass &$data, ECash_DataX_IResponse $response)
	{
		$this->acl = $acl;
	...
	private function getBucketErrors()
	{
		...
		if(empty($connection_error))
		{
			...
			$this->response->parseXML($newest_record->received_package);
			
			$buckets 		 = $this->response->getDecisionBuckets();
	...
	private function buildDataJavascript($inquiry_packages)
	{
		...
		if(!empty($inquiry_packages))
		{
			...
			{
				$this->response->parseXML($package->received_package);
				...
				$decision   = $this->response->getDecision();
