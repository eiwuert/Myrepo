	...
	public function __construct(ECash_Transport $transport, $module_name)
	{
		...
		*/
		...
		$this->download_file_name = preg_replace("/\s/", "_", $this->report_title) .
			date('Ymd', strtotime($this->search_criteria['specific_date'])) . '.xls';
	}
	...
	public function Get_Module_HTML_Data()
	{
		...
		while (!is_null($next_level = ECash::getTransport()->Get_Next_Level()))
		{
			if($this->dl_ready && $next_level == 'message')
			{
				...
				$substitutions->download_link = "[ <a href=\"?module=reporting&mode=" . urlencode($mode) . "&action=download_report\" class=\"download\">Download Data to Excel</a> ]";
				//add message
