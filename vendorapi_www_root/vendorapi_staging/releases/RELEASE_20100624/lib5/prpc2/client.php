...
class Prpc_Client2 extends Prpc_Base2
{
	...
	function _Prpc_Set_Url ($url = NULL)
	{
		...
		{
			return $this->dieOrSetError('FATAL: Invalid url ('.$url.")");
		}
	...
	function _Prpc_Call ($call)
	{
		...
		if (! preg_match ('/^HTTP\/1\.\d (\d+) (.*)/', $http_response_head, $m))
		{
			...
			if($stm_data['timed_out'])
			{
				return $this->dieOrSetError("FATAL: Socket Timeout from ".$this->_prpc_url);
			}
			return $this->dieOrSetError("FATAL: No HTTP response header from ".$this->_prpc_url."\nGot ".$http_response_head);
		}
		...
		{
			return $this->dieOrSetError("FATAL: Bad HTTP response ".$http_response_code." - ".$http_response_msg."\n".$pack);
		}
		...
		if (! is_object ($rpc))
		{
			 return $this->dieOrSetError("FATAL: Did not recieve an object. pack:".print_r($pack, TRUE));
		}
		...
		else
		{
			...
			if ($rpc instanceof SourcePro_Prpc_Message_Except)
			{
	...
	private function dieOrSetError($error)
	{
		if ($this->die) die($error."\n");
