...
/**
 * Basic FactorTrust Under Writer rule
 *
 * This does not implement adverse actions... Check the adverse action
 * observers (@see VendorAPI_Blackbox_FactorTrust_AdverseActionObserver and
 * children) for those.
 *
 ...
 * prevents making multiple FactorTrust calls for companies that have multiple
 * blackbox campaigns (like NSC). Likewise, it stores the FactorTrust track hash
 ...
 * decision is cached, this rule will never make a second call to FactorTrust. If
 * the track  hash is reused, FactorTrust will NOT rerun the IDV portion of
 * the performance call and will immediately fail the application.
 *
 * @author Andrew Minerd <andrew.minerd@sellingsource.com>
 */
class VendorAPI_Blackbox_Rule_FactorTrust extends VendorAPI_Blackbox_Rule
{
	const EVENT_ON_CALL = "onCall";

	/**
	 * @var FactorTrust_UW_Call
	 */
	protected $call;

	/**
	 * @var array VendorAPI_Blackbox_FactorTrust_ICallObserver[]
	 */
	 ...
	 * @return void
	 */
	public function __construct(VendorAPI_Blackbox_EventLog $log, FactorTrust_UW_Call $call, $rework = FALSE, $cache_decision = TRUE)
	{
	...
	{
		return new VendorAPI_Blackbox_FailureReason("FactorTrust UW", "FACTORTRUST UW FAILED!");
	}

	/**
	 * Attach a new observer to this object
	 *
	 * @param VendorAPI_Blackbox_FactorTrust_ICallObserver $observer
	 * @return void
	 */
	public function attachObserver(VendorAPI_Blackbox_FactorTrust_ICallObserver $observer)
	{
	 ...
	 * @return void
	 */
	public function detachObserver(VendorAPI_Blackbox_FactorTrust_ICallObserver $observer)
	{
	 ...
	 *
	 * @param string $event
	 * @param FactorTrust_UW_Result $result
	 * @param Blackbox_IStateData $state
	 * @param Blackbox_Data $data
	 * @return void
	 */
	public function notifyObservers($event, FactorTrust_UW_Result $result, Blackbox_IStateData $state, VendorAPI_Blackbox_Data $data)
	{
	...
	public function isValid(Blackbox_Data $data, Blackbox_IStateData $state)
	{
		...
		catch (TSS_FactorTrust_TransportException $e)
		{
			return ($this->onTransportError($e, $data, $state) === TRUE);
		}
		catch (TSS_FactorTrust_ProviderException $e)
		{
	...
	protected function onInvalid(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		{
			// reset the track hash to force FactorTrust to rerun IDV
            $state_data->uw_provider = 'factortrust';
			$state_data->uw_track_hash = NULL;
			$state_data->uw_decision = NULL;

			throw new VendorAPI_Blackbox_FactorTrust_ReworkException(
				'IDV call failed',
				$this->response
			);
		}
	...
	protected function onTransportError(Exception $e, Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		$state_data->loan_actions->addLoanAction('FACTORTRUST_PERF');
	...
	protected function runRule(Blackbox_Data $data, Blackbox_IStateData $state)
	{
		...
		if (isset($state->uw_call_history) && isset($state->uw_call_history[1]))
		{
			$hist = $state->uw_call_history[1];
		...
		if ($this->response->hasError())
		{
			...
			{
				throw new FactorTrust_UW_ProviderException(
			...
			{
				throw new FactorTrust_UW_TransportException(
					$this->response->getErrorMsg(),
		...
		// HMS/RRV checks to see if FactorTrust says they can get a loan amount increase
		if ($this->response instanceof FactorTrust_UW_ILoanAmountResponse)
		{
			$state->loan_amount_decision = $this->response->getLoanAmountDecision();
		}
		if ($this->response instanceof FactorTrust_UW_IAutoFundResponse)
		{
	...
	/**
	 * Builds the array for the FactorTrust request
	 * @param Blackbox_Data $data
	 * @return array
	 */
