...
/**
 * Basic Clarity Under Writer rule
 *
 * This does not implement adverse actions... Check the adverse action
 * observers (@see VendorAPI_Blackbox_Clarity_AdverseActionObserver and
 * children) for those.
 *
 ...
 * prevents making multiple Clarity calls for companies that have multiple
 * blackbox campaigns (like NSC). Likewise, it stores the Clarity track hash
 ...
 * decision is cached, this rule will never make a second call to Clarity. If
 * the track  hash is reused, Clarity will NOT rerun the IDV portion of
 * the performance call and will immediately fail the application.
 *
 * @author Andrew Minerd <andrew.minerd@sellingsource.com>
 */
class VendorAPI_Blackbox_Rule_Clarity extends VendorAPI_Blackbox_Rule
{
	...
	/**
	 * @var array VendorAPI_Blackbox_Clarity_ICallObserver[]
	 */
	 ...
	 * @return void
	 */
	public function __construct(VendorAPI_Blackbox_EventLog $log, Clarity_UW_Call $call, $rework = FALSE, $cache_decision = TRUE)
	{
	...
	{
		return new VendorAPI_Blackbox_FailureReason("Clarity UW", "CLARITY UW FAILED!");
	}

	/**
	 * Attach a new observer to this object
	 *
	 * @param VendorAPI_Blackbox_Clarity_ICallObserver $observer
	 * @return void
	 */
	public function attachObserver(VendorAPI_Blackbox_Clarity_ICallObserver $observer)
	{
	 ...
	 * @return void
	 */
	public function detachObserver(VendorAPI_Blackbox_Clarity_ICallObserver $observer)
	{
	 ...
	 *
	 * @param string $event
	 * @param Clarity_UW_Result $result
	 * @param Blackbox_IStateData $state
	 * @param Blackbox_Data $data
	 * @return void
	 */
	public function notifyObservers($event, Clarity_UW_Result $result, Blackbox_IStateData $state, VendorAPI_Blackbox_Data $data)
	{
	...
	public function isValid(Blackbox_Data $data, Blackbox_IStateData $state)
	{
		...
		catch (TSS_Clarity_TransportException $e)
		{
			return ($this->onTransportError($e, $data, $state) === TRUE);
		}
		catch (TSS_Clarity_ProviderException $e)
		{
	...
	protected function onInvalid(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		{
			// reset the track hash to force Clarity to rerun IDV
            $state_data->uw_provider = 'clarity';
			$state_data->uw_track_hash = NULL;
			$state_data->uw_decision = NULL;

			throw new VendorAPI_Blackbox_Clarity_ReworkException(
				'IDV call failed',
				$this->response
			);
		}
	...
	protected function onTransportError(Exception $e, Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		$state_data->loan_actions->addLoanAction('CLARITY_PERF');
	...
	protected function runRule(Blackbox_Data $data, Blackbox_IStateData $state)
	{
		...
		if (isset($state->uw_call_history) && isset($state->uw_call_history[2]))
		{
			$hist = $state->uw_call_history[2];
		...
		if ($this->response->hasError())
		{
			...
			{
				throw new Clarity_UW_ProviderException(
			...
			{
				throw new Clarity_UW_TransportException(
					$this->response->getErrorMsg(),
		...
		// HMS/RRV checks to see if Clarity says they can get a loan amount increase
		if ($this->response instanceof Clarity_UW_ILoanAmountResponse)
		{
			$state->loan_amount_decision = $this->response->getLoanAmountDecision();
		}
		if ($this->response instanceof Clarity_UW_IAutoFundResponse)
		{
	...
	/**
	 * Builds the array for the Clarity request
	 * @param Blackbox_Data $data
	 * @return array
	 */
