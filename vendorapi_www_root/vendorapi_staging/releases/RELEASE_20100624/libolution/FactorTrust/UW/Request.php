...
class FactorTrust_UW_Request implements FactorTrust_UW_IRequest
{
	...
	protected static $function_map = array(
		...
		'DateOfBirth'        => array('FTformatDate', 'dob'),
		'RequestedDueDate'   => array('FTformatDate', 'date_first_payment')
    );
	 ...
	 */
	public function __construct($inquiry, $store_id)
	{
		if (empty($store_id))
		{
		...
		if (empty($inquiry))
		{
			throw new InvalidArgumentException('empty factor trust inquiry passed');
		}
	...
	public function transformData(array $data)
	{
		...
		$auth->appendChild($dom->createElement('StoreIdentifier', $this->store));
                $auth->appendChild($dom->createElement('ProductType', 'PER'));
    ...
    {
        return date('m/d/Y',time() + (24 * 60 * 60));
    }
    
	protected function FTformatDate($date_str)
    {
