...
abstract class Stats_StatPro_ClientBase_1 //extends Object_1
{
	...
	public function setStatproKey($value)
	{
		...
		{
			throw new Exception("Invalid StatPro key ({$value})");
		}
	...
	protected function openJournal()
	{
		...
		if (!is_array($glob) || count($glob) == 0)
		{
			...
			$glob = array($journal_path."journal_".sprintf("%06d", mt_rand(0, 9999)).".db");
		}
		...
		for ($i = 0, $found = FALSE; $i < self::MAX_JOURNAL_ATTEMPTS && !$found; $i++)
		{
			...
			for ($j = 0; $j < count($glob) && !$found; $j++)
			{
				...
				if (!(file_exists($this->journal_lock_file) || file_exists($journal_name . '-journal'))
					&& (!file_exists($journal_name) || is_writable($journal_name)))
				{
			...
			if (!$found)
			{
				...
				array_unshift($glob, $journal_path."journal_".sprintf("%06d", mt_rand(0, 9999)).".db");
