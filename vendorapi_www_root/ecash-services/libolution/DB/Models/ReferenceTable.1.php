...
class DB_Models_ReferenceTable_1 implements DB_Models_IReferenceTable_1, Iterator, ArrayAccess
{
	...
	{
		// allow people to add things to the reference table,
		// in case they've been keeping it around
		if ($index === NULL
			&& $value instanceof $this->empty
			&& $value->isStored()
			&& !$value->isAltered())
		{
			$name = $value->{$this->getColumnName()};
			$id = $value->{$this->getColumnID()};

			if (!isset($this->name_map[$name])
				&& !isset($this->id_map[$id]))
			{
				$this->fromRow($value->getColumnData());
				return;
			}
		}
