...
abstract class Trigger
{
	...
	private static $nullity_exception_columns = array('name_last', 'name_first', 'employer_name');
	...
	public function Escape_Value($column, $value)
	{
		...
		$nullable	 = $_SESSION['table_trigger'][$this->table]->$column->Null;
		
		if(	strtoupper($value) == "NULL" && 
			!in_array($column, self::$nullity_exception_columns) && 
			strtoupper($nullable) == "YES"
		  )
			return $value;
	...
	public function Status_History_ID()
	{
	...
	private function Add_Status_History($old_info = NULL)
	{
			...
			$agent_different) )
		{
			...
			$this->mysqli->Query($query);
			$this->status_history_id = $this->mysqli->Insert_ID();
			...
			if(array_search($column_name, $this->audit) !== FALSE)
			{
				...
				if(!$old_row || $old_row->$column_name != $new_row->value)
				{
					$values[] = "(now(),
								...
								'{$this->table}',
								'{$column_name}',
								'" . $this->mysqli->Escape_String($old_row->$column_name) . "',
								{$new_value},
...
class Card_Trigger extends Trigger
{
	public function __construct(MySQLi_1 $mysqli, $agent = NULL, $add_no_duplicate_status = NULL)
	{
		parent::__construct($mysqli, 'card', $agent, $add_no_duplicate_status);
		//if the array below is not empty, $agent is not optional!
		$this->audit = array();
	}
}
