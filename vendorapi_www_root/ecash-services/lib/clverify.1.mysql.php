<?php
	...
	*/
	class CLVerify_1_Mysql extends CLVerify_1
	{
		function CLVerify_1_Mysql (&$sql, $base, $live_mode = FALSE)
		{
		...
		function Get_Record ($application_id, $source_id = 1)
		{
			$fetch_exist = "
				SELECT
					date_modified, date_created, sent_package, received_package, score
				FROM
		...
		function _Update($xml, $display_data, $authentication, $source_id, $authentication_id, $customer_id)
		{
				...
				$query = "
					UPDATE
						authentication
					SET
						sent_package = '".mysql_escape_string($xml["sent"])."',
