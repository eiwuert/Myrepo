...
abstract class Blackbox_Rule implements Blackbox_IRule
{
	...
	/**
	 * Event that's run when the rule is skipped (canRun returns false)
	 *
	 ...
	 * onValid will not be called.
	 *
	 * @param Exception $e exception that caused this onError to run.
	...
	public function isValid(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		if ($this->checkSkipConditions() && $this->canRun($data, $state_data))
		{
			try
			{
	...
	/**
	 * Used to check rule conditions that may cause a rule skip when needed
	 *
	 * @return bool
	 */
	public function checkSkipConditions()
	{
		return TRUE;
	}
}
