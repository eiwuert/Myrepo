...
class Blackbox_TargetCollection implements Blackbox_ITarget
{
	...
	public function pickTarget(Blackbox_Data $data)
	{
		...
		$keys = array_keys($this->valid_list);
		
		for ($k = 0; $k < $count && $winner === FALSE; $k++)
		{
			// make sure we've not gone invalid by events sent on an event bus
			if ($this->isInvalidated())
			{
				return FALSE;
			}
			
			$target = $this->valid_list[$keys[$k]];
			$winner = $target->pickTarget($data);
			
			if ($winner === FALSE)
			{
				unset($this->valid_list[$keys[$k]]);
			}
		}
	...
	public function isValid(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		if ($this->rules instanceof Blackbox_IRule)
		{
			...
			$this->setValid($this->rules->isValid($data, $this->state_data));
		...
		if (!$this->isInvalidated())
		{
			...
			{
				// child classes might have caught an event that invalidated $this
				if ($this->isInvalidated())
				{
					$this->valid_list = array();
					break;
				}
				elseif ($target->isValid($data, $this->state_data))
				{
			...
			$this->setValid(!empty($this->valid_list));
	...
	/**
	 * Try to avoid "race conditions" with setting valid on this collection.
	 * 
	 * This avoids the situation where we say $this->valid = x() where x() might
	 * be a function/method which sends an event on an event bus, which $this
	 * target collection catches, invalidates itself, then x() returns TRUE
	 * and the invalid state in $this we wanted to preserve is now overridden
	 * with TRUE from x().
	 * 
	...
	/**
	 * Determines whether this rule has been explicitly invalidated.
	 *
	 * @return bool
	 */
	...
	{
		//incase of root target collection, the rules would be assigned twice
		//1. the rules that are associated with the target collectio [read from database]
		//2. inside code/OLPBlackbox/Factory/OLPBlackbox manually for generic ones
		
		//So in this case the later would be tucked under the primary ones.
		if ($this instanceof OLPBlackbox_OrderedCollection && !empty($this->rules))
		{
			foreach ($rules as $rule) $this->rules->addRule($rule);
		}
