 ...
 */
class OLPBlackbox_Rule_Suppression_ExcludeTest extends OLPBlackbox_Rule_Suppression_Base
{
	/**
	...
	{
		// set up special state data since we will test failure reasons here.
		...
		$list->expects($this->any())
			->method('Match')
			->will($this->returnValue($list_return)
		);
		
		$rule = $this->getMock(
			'OLPBlackbox_Rule_Suppression_Exclude', 
		...
		// if this test fails, double check a failure reason is set.
		if (!$valid)
		{
