<?php
/** PHPUnit test class for the DataX_Parser class.
 *
 * @author Ryan Murphy <ryan.murphy@sellingsource.com>
 */
class DataX_ParserTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$dom = new DataX_Parser($this->getXML('ERROR'));
	...
	{
		$dom = new DataX_Parser($this->getXML('INVALID'));
	...
	{
		$dom = new DataX_Parser($this->getXML(''));
	...
	{
		$dom = new DataX_Parser($this->getXML('SENT'));
	...
	{
		$dom = new DataX_Parser($this->getXML('IMPACT-IDVE'));
	...
	{
		$dom = new DataX_Parser($this->getXML('IMPACTPDL-IDVE'));
	...
	{
		$dom = new DataX_Parser($this->getXML('PERF-L3'));
	...
	{
		$dom = new DataX_Parser($this->getXML('AALM-PERF'));
	...
	/** Checks the CLK-PERF packet.
	 *
	 * @return void
	 */
	public function testClkPerf()
	{
		$dom = new DataX_Parser($this->getXML('CLK-PERF'));
		
		$this->assertTrue($dom->isValid());
		$this->assertNull($dom->getError());
		$this->assertEquals('Y', $dom->getDecisionResult());
		$this->assertEquals('IDV-A1,CRA-A1,TLT-A1,REFERBUCKETS-R2,REFERBUCKETS-P1', $dom->getDecisionCode());
	}
	...
	{
		$dom = new DataX_Parser($this->getXML('FBOD-PERF'));
	...
	{
		$dom = new DataX_Parser($this->getXML('STRICT'));
