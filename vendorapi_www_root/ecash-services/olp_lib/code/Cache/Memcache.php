...
class Cache_Memcache
{
	...
	/**
	 * The default compression threshold size.
	 */
	const MEMCACHE_COMPRESS_THRESHOLD = 1000;
	...
	{
		if ($this->module_exists) return $this->memcache->flush();
		return FALSE;
	}
	...
	/**
	 * Sets the automatic compression threshold.
	 *
	 * @param int   $threshold   an int represting the minimum length before attempting to compress
	 * @return bool
	 */
	public function setCompressThreshold($threshold)
	{
		if ($this->module_exists)
		{
			return $this->memcache->setCompressThreshold($threshold);
		}
