			...
			if (is_numeric($application_id) && $good)
			{
				...
				$query = "
					...
					FROM
						...
						LEFT join olp_blackbox.target ON target.target_id=event_log.target_id,
						events,
		...
		public function Find_Target_ID($target)
		{
			...
			if (isset($find) && count($find))
			{
				...
				try
				{
					// build our query
					$query = "
					SELECT LOWER(property_short) AS target, target_id
					FROM
						olp_blackbox.target
					...
					WHERE
						...
						AND property_short ". ((count($find) > 1) ? "IN ('".implode("', '", $find)."')" : "='".reset($find)."'");
					$result = $this->sql->Query($this->database, $query);
				}
				catch (Exception $e)
				{
