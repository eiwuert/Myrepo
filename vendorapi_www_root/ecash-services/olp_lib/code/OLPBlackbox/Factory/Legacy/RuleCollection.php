...
class OLPBlackbox_Factory_Legacy_RuleCollection
{
	...
	public static function getInstance($property_short = 'default')
	{
		...
		{
			$company_name = CompanyData::getCompany($property_short);
		}
		...
		if (!isset(self::$factory_instances[$company_name]))
		{
			switch (strtoupper($company_name))
			{
				...
				case EnterpriseData::COMPANY_AGEAN:
					self::$factory_instances[$company_name] = new OLPBlackbox_Enterprise_Agean_Factory_Legacy_RuleCollection(
						OLPBlackbox_Factory_Legacy_Rule::getInstance($property_short)
					);
					break;
				// CashNet has a special collection since they have special rules that check against
				// bloom filter files to do some basic previous/active customer checks.
				case CompanyData::COMPANY_CASHNET:
					self::$factory_instances[$company_name] = new OLPBlackbox_Factory_Legacy_RuleCollectionCashNet(
						OLPBlackbox_Factory_Legacy_Rule::getInstance($property_short)
					);								
					break;
				case CompanyData::COMPANY_ZIPCASH:
					self::$factory_instances[$company_name] = new OLPBlackbox_Factory_Legacy_RuleCollectionZipCash(
						OLPBlackbox_Factory_Legacy_Rule::getInstance($property_short)
					);								
					break;
				case EnterpriseData::COMPANY_HMS:
					self::$factory_instances[$company_name] = new OLPBlackbox_Enterprise_HMS_Factory_Legacy_RuleCollection(
						OLPBlackbox_Factory_Legacy_Rule::getInstance($property_short)
					);
					break;
				default:
	...
	protected function getDefaultRuleCollection(array $rules, array $allowed_rules = NULL)
	{
		...
		foreach ($rules as $rule_name => $rule_value)
		{
			...
			if ($rule_name == 'income_type')
			{
				// Basically if we're an income type.. We need to attempt to get
				// the new fangled income_source rule.. If that fails, we'll
				// default back to the income_type
				$rule = $this->rule_factory->getRule('income_source', $rules['income_source']);
				if ($rule instanceof Blackbox_IRule)
				{
					$rule_collection->addRule($rule);
					unset($rules['income_source']);
					continue;
				}
			}
