...
class OLPBlackbox_Factory_Legacy_Rule
{
	...
	public static function getInstance($property_short = 'default')
	{
		...
		{
			switch (strtoupper($company_name))
			{
	...
	public function getRule($rule_name, $rule_value)
	{
		...
		if (empty($rule_value) 
			&& ($rule_value !== FALSE)
			// kind of dirty? [#14858] [DY]
			// Added bank_account_type [#17707] [AE]
			&& !in_array($rule_name, array('max_loan_amount_requested','bank_account_type')) 
		)
		{
			return FALSE;
		}
		...
		$event = NULL;
		$stat = NULL;
		$skippable = NULL; // @var boolean if not set, use default settings. 
		switch ($rule_name)
		{
			case 'weekends':
				if ($rule_value === FALSE)
			...
			case 'bank_account_type':
				$rule = new OLPBlackbox_Rule_BankAccountType();
				$rule->setupRule(array(
					OLPBlackbox_Rule::PARAM_FIELD => 'bank_account_type',
			...
			case 'minimum_income':
				$rule = new OLPBlackbox_Rule_MinimumIncome($rule_value);
				$event = 'MIN_INCOME';
				$stat = 'min_income';
				break;
			...
			case 'excluded_states':
				$rule = new OLPBlackbox_Rule_ExcludedStates($rule_value);
				$event = 'EXCL_STATES';
				$stat = 'excl_states';
				break;
			...
			case 'minimum_recur_withheld_ssn':
				$field = 'social_security_number_encrypted';
				$rule = 'MinimumRecur_WithheldSSN'; 
				$event = 'WITHHELD_SSN_RECUR';
				$stat = 'withheld_ssn_recur';
				break;

			case 'minimum_recur_withheld_email':
				$field = 'email_primary';
				$rule = 'MinimumRecur_WithheldEmail'; 
				$event = 'WITHHELD_EMAIL_RECUR';
				$stat = 'withheld_email_recur';
				break;
			
			case 'income_recur':
				if (isset($rule_value['changes']) && isset($rule_value['days']))
				{
					$rule = new OLPBlackbox_Rule_MinimumRecur_Income($rule_value['changes']);
					$rule->setupRule(array(
						Blackbox_StandardRule::PARAM_VALUE => $rule_value['days']
					));
					$event = 'INCOME_RECUR';
				}
				break;
			...
			case 'pay_date_recur':
				if (isset($rule_value['changes']) && isset($rule_value['days']))
				{
					$rule = new OLPBlackbox_Rule_MinimumRecur_PayDate($rule_value['changes']);
					$rule->setupRule(array(
						Blackbox_StandardRule::PARAM_VALUE => $rule_value['days']
					));
					$event = 'PAY_DATE_RECUR';
				}
				break;
			...
			case 'income_type':
				...
				$stat = 'income_type';
				break;
			// Really just a replacement for
			// income_type. Not gonna change event/stats
			// quite yet.
			case 'income_source':
				$field = 'income_type';
				$rule = 'In';
...
/*
			case 'min_loan_amount_requested':
				...
				//********************************************* 
				// This rule should be skippable if the 
				// field is not set as per GFORGE_13818  [AuMa]
				// on a side note this code should actually be
...
*/
			...
			case 'max_loan_amount_requested':
				// This if statement is for CLK campaigns only [#15021] [DY]
				if (($this instanceof OLPBlackbox_Enterprise_CLK_Factory_Legacy_Rule) 
					&& !empty(SiteConfig::getInstance()->clk_max_loan)
					&& is_numeric(SiteConfig::getInstance()->clk_max_loan))
				{
					$rule_value = (int)SiteConfig::getInstance()->clk_max_loan;
				}
				elseif (empty($rule_value) || !is_numeric($rule_value))
				{
					return FALSE;
				}
			...
			case 'identical_first_last_name':
				if ($rule_value === FALSE)
				{
					// If the value is true, we can allow identical names, so dont run this check.
					$field = array('name_first', 'name_last');
					$rule = 'NotCompare';
					$event = 'IDENTICAL_FIRST_LAST_NAME';
					$stat = 'identical_first_last_name';
				}
				break;
		...
		if ( $field || $rule )
		{
			...
			{
				$rule->setSkippable($skippable);
			}			
			else if ($this->getConfig()->blackbox_mode != OLPBlackbox_Config::MODE_BROKER
