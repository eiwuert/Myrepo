<?php
/**
 * OLPBlackbox_Rule_ExcludeDayOfWeek class file.
 *
 * @author Matt Piper <matt.piper@sellingsource.com>
 */

/**
 * Checks to make sure the current date is not in an array of days of
 * the week that should be excluded.
 *
 * @author Matt Piper <matt.piper@sellingsource.com>
 */
class OLPBlackbox_Rule_ExcludeDayOfWeek extends OLPBlackbox_Rule
{
	/**
	...
	/**
	 * Runs the ExcludeDayOfWeek rule.
	 *
	...
	public function runRule(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		// Check to see if the day of the week for today is in a list of days to exclude.
		$today = strtolower(date('D', $utils->getToday()));
