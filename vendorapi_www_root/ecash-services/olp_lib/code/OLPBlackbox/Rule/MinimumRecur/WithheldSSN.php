<?php
/**
 * Withheld Targets SSN mimimum recur check.
 *
 ...
 */
class OLPBlackbox_Rule_MinimumRecur_WithheldSSN extends OLPBlackbox_Rule_MinimumRecur
{
	/**
	...
	protected function checkCache(Blackbox_Data $data, Blackbox_IStateData $state_data)
	{
		...
		$this->cache_key = 'WithheldSSNMinRecur:' . md5($key_data . ':' . $query_date . ':' . $state_data->campaign_name);
	...
	protected function runRecurCheck($data, DateTime $date, $properties)
	{
		...
		$query = sprintf("
			SELECT
				p.application_id
			FROM
				personal_encrypted p USE INDEX (idx_ssn)
		...
		catch (Exception $e)
		{
			$this->getConfig()->applog->Write(
				sprintf("%s:%s - WithheldSSN query failed: %s", __CLASS__, __METHOD__, $e->getMessage())
			);
		}
